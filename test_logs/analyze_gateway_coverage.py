...
def analyze_coverage():
    """分析网关层覆盖率"""
    coverage_file = Path(__file__).parent / "gateway_coverage.json"
    ...
    for file_path, file_data in files.items():
        if 'gateway' not in file_path:
            continue
        
        # 提取子模块名称 (src/gateway/xxx/...)
        parts = file_path.replace('\\', '/').split('/')
        if len(parts) >= 3 and parts[0] == 'src' and parts[1] == 'gateway':
            if len(parts) > 3:
