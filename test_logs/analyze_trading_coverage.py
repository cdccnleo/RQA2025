...
def analyze_coverage():
    """分析交易层覆盖率"""
    coverage_file = Path(__file__).parent / "trading_coverage.json"
    ...
    for file_path, file_data in files.items():
        if 'trading' not in file_path:
            continue
        
        # 提取子模块名称 (src/trading/xxx/...)
        parts = file_path.replace('\\', '/').split('/')
        if len(parts) >= 3 and parts[0] == 'src' and parts[1] == 'trading':
            if len(parts) > 3:
