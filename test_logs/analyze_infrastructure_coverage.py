...
def analyze_coverage():
    """分析基础设施层覆盖率"""
    coverage_file = Path(__file__).parent / "infrastructure_coverage.json"
    ...
    for file_path, file_data in files.items():
        ...
        # 提取子模块名称 (src/infrastructure/xxx/...)
        parts = file_path.replace('\\', '/').split('/')
        if len(parts) >= 3 and parts[0] == 'src' and parts[1] == 'infrastructure':
            if len(parts) > 3:
    ...
    print(f"\n{'子模块':<20} {'覆盖率':<12} {'已覆盖/总数':<15} {'文件数':<10} {'状态'}")
    print("-" * 80)
    ...
    for module_name, module_data in sorted_modules:
        ...
        print(f"{module_name:<20} {coverage_percent:>6.2f}%     {covered:>6}/{total:<6} {file_count:>6}      {status}")
