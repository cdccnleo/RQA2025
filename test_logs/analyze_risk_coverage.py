...
def analyze_coverage():
    """分析风险控制层覆盖率"""
    coverage_file = Path(__file__).parent / "risk_coverage.json"
    ...
    for file_path, file_data in files.items():
        if 'risk' not in file_path:
            continue
        
        # 提取子模块名称 (src/risk/xxx/...)
        parts = file_path.replace('\\', '/').split('/')
        if len(parts) >= 3 and parts[0] == 'src' and parts[1] == 'risk':
            if len(parts) > 3:
