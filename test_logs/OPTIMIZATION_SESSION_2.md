...
#### 1. 优化 `test_risk_calculation_engine_api.py`
- **修复的测试方法**: 20个测试方法
- **主要改进**:
  - 添加了 `_ensure_engine_available()` 辅助方法，简化代码
...
- **修复的测试方法列表**:
  ...
  3. `test_calculate_risk_cvar`
  4. `test_calculate_risk_volatility`
  5. `test_calculate_risk_beta`
  6. `test_calculate_risk_sharpe`
  7. `test_calculate_risk_max_drawdown`
  8. `test_calculate_risk_concentration`
  9. `test_calculate_risk_liquidity`
  10. `test_calculate_risk_correlation`
  11. `test_batch_calculate`
  12. `test_get_engine_stats`
  13. `test_get_calculation_history`
  14. `test_clear_cache`
  15. `test_clear_cache_specific_portfolio`
  16. `test_empty_portfolio_handling`
  17. `test_single_position_portfolio`
  18. `test_portfolio_with_missing_data`
  19. `test_multiple_portfolios`
  20. `test_engine_shutdown`

- **优化效果**:
  - `test_risk_calculation_engine_api.py` 跳过测试从 36 降至 35
  - 净增加 1 个通过测试
