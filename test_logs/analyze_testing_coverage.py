...
def analyze_coverage():
    """分析测试层覆盖率"""
    coverage_file = Path(__file__).parent / "testing_coverage.json"
    ...
    for file_path, file_data in files.items():
        if 'testing' not in file_path:
            continue
        
        # 提取子模块名称 (src/testing/xxx/...)
        parts = file_path.replace('\\', '/').split('/')
        if len(parts) >= 3 and parts[0] == 'src' and parts[1] == 'testing':
            if len(parts) > 3:
