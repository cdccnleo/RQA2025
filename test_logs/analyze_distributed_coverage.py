...
def analyze_coverage():
    """分析分布式协调器层覆盖率"""
    coverage_file = Path(__file__).parent / "distributed_coverage.json"
    ...
    for file_path, file_data in files.items():
        if 'distributed' not in file_path:
            continue
        
        # 提取子模块名称 (src/distributed/xxx/...)
        parts = file_path.replace('\\', '/').split('/')
        if len(parts) >= 3 and parts[0] == 'src' and parts[1] == 'distributed':
            if len(parts) > 3:
