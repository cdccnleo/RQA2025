...
def analyze_coverage():
    """分析工具层覆盖率"""
    coverage_file = Path(__file__).parent / "utils_coverage.json"
    ...
    for file_path, file_data in files.items():
        if 'utils' not in file_path or 'infrastructure/utils' in file_path:
            continue
        
        # 提取子模块名称 (src/utils/xxx/...)
        parts = file_path.replace('\\', '/').split('/')
        if len(parts) >= 3 and parts[0] == 'src' and parts[1] == 'utils':
            if len(parts) > 3:
