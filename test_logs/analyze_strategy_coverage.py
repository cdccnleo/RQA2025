...
def analyze_coverage():
    """分析策略服务层覆盖率"""
    coverage_file = Path(__file__).parent / "strategy_coverage.json"
    ...
    for file_path, file_data in files.items():
        if 'strategy' not in file_path:
            continue
        
        # 提取子模块名称 (src/strategy/xxx/...)
        parts = file_path.replace('\\', '/').split('/')
        if len(parts) >= 3 and parts[0] == 'src' and parts[1] == 'strategy':
            if len(parts) > 3:
