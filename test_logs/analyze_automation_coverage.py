...
def analyze_coverage():
    """分析自动化层覆盖率"""
    coverage_file = Path(__file__).parent / "automation_coverage.json"
    ...
    for file_path, file_data in files.items():
        if 'automation' not in file_path:
            continue
        
        # 提取子模块名称 (src/automation/xxx/...)
        parts = file_path.replace('\\', '/').split('/')
        if len(parts) >= 3 and parts[0] == 'src' and parts[1] == 'automation':
            if len(parts) > 3:
