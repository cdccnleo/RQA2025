...
def analyze_coverage():
    """分析业务边界层覆盖率"""
    coverage_file = Path(__file__).parent / "boundary_coverage.json"
    ...
    for file_path, file_data in files.items():
        if 'boundary' not in file_path:
            continue
        
        # 提取子模块名称 (src/boundary/xxx/...)
        parts = file_path.replace('\\', '/').split('/')
        if len(parts) >= 3 and parts[0] == 'src' and parts[1] == 'boundary':
            if len(parts) > 3:
