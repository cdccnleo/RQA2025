...
def analyze_coverage():
    """分析流处理层覆盖率"""
    coverage_file = Path(__file__).parent / "streaming_coverage.json"
    ...
    for file_path, file_data in files.items():
        if 'streaming' not in file_path:
            continue
        
        # 提取子模块名称 (src/streaming/xxx/...)
        parts = file_path.replace('\\', '/').split('/')
        if len(parts) >= 3 and parts[0] == 'src' and parts[1] == 'streaming':
            if len(parts) > 3:
