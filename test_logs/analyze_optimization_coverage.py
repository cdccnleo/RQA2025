...
def analyze_coverage():
    """分析优化层覆盖率"""
    coverage_file = Path(__file__).parent / "optimization_coverage.json"
    ...
    for file_path, file_data in files.items():
        if 'optimization' not in file_path:
            continue
        
        # 提取子模块名称 (src/optimization/xxx/...)
        parts = file_path.replace('\\', '/').split('/')
        if len(parts) >= 3 and parts[0] == 'src' and parts[1] == 'optimization':
            if len(parts) > 3:
