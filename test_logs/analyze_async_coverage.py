...
def analyze_coverage():
    """分析异步处理器层覆盖率"""
    coverage_file = Path(__file__).parent / "async_coverage.json"
    ...
    for file_path, file_data in files.items():
        if 'async' not in file_path or 'infrastructure' in file_path:
            continue
        
        # 提取子模块名称 (src/async/xxx/...)
        parts = file_path.replace('\\', '/').split('/')
        if len(parts) >= 3 and parts[0] == 'src' and parts[1] == 'async':
            if len(parts) > 3:
