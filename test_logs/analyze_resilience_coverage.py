...
def analyze_coverage():
    """分析弹性层覆盖率"""
    coverage_file = Path(__file__).parent / "resilience_coverage.json"
    ...
    for file_path, file_data in files.items():
        if 'resilience' not in file_path:
            continue
        
        # 提取子模块名称 (src/resilience/xxx/...)
        parts = file_path.replace('\\', '/').split('/')
        if len(parts) >= 3 and parts[0] == 'src' and parts[1] == 'resilience':
            if len(parts) > 3:
