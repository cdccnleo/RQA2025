...
def analyze_coverage():
    """分析监控层覆盖率"""
    coverage_file = Path(__file__).parent / "monitoring_coverage.json"
    ...
    for file_path, file_data in files.items():
        if 'monitoring' not in file_path:
            continue
        
        # 提取子模块名称 (src/monitoring/xxx/...)
        parts = file_path.replace('\\', '/').split('/')
        if len(parts) >= 3 and parts[0] == 'src' and parts[1] == 'monitoring':
            if len(parts) > 3:
