...
def test_component_factory_create_component_exception(monkeypatch):
    """测试ComponentFactory.create_component的异常处理（25-27行）"""
...
def test_component_factory_create_component_initialize_false(monkeypatch):
    """测试ComponentFactory.create_component中initialize返回False（23行）"""
    factory = ComponentFactory()
...
def test_component_factory_create_component_component_none(monkeypatch):
    """测试ComponentFactory.create_component中component为None（23行）"""
    factory = ComponentFactory()
...
def test_component_factory_create_component_success(monkeypatch):
    """测试ComponentFactory.create_component成功返回component（23行）"""
    factory = ComponentFactory()
...
def test_data_processor_component_process_exception(monkeypatch):
    """测试DataProcessorComponent.process的异常处理（102-103行）"""
    component = DataProcessorComponent(processor_id=1)
    
    # Mock the f-string formatting to raise exception
    # We'll mock the component_name attribute access
    original_name = component.component_name
    
    # Temporarily replace component_name with a property that raises exception
    class FailingName:
        def __str__(self):
...
def test_data_processor_component_get_status_exception(monkeypatch):
    """测试DataProcessorComponent.get_status的异常处理（如果存在）"""
    component = DataProcessorComponent(processor_id=1)
    
    # This test verifies get_status works correctly
    # If there's exception handling in get_status, we can test it
