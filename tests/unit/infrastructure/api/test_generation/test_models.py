...
覆盖 models.py 中的 TestCase、TestScenario、TestSuite、TestStatistics 类
"""

import pytest
from datetime import datetime
from src.infrastructure.api.test_generation.models import (
    TestCase,
    TestScenario,
    TestSuite,
    TestStatistics
...
class TestTestStatistics:
    ...
    def test_initialization_custom(self):
        ...
        assert stats.total_scenarios == 15
        assert stats.total_test_cases == 150
        assert stats.passed_tests == 135
        assert stats.failed_tests == 10
        assert stats.pending_tests == 5
        assert stats.execution_time == 125.5
        assert stats.coverage_percentage == 85.5

    def test_pass_rate_property(self):
        """测试通过率属性"""
        stats = TestStatistics(passed_tests=8, failed_tests=2, total_test_cases=10)
        assert stats.pass_rate == 80.0

        stats = TestStatistics(passed_tests=0, failed_tests=0, total_test_cases=0)
