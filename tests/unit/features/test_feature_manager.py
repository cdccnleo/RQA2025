...
from .conftest import sample_features, broken_features
...
class TestFeatureManager:
    ...
    def test_full_pipeline(self, mock_std, mock_sel, mock_eng, sample_features):
        """测试完整特征处理流程"""
        # 配置mock返回值
    ...
    def test_pipeline_failure(self, broken_part, sample_features):
        """测试各环节异常处理"""
        manager = FeatureManager()
    ...
    def test_metadata_versioning(self, sample_features):
        """测试元数据版本更新"""
        manager = FeatureManager()
    ...
    def test_sentiment_analysis_models(self, mock_analyze, sample_features):
        """测试多模型情感分析集成"""
        # 配置多模型返回结果
    ...
    @pytest.mark.parametrize("input_data,expected_cols", [
        ...
        ("sample_features_close", ['ma5', 'ma10']),
        ("sample_features_close_volume", ['ma5', 'ma10', 'obv'])
    ])
    def test_technical_indicators(self, input_data, expected_cols, sample_features):
        """参数化测试技术指标生成"""
        # 处理不同输入
        if input_data == "sample_features":
            data = sample_features
        elif input_data == "sample_features_close":
            data = sample_features[['close']]
        elif input_data == "sample_features_close_volume":
            data = sample_features[['close', 'volume']]
        else:
            data = sample_features
        manager = FeatureManager()
    ...
    def test_feature_selection_strategies(self, sample_features):
        """测试特征选择策略"""
        manager = FeatureManager()
    ...
    def test_edge_cases(self, sample_features):
        """测试边界条件"""
        manager = FeatureManager()
