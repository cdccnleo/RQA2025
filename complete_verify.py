...
try:
    ...
    for source in sources.get('data', []):
        if 'baostock' in source.get('id', '').lower():
            ...
            initial_collection_time = source.get('last_collection_time')
            print(f"最后采集时间: {initial_collection_time or '从未采集'}")
            break
except Exception as e:
    print(f"获取失败: {e}")
...
try:
    ...
    for source in sources.get('data', []):
        if 'baostock' in source.get('id', '').lower():
            print(f"\nbaostock数据源:")
            current_collection_time = source.get('last_collection_time')
            print(f"  最后采集时间: {current_collection_time or '从未采集'}")
            
            if current_collection_time and current_collection_time != initial_collection_time:
                print("\n🎉 修复成功！baostock数据采集完成并已更新采集时间！")
            elif current_collection_time:
                print("\n✅ 数据源已有采集时间记录")
            else:
                print("\n⏳ 采集时间尚未更新，但任务可能已执行")
            break
...
print('\n6. 检查最近日志')
print('-' * 60)
print("请手动运行: docker-compose -f docker-compose.prod.yml logs --tail=50 app")
print("查看是否有任务执行和完成的日志")
