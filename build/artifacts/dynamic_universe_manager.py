    ...
    def _pass_liquidity_filter(self, metrics: pd.Series) -> bool:
        ...
        if hasattr(daily_volume, 'item'):
            try:
                ...
                daily_volume = float(daily_volume.iloc[0]) if hasattr(daily_volume, 'iloc') else float(daily_volume)
        if hasattr(turnover_rate, 'item'):
            try:
                turnover_rate = turnover_rate.item()
            except ValueError:
                turnover_rate = float(turnover_rate.iloc[0]) if hasattr(turnover_rate, 'iloc') else float(turnover_rate)
        if hasattr(market_cap, 'item'):
            try:
                market_cap = market_cap.item()
            except ValueError:
                market_cap = float(market_cap.iloc[0]) if hasattr(market_cap, 'iloc') else float(market_cap)
    ...
    def _pass_fundamental_filter(self, metrics: pd.Series) -> bool:
        ...
        if hasattr(profit_growth, 'item'):
            try:
                ...
                profit_growth = float(profit_growth.iloc[0]) if hasattr(profit_growth, 'iloc') else float(profit_growth)
