...
class ConfigEncryptionService:
    ...
    def encrypt_config(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        def _encrypt_value(value: Any, key: str) -> Any:
            ...
            if isinstance(value, dict):
                return {k: _encrypt_value(v, f"{key}.{k}" if key else k) for k, v in value.items()}
            elif isinstance(value, list):
                return [_encrypt_value(item, f"{key}[{i}]") for i, item in enumerate(value)]
        ...
        encrypted_config = _encrypt_value(config, "")
    ...
    def decrypt_config(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        def _decrypt_value(value: Any, key: str) -> Any:
            """递归解密值"""
            if isinstance(value, dict):
                return {k: _decrypt_value(v, f"{key}.{k}" if key else k) for k, v in value.items()}
            elif isinstance(value, list):
                return [_decrypt_value(item, f"{key}[{i}]") for i, item in enumerate(value)]
    ...
    def encrypt_sensitive_values(self, config: Dict[str, Any], sensitive_keys: List[str]):
        """
        加密指定的敏感键值
