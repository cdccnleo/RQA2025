...
class RiskLayerAdapter(BaseBusinessAdapter):
    ...
    def assess_risk_with_infrastructure(self, risk_request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # 记录风险评估开始
            if monitoring:
                monitoring.record_metric('risk_assessment_start', 1, {'layer': 'risk'})
            ...
            if risk_calculator:
                ...
                # 缓存评估结果
                if cache_manager:
                    cache_manager.set(cache_key, assessment_result, 600)  # 缓存10分钟
                    result['infrastructure_used'].append('cache')

            # 记录风险评估完成
            if monitoring:
                monitoring.record_metric('risk_assessment_complete', 1, {'layer': 'risk'})
            ...
            monitoring = self.get_infrastructure_services().get('monitoring')
            if monitoring:
                monitoring.record_metric('risk_assessment_error', 1, {
                                         'layer': 'risk', 'error': str(e)})
