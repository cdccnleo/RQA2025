...
class TradingLayerAdapter(BaseBusinessAdapter):
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        # 添加交易层特定检查
        trading_specific_health = {
            'trading_engine': self._check_trading_engine_health(),
            'order_manager': self._check_order_manager_health(),
            'execution_engine': self._check_execution_engine_health(),
            'portfolio_manager': self._check_portfolio_manager_health()
        }
    ...
    def _check_trading_engine_health(self) -> Dict[str, Any]:
        """检查交易引擎健康状态"""
        engine = self.get_trading_engine()
    ...
    def _check_order_manager_health(self) -> Dict[str, Any]:
        """检查订单管理器健康状态"""
        manager = self.get_order_manager()
    ...
    def _check_portfolio_manager_health(self) -> Dict[str, Any]:
        """检查投资组合管理器健康状态"""
        manager = self.get_portfolio_manager()
        if manager and hasattr(manager, 'health_check'):
            try:
                return manager.health_check()
            except Exception as e:
    ...
    def get_trading_layer_metrics(self) -> Dict[str, Any]:
        ...
        # 获取交易组件指标
        components = {
            'trading_engine': self.get_trading_engine(),
    ...
    def execute_trade_with_infrastructure(self, trade_request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # 记录交易开始
            if monitoring:
                monitoring.record_metric('trade_execution_start', 1, {'layer': 'trading'})
            ...
            monitoring = self.get_infrastructure_services().get('monitoring')
            if monitoring:
                monitoring.record_metric('trade_execution_error', 1, {
                                         'layer': 'trading', 'error': str(e)})
    ...
    def execute_trading_flow(self, trade_request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if cache_manager and result['success']:
                ...
                cache_manager.set(cache_key, result, 3600)  # 缓存1小时
            ...
            result['errors'].append(f"交易流程执行异常: {e}")
            logger.error(f"交易流程执行失败: {e}")
        ...
        # 计算流程耗时
        result['duration_ms'] = (
            datetime.now() - datetime.fromisoformat(result['timestamp'])).total_seconds() * 1000
