...
处理分布在src/ml/ensemble目录中的多种模板文件：
- bagging_*.py (装袋组件)
- boosting_*.py (提升组件)
- ensemble_*.py (集成组件)
- stacking_*.py (堆叠组件)
...
class MLEnsembleTemplatesOptimizer:
    """ML Ensemble模板文件优化器"""
    ...
    def find_ml_ensemble_files(self):
        """查找ML Ensemble相关模板文件"""
        print("🔍 查找ML Ensemble相关模板文件...")
        print("="*60)
        ...
        # 模板文件模式
        template_patterns = {
            'bagging': r'bagging_(\d+)\.py$',
            'boosting': r'boosting_(\d+)\.py$',
            'ensemble': r'ensemble_(\d+)\.py$',
            'stacking': r'stacking_(\d+)\.py$',
            'voting': r'voting_(\d+)\.py$'
        }

        # 查找src/ml/ensemble目录
        ensemble_dir = Path('src/ml/ensemble')
        if not ensemble_dir.exists():
            print("   ⚠️  src/ml/ensemble目录不存在")
            return []

        for file_path in ensemble_dir.iterdir():
        ...
        for file_info in self.ensemble_files:
            type_groups[file_info['template_type']].append(file_info)

        print(f"   📁 发现 {len(self.ensemble_files)} 个ML Ensemble模板文件")
        print("   📂 按类型分布:")
        for template_type, files in type_groups.items():
            print(f"      - {template_type}_*.py: {len(files)}个文件")

        return self.ensemble_files

    def analyze_ensemble_structure(self):
        """分析ensemble文件结构"""
        print("🔍 分析ensemble文件结构...")

        if not self.ensemble_files:
            return None

        # 读取第一个文件来了解结构
        first_file = self.ensemble_files[0]['path']
        try:
    ...
    def create_ensemble_component_factories(self):
        """为每个模板类型创建组件工厂"""
        print("🏭 创建ML Ensemble组件工厂...")
...
class {component_type}Component(I{component_type}Component):
    ...
    def get_info(self) -> Dict[str, Any]:
        ...
        return {{
            ...
            "version": "2.0.0",
            "type": "unified_ml_ensemble_component"
        }}
...
'''
    ...
    def backup_and_remove_ensemble_files(self, factories):
        """备份并删除ensemble文件"""
        print("📦 备份和删除ML Ensemble文件...")
        ...
        for template_type, factory_info in factories.items():
            ...
            # 创建备份目录
            backup_dir = Path('src/ml/ensemble').parent / \
                f"ensemble_{template_type}_backup_optimization"
    ...
    def save_factories(self, factories):
        ...
        for template_type, factory_info in factories.items():
            factory_file = Path('src/ml/ensemble') / f"{template_type}_components.py"
    ...
    def run_optimization(self):
        """运行优化"""
        print("🚀 开始ML Ensemble模板文件优化...")
        print("="*60)

        try:
            # 1. 查找ML Ensemble文件
            self.find_ml_ensemble_files()

            if not self.ensemble_files:
                print("   ⚠️  未发现需要优化的ML Ensemble文件")
                return None

            # 2. 分析文件结构
            structure_info = self.analyze_ensemble_structure()

            # 3. 创建组件工厂
            factories = self.create_ensemble_component_factories()

            # 4. 保存组件工厂文件
            self.save_factories(factories)

            # 5. 备份并删除ML Ensemble文件
            removed_count = self.backup_and_remove_ensemble_files(factories)

            print("\n" + "="*60)
            print("✅ ML Ensemble模板文件优化完成！")
            print("="*60)

            print("\n📊 优化结果:")
            print(f"   删除ML Ensemble文件: {removed_count}个")
            ...
            return {
                ...
                'ensemble_files': self.ensemble_files,
...
def main():
    """主函数"""
    optimizer = MLEnsembleTemplatesOptimizer()
    result = optimizer.run_optimization()

    if result:
        print("\n🎉 ML Ensemble模板文件优化成功完成！")
