#!/usr/bin/env python3
"""
优化Data Adapters相关模板文件

处理分布在src/data/adapters目录中的多种模板文件：
- connector_*.py (连接器组件)
- provider_*.py (提供者组件)
- source_*.py (源组件)
"""
...
class DataAdaptersTemplatesOptimizer:
    """Data Adapters模板文件优化器"""

    def __init__(self):
        self.data_files = []
        self.backup_dirs = set()

    def find_data_adapters_files(self):
        """查找Data Adapters相关模板文件"""
        print("🔍 查找Data Adapters相关模板文件...")
        print("="*60)
        ...
        template_patterns = {
            ...
            'provider': r'provider_(\d+)\.py$',
            'source': r'source_(\d+)\.py$'
        }

        # 查找src/data/adapters目录
        adapters_dir = Path('src/data/adapters')
        if not adapters_dir.exists():
            print("   ⚠️  src/data/adapters目录不存在")
            return []

        for file_path in adapters_dir.iterdir():
        ...
        print(f"   📁 发现 {len(self.data_files)} 个Data Adapters模板文件")
        print("   📂 按类型分布:")
...
'''
    ...
    def backup_and_remove_data_files(self, factories):
        ...
        for template_type, factory_info in factories.items():
            ...
            # 创建备份目录
            backup_dir = Path('src/data/adapters').parent / \
                f"adapters_{template_type}_backup_optimization"
    ...
    def save_factories(self, factories):
        ...
        for template_type, factory_info in factories.items():
            factory_file = Path('src/data/adapters') / f"{template_type}_components.py"
    ...
    def run_optimization(self):
        """运行优化"""
        print("🚀 开始Data Adapters模板文件优化...")
        print("="*60)

        try:
            # 1. 查找Data Adapters文件
            self.find_data_adapters_files()

            if not self.data_files:
                print("   ⚠️  未发现需要优化的Data Adapters文件")
                return None
            ...
            # 5. 备份并删除Data Adapters文件
            removed_count = self.backup_and_remove_data_files(factories)

            print("\n" + "="*60)
            print("✅ Data Adapters模板文件优化完成！")
            print("="*60)

            print("\n📊 优化结果:")
            print(f"   删除Data Adapters文件: {removed_count}个")
...
def main():
    """主函数"""
    optimizer = DataAdaptersTemplatesOptimizer()
    result = optimizer.run_optimization()

    if result:
        print("\n🎉 Data Adapters模板文件优化成功完成！")
