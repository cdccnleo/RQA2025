#!/usr/bin/env python3
"""
优化Backtest Engine相关模板文件

处理分布在src/backtest/engine目录中的多种模板文件：
- backtest_*.py (回测组件)
- engine_*.py (引擎组件)
- executor_*.py (执行器组件)
- runner_*.py (运行器组件)
...
class BacktestEngineTemplatesOptimizer:
    """Backtest Engine模板文件优化器"""

    def __init__(self):
        self.backtest_files = []
        self.backup_dirs = set()

    def find_backtest_engine_files(self):
        """查找Backtest Engine相关模板文件"""
        print("🔍 查找Backtest Engine相关模板文件...")
        print("="*60)
        ...
        # 模板文件模式
        template_patterns = {
            'backtest': r'backtest_(\d+)\.py$',
            'engine': r'engine_(\d+)\.py$',
            'executor': r'executor_(\d+)\.py$',
            'runner': r'runner_(\d+)\.py$',
            'simulator': r'simulator_(\d+)\.py$'
        }

        # 查找src/backtest/engine目录
        engine_dir = Path('src/backtest/engine')
        if not engine_dir.exists():
            print("   ⚠️  src/backtest/engine目录不存在")
            return []

        for file_path in engine_dir.iterdir():
        ...
        print(f"   📁 发现 {len(self.backtest_files)} 个Backtest Engine模板文件")
        print("   📂 按类型分布:")
...
'''
    ...
    def backup_and_remove_backtest_files(self, factories):
        ...
        for template_type, factory_info in factories.items():
            ...
            # 创建备份目录
            backup_dir = Path('src/backtest/engine').parent / \
                f"engine_{template_type}_backup_optimization"
    ...
    def save_factories(self, factories):
        ...
        for template_type, factory_info in factories.items():
            factory_file = Path('src/backtest/engine') / f"{template_type}_components.py"
    ...
    def run_optimization(self):
        """运行优化"""
        print("🚀 开始Backtest Engine模板文件优化...")
        print("="*60)

        try:
            # 1. 查找Backtest Engine文件
            self.find_backtest_engine_files()

            if not self.backtest_files:
                print("   ⚠️  未发现需要优化的Backtest Engine文件")
                return None
            ...
            # 5. 备份并删除Backtest Engine文件
            removed_count = self.backup_and_remove_backtest_files(factories)

            print("\n" + "="*60)
            print("✅ Backtest Engine模板文件优化完成！")
            print("="*60)

            print("\n📊 优化结果:")
            print(f"   删除Backtest Engine文件: {removed_count}个")
...
def main():
    """主函数"""
    optimizer = BacktestEngineTemplatesOptimizer()
    result = optimizer.run_optimization()

    if result:
        print("\n🎉 Backtest Engine模板文件优化成功完成！")
