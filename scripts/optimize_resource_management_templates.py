#!/usr/bin/env python3
"""
优化Resource Management相关模板文件

处理分布在src/infrastructure/resource目录中的多种模板文件：
- monitor_*.py (监控器组件)
- pool_*.py (资源池组件)
- quota_*.py (配额组件)
- resource_*.py (资源组件)
...
class ResourceManagementTemplatesOptimizer:
    """Resource Management模板文件优化器"""

    def __init__(self):
        self.resource_files = []
        self.backup_dirs = set()

    def find_resource_management_files(self):
        """查找Resource Management相关模板文件"""
        print("🔍 查找Resource Management相关模板文件...")
        print("="*60)
        ...
        template_patterns = {
            ...
            'pool': r'pool_(\d+)\.py$',
            'quota': r'quota_(\d+)\.py$',
            'resource': r'resource_(\d+)\.py$'
        }

        # 查找src/infrastructure/resource目录
        resource_dir = Path('src/infrastructure/resource')
        if not resource_dir.exists():
            print("   ⚠️  src/infrastructure/resource目录不存在")
            return []
        ...
        for file_info in self.resource_files:
            type_groups[file_info['template_type']].append(file_info)

        print(f"   📁 发现 {len(self.resource_files)} 个Resource Management模板文件")
        print("   📂 按类型分布:")
    ...
    def analyze_resource_structure(self):
        """分析resource文件结构"""
        print("🔍 分析resource文件结构...")

        if not self.resource_files:
            return None

        # 读取第一个文件来了解结构
        first_file = self.resource_files[0]['path']
        try:
    ...
    def create_resource_component_factories(self):
        """为每个模板类型创建组件工厂"""
        print("🏭 创建Resource Management组件工厂...")
...
'''
    ...
    def backup_and_remove_resource_files(self, factories):
        """备份并删除resource文件"""
        print("📦 备份和删除Resource Management文件...")
        ...
        for template_type, factory_info in factories.items():
            ...
            # 创建备份目录
            backup_dir = Path('src/infrastructure/resource').parent / \
                f"resource_{template_type}_backup_optimization"
    ...
    def save_factories(self, factories):
        ...
        for template_type, factory_info in factories.items():
            factory_file = Path('src/infrastructure/resource') / f"{template_type}_components.py"
    ...
    def run_optimization(self):
        """运行优化"""
        print("🚀 开始Resource Management模板文件优化...")
        print("="*60)

        try:
            # 1. 查找Resource Management文件
            self.find_resource_management_files()

            if not self.resource_files:
                print("   ⚠️  未发现需要优化的Resource Management文件")
                return None

            # 2. 分析文件结构
            structure_info = self.analyze_resource_structure()

            # 3. 创建组件工厂
            factories = self.create_resource_component_factories()

            # 4. 保存组件工厂文件
            self.save_factories(factories)

            # 5. 备份并删除Resource Management文件
            removed_count = self.backup_and_remove_resource_files(factories)

            print("\n" + "="*60)
            print("✅ Resource Management模板文件优化完成！")
            print("="*60)

            print("\n📊 优化结果:")
            print(f"   删除Resource Management文件: {removed_count}个")
...
def main():
    """主函数"""
    optimizer = ResourceManagementTemplatesOptimizer()
    result = optimizer.run_optimization()

    if result:
        print("\n🎉 Resource Management模板文件优化成功完成！")
