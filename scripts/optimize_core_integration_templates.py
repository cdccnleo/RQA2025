#!/usr/bin/env python3
"""
优化Core Integration相关模板文件

处理分布在src/core/integration目录中的多种模板文件：
- bridge_*.py (桥接组件)
- connector_*.py (连接器组件)
- integration_*.py (集成组件)
- middleware_*.py (中间件组件)
"""
...
class CoreIntegrationTemplatesOptimizer:
    """Core Integration模板文件优化器"""

    def __init__(self):
        self.core_files = []
        self.backup_dirs = set()

    def find_core_integration_files(self):
        """查找Core Integration相关模板文件"""
        print("🔍 查找Core Integration相关模板文件...")
        print("="*60)
        ...
        # 模板文件模式
        template_patterns = {
            'bridge': r'bridge_(\d+)\.py$',
            'connector': r'connector_(\d+)\.py$',
            'integration': r'integration_(\d+)\.py$',
            'middleware': r'middleware_(\d+)\.py$'
        }

        # 查找src/core/integration目录
        integration_dir = Path('src/core/integration')
        if not integration_dir.exists():
            print("   ⚠️  src/core/integration目录不存在")
            return []
        ...
        print(f"   📁 发现 {len(self.core_files)} 个Core Integration模板文件")
        print("   📂 按类型分布:")
...
'''
    ...
    def backup_and_remove_core_files(self, factories):
        ...
        for template_type, factory_info in factories.items():
            ...
            # 创建备份目录
            backup_dir = Path('src/core/integration').parent / \
                f"integration_{template_type}_backup_optimization"
    ...
    def save_factories(self, factories):
        ...
        for template_type, factory_info in factories.items():
            factory_file = Path('src/core/integration') / f"{template_type}_components.py"
    ...
    def run_optimization(self):
        """运行优化"""
        print("🚀 开始Core Integration模板文件优化...")
        print("="*60)

        try:
            # 1. 查找Core Integration文件
            self.find_core_integration_files()

            if not self.core_files:
                print("   ⚠️  未发现需要优化的Core Integration文件")
                return None
            ...
            # 5. 备份并删除Core Integration文件
            removed_count = self.backup_and_remove_core_files(factories)

            print("\n" + "="*60)
            print("✅ Core Integration模板文件优化完成！")
            print("="*60)

            print("\n📊 优化结果:")
            print(f"   删除Core Integration文件: {removed_count}个")
...
def main():
    """主函数"""
    optimizer = CoreIntegrationTemplatesOptimizer()
    result = optimizer.run_optimization()

    if result:
        print("\n🎉 Core Integration模板文件优化成功完成！")
