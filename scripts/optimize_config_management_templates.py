#!/usr/bin/env python3
"""
优化Config Management相关模板文件

处理分布在src/infrastructure/config目录中的多种模板文件：
- config_*.py (配置组件)
- loader_*.py (加载器组件)
- validator_*.py (验证器组件)
"""
...
class ConfigManagementTemplatesOptimizer:
    """Config Management模板文件优化器"""

    def __init__(self):
        self.config_files = []
        self.backup_dirs = set()

    def find_config_management_files(self):
        """查找Config Management相关模板文件"""
        print("🔍 查找Config Management相关模板文件...")
        print("="*60)
        ...
        template_patterns = {
            ...
            'loader': r'loader_(\d+)\.py$',
            'validator': r'validator_(\d+)\.py$'
        }

        # 查找src/infrastructure/config目录
        config_dir = Path('src/infrastructure/config')
        if not config_dir.exists():
            print("   ⚠️  src/infrastructure/config目录不存在")
            return []
        ...
        print(f"   📁 发现 {len(self.config_files)} 个Config Management模板文件")
        print("   📂 按类型分布:")
    ...
    def analyze_config_structure(self):
        ...
        # 读取第一个文件来了解结构
        first_file = self.config_files[0]['path']
        try:
    ...
    def create_config_component_factories(self):
        """为每个模板类型创建组件工厂"""
        print("🏭 创建Config Management组件工厂...")
...
class {component_type}Component(I{component_type}Component):
    ...
    def get_info(self) -> Dict[str, Any]:
        ...
        return {{
            ...
            "version": "2.0.0",
            "type": "unified_config_management_component"
        }}
...
'''
    ...
    def backup_and_remove_config_files(self, factories):
        """备份并删除config文件"""
        print("📦 备份和删除Config Management文件...")
        ...
        for template_type, factory_info in factories.items():
            ...
            # 创建备份目录
            backup_dir = Path('src/infrastructure/config').parent / \
                f"config_{template_type}_backup_optimization"
    ...
    def save_factories(self, factories):
        ...
        for template_type, factory_info in factories.items():
            factory_file = Path('src/infrastructure/config') / f"{template_type}_components.py"
    ...
    def run_optimization(self):
        """运行优化"""
        print("🚀 开始Config Management模板文件优化...")
        print("="*60)

        try:
            # 1. 查找Config Management文件
            self.find_config_management_files()

            if not self.config_files:
                print("   ⚠️  未发现需要优化的Config Management文件")
                return None

            # 2. 分析文件结构
            structure_info = self.analyze_config_structure()

            # 3. 创建组件工厂
            factories = self.create_config_component_factories()

            # 4. 保存组件工厂文件
            self.save_factories(factories)

            # 5. 备份并删除Config Management文件
            removed_count = self.backup_and_remove_config_files(factories)

            print("\n" + "="*60)
            print("✅ Config Management模板文件优化完成！")
            print("="*60)

            print("\n📊 优化结果:")
            print(f"   删除Config Management文件: {removed_count}个")
...
def main():
    """主函数"""
    optimizer = ConfigManagementTemplatesOptimizer()
    result = optimizer.run_optimization()

    if result:
        print("\n🎉 Config Management模板文件优化成功完成！")
