#!/usr/bin/env python3
"""
优化Logging System相关模板文件

处理分布在src/infrastructure/logging目录中的多种模板文件：
- logger_*.py (日志组件)
- formatter_*.py (格式化组件)
- config_*.py (配置组件)
"""
...
class LoggingSystemTemplatesOptimizer:
    """Logging System模板文件优化器"""

    def __init__(self):
        self.logging_files = []
        self.backup_dirs = set()

    def find_logging_system_files(self):
        """查找Logging System相关模板文件"""
        print("🔍 查找Logging System相关模板文件...")
        print("="*60)
        ...
        # 模板文件模式
        template_patterns = {
            'logger': r'logger_(\d+)\.py$',
            'formatter': r'formatter_(\d+)\.py$',
            'config': r'config_(\d+)\.py$'
        }

        # 查找src/infrastructure/logging目录
        logging_dir = Path('src/infrastructure/logging')
        if not logging_dir.exists():
            print("   ⚠️  src/infrastructure/logging目录不存在")
            return []

        for file_path in logging_dir.iterdir():
        ...
        for file_info in self.logging_files:
            type_groups[file_info['template_type']].append(file_info)

        print(f"   📁 发现 {len(self.logging_files)} 个Logging System模板文件")
        print("   📂 按类型分布:")
    ...
    def analyze_logging_structure(self):
        """分析logging文件结构"""
        print("🔍 分析logging文件结构...")

        if not self.logging_files:
            return None

        # 读取第一个文件来了解结构
        first_file = self.logging_files[0]['path']
        try:
    ...
    def create_logging_component_factories(self):
        """为每个模板类型创建组件工厂"""
        print("🏭 创建Logging System组件工厂...")
...
'''
    ...
    def backup_and_remove_logging_files(self, factories):
        """备份并删除logging文件"""
        print("📦 备份和删除Logging System文件...")
        ...
        for template_type, factory_info in factories.items():
            ...
            # 创建备份目录
            backup_dir = Path('src/infrastructure/logging').parent / \
                f"logging_{template_type}_backup_optimization"
    ...
    def save_factories(self, factories):
        ...
        for template_type, factory_info in factories.items():
            factory_file = Path('src/infrastructure/logging') / f"{template_type}_components.py"
    ...
    def run_optimization(self):
        """运行优化"""
        print("🚀 开始Logging System模板文件优化...")
        print("="*60)

        try:
            # 1. 查找Logging System文件
            self.find_logging_system_files()

            if not self.logging_files:
                print("   ⚠️  未发现需要优化的Logging System文件")
                return None

            # 2. 分析文件结构
            structure_info = self.analyze_logging_structure()

            # 3. 创建组件工厂
            factories = self.create_logging_component_factories()

            # 4. 保存组件工厂文件
            self.save_factories(factories)

            # 5. 备份并删除Logging System文件
            removed_count = self.backup_and_remove_logging_files(factories)

            print("\n" + "="*60)
            print("✅ Logging System模板文件优化完成！")
            print("="*60)

            print("\n📊 优化结果:")
            print(f"   删除Logging System文件: {removed_count}个")
...
def main():
    """主函数"""
    optimizer = LoggingSystemTemplatesOptimizer()
    result = optimizer.run_optimization()

    if result:
        print("\n🎉 Logging System模板文件优化成功完成！")
