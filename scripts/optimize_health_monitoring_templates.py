#!/usr/bin/env python3
"""
优化Health Monitoring相关模板文件

处理分布在src/infrastructure/health目录中的多种模板文件：
- alert_*.py (告警组件)
- checker_*.py (检查器组件)
- health_*.py (健康组件)
- monitor_*.py (监控器组件)
...
class HealthMonitoringTemplatesOptimizer:
    """Health Monitoring模板文件优化器"""

    def __init__(self):
        self.health_files = []
        self.backup_dirs = set()

    def find_health_monitoring_files(self):
        """查找Health Monitoring相关模板文件"""
        print("🔍 查找Health Monitoring相关模板文件...")
        print("="*60)
        ...
        template_patterns = {
            ...
            'health': r'health_(\d+)\.py$',
            'monitor': r'monitor_(\d+)\.py$',
            'probe': r'probe_(\d+)\.py$',
            'status': r'status_(\d+)\.py$'
        }

        # 查找src/infrastructure/health目录
        health_dir = Path('src/infrastructure/health')
        if not health_dir.exists():
            print("   ⚠️  src/infrastructure/health目录不存在")
            return []

        for file_path in health_dir.iterdir():
        ...
        for file_info in self.health_files:
            type_groups[file_info['template_type']].append(file_info)

        print(f"   📁 发现 {len(self.health_files)} 个Health Monitoring模板文件")
        print("   📂 按类型分布:")
        for template_type, files in type_groups.items():
            print(f"      - {template_type}_*.py: {len(files)}个文件")

        return self.health_files

    def analyze_health_structure(self):
        """分析health文件结构"""
        print("🔍 分析health文件结构...")

        if not self.health_files:
            return None

        # 读取第一个文件来了解结构
        first_file = self.health_files[0]['path']
        try:
    ...
    def create_health_component_factories(self):
        """为每个模板类型创建组件工厂"""
        print("🏭 创建Health Monitoring组件工厂...")
...
'''
    ...
    def backup_and_remove_health_files(self, factories):
        """备份并删除health文件"""
        print("📦 备份和删除Health Monitoring文件...")
        ...
        for template_type, factory_info in factories.items():
            ...
            # 创建备份目录
            backup_dir = Path('src/infrastructure/health').parent / \
                f"health_{template_type}_backup_optimization"
    ...
    def save_factories(self, factories):
        ...
        for template_type, factory_info in factories.items():
            factory_file = Path('src/infrastructure/health') / f"{template_type}_components.py"
    ...
    def run_optimization(self):
        """运行优化"""
        print("🚀 开始Health Monitoring模板文件优化...")
        print("="*60)

        try:
            # 1. 查找Health Monitoring文件
            self.find_health_monitoring_files()

            if not self.health_files:
                print("   ⚠️  未发现需要优化的Health Monitoring文件")
                return None

            # 2. 分析文件结构
            structure_info = self.analyze_health_structure()

            # 3. 创建组件工厂
            factories = self.create_health_component_factories()

            # 4. 保存组件工厂文件
            self.save_factories(factories)

            # 5. 备份并删除Health Monitoring文件
            removed_count = self.backup_and_remove_health_files(factories)

            print("\n" + "="*60)
            print("✅ Health Monitoring模板文件优化完成！")
            print("="*60)

            print("\n📊 优化结果:")
            print(f"   删除Health Monitoring文件: {removed_count}个")
            ...
            return {
                ...
                'health_files': self.health_files,
...
def main():
    """主函数"""
    optimizer = HealthMonitoringTemplatesOptimizer()
    result = optimizer.run_optimization()

    if result:
        print("\n🎉 Health Monitoring模板文件优化成功完成！")
