...
def test_gpu_functionality(processor: GPUTechnicalProcessor, data: pd.DataFrame):
    """测试GPU功能"""
    print("\n" + "="*60)
    print("GPU功能测试")
    print("="*60)
    ...
    for indicator in indicators:
        ...
        try:
            ...
            if isinstance(result, pd.Series):
                ...
                print(f"  ✅ CPU计算成功，结果形状: {cpu_result.shape}")
                print(f"  ✅ 结果类型一致: {type(result) == type(cpu_result)}")
    ...
    try:
        ...
        print(f"  ✅ GPU多指标计算成功，结果形状: {multi_result.shape}")
        print(f"  ✅ CPU多指标计算成功，结果形状: {cpu_multi_result.shape}")
...
def main():
    ...
    # 功能测试
    performance_results = test_gpu_functionality(processor, data)
    ...
    successful_tests = [k for k, v in performance_results.items() if v.get('success', False)]
    failed_tests = [k for k, v in performance_results.items() if not v.get('success', False)]

    print(f"成功测试: {len(successful_tests)}/{len(performance_results)}")
    if successful_tests:
        print(f"成功指标: {', '.join(successful_tests)}")

    if failed_tests:
        print(f"失败指标: {', '.join(failed_tests)}")
        for test in failed_tests:
            error = performance_results[test].get('error', '未知错误')
            print(f"  {test}: {error}")

    # 性能统计
    successful_results = {k: v for k, v in performance_results.items() if v.get('success', False)}
    if successful_results:
        avg_speedup = np.mean([result['speedup'] for result in successful_results.values()])
        max_speedup = max([result['speedup'] for result in successful_results.values()])
        min_speedup = min([result['speedup'] for result in successful_results.values()])
    ...
    if processor.gpu_available:
        print("\n✅ GPU加速功能正常工作")
        if successful_results and avg_speedup > 1.0:
            print("✅ GPU提供了性能提升")
        else:
