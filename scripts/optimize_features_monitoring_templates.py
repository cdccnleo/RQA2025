#!/usr/bin/env python3
"""
优化Features Monitoring相关模板文件

处理分布在src/features/monitoring目录中的多种模板文件：
- analyzer_*.py (分析器组件)
- metrics_*.py (指标组件)
- monitor_*.py (监控组件)
- profiler_*.py (性能分析组件)
...
class FeaturesMonitoringTemplatesOptimizer:
    """Features Monitoring模板文件优化器"""

    def __init__(self):
        self.monitoring_files = []
        self.backup_dirs = set()

    def find_features_monitoring_files(self):
        """查找Features Monitoring相关模板文件"""
        print("🔍 查找Features Monitoring相关模板文件...")
        print("="*60)
        ...
        # 模板文件模式
        template_patterns = {
            'analyzer': r'analyzer_(\d+)\.py$',
            'metrics': r'metrics_(\d+)\.py$',
            'monitor': r'monitor_(\d+)\.py$',
            'profiler': r'profiler_(\d+)\.py$',
            'tracker': r'tracker_(\d+)\.py$'
        }

        # 查找src/features/monitoring目录
        monitoring_dir = Path('src/features/monitoring')
        if not monitoring_dir.exists():
            print("   ⚠️  src/features/monitoring目录不存在")
            return []

        for file_path in monitoring_dir.iterdir():
        ...
        for file_info in self.monitoring_files:
            type_groups[file_info['template_type']].append(file_info)

        print(f"   📁 发现 {len(self.monitoring_files)} 个Features Monitoring模板文件")
        print("   📂 按类型分布:")
        for template_type, files in type_groups.items():
            print(f"      - {template_type}_*.py: {len(files)}个文件")

        return self.monitoring_files

    def analyze_monitoring_structure(self):
        """分析monitoring文件结构"""
        print("🔍 分析monitoring文件结构...")

        if not self.monitoring_files:
            return None

        # 读取第一个文件来了解结构
        first_file = self.monitoring_files[0]['path']
        try:
    ...
    def create_monitoring_component_factories(self):
        """为每个模板类型创建组件工厂"""
        print("🏭 创建Features Monitoring组件工厂...")
...
class {component_type}Component(I{component_type}Component):
    ...
    def get_info(self) -> Dict[str, Any]:
        ...
        return {{
            ...
            "version": "2.0.0",
            "type": "unified_features_monitoring_component"
        }}
...
'''
    ...
    def backup_and_remove_monitoring_files(self, factories):
        """备份并删除monitoring文件"""
        print("📦 备份和删除Features Monitoring文件...")
        ...
        for template_type, factory_info in factories.items():
            ...
            # 创建备份目录
            backup_dir = Path('src/features/monitoring').parent / \
                f"monitoring_{template_type}_backup_optimization"
    ...
    def save_factories(self, factories):
        ...
        for template_type, factory_info in factories.items():
            factory_file = Path('src/features/monitoring') / f"{template_type}_components.py"
    ...
    def run_optimization(self):
        """运行优化"""
        print("🚀 开始Features Monitoring模板文件优化...")
        print("="*60)

        try:
            # 1. 查找Features Monitoring文件
            self.find_features_monitoring_files()

            if not self.monitoring_files:
                print("   ⚠️  未发现需要优化的Features Monitoring文件")
                return None

            # 2. 分析文件结构
            structure_info = self.analyze_monitoring_structure()

            # 3. 创建组件工厂
            factories = self.create_monitoring_component_factories()

            # 4. 保存组件工厂文件
            self.save_factories(factories)

            # 5. 备份并删除Features Monitoring文件
            removed_count = self.backup_and_remove_monitoring_files(factories)

            print("\n" + "="*60)
            print("✅ Features Monitoring模板文件优化完成！")
            print("="*60)

            print("\n📊 优化结果:")
            print(f"   删除Features Monitoring文件: {removed_count}个")
            ...
            return {
                ...
                'monitoring_files': self.monitoring_files,
...
def main():
    """主函数"""
    optimizer = FeaturesMonitoringTemplatesOptimizer()
    result = optimizer.run_optimization()

    if result:
        print("\n🎉 Features Monitoring模板文件优化成功完成！")
