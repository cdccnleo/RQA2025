...
class DataCacheTemplatesOptimizer:
    """Data Cache模板文件优化器"""
    ...
    def find_data_cache_files(self):
        """查找Data Cache相关模板文件"""
        print("🔍 查找Data Cache相关模板文件...")
        print("="*60)
        ...
        print(f"   📁 发现 {len(self.data_files)} 个Data Cache模板文件")
        print("   📂 按类型分布:")
...
'''
    ...
    def backup_and_remove_data_files(self, factories):
        ...
        for template_type, factory_info in factories.items():
            ...
            # 创建备份目录
            backup_dir = Path('src/data/cache').parent / \
                f"cache_{template_type}_backup_optimization"
    ...
    def save_factories(self, factories):
        ...
        for template_type, factory_info in factories.items():
            factory_file = Path('src/data/cache') / f"{template_type}_components.py"
    ...
    def run_optimization(self):
        """运行优化"""
        print("🚀 开始Data Cache模板文件优化...")
        print("="*60)

        try:
            # 1. 查找Data Cache文件
            self.find_data_cache_files()

            if not self.data_files:
                print("   ⚠️  未发现需要优化的Data Cache文件")
                return None
            ...
            print("\n" + "="*60)
            print("✅ Data Cache模板文件优化完成！")
            print("="*60)

            print("\n📊 优化结果:")
            print(f"   删除Data Cache文件: {removed_count}个")
...
def main():
    """主函数"""
    optimizer = DataCacheTemplatesOptimizer()
    result = optimizer.run_optimization()

    if result:
        print("\n🎉 Data Cache模板文件优化成功完成！")
