...
"""
RQA2025 引擎层统一Web管理界面快速访问脚本
自动检测服务状态并打开浏览器访问引擎层的统一Web管理界面
"""
...
def open_dashboard(host: str = "127.0.0.1"):
    ...
    if port is None:
        logger.error("未找到运行中的引擎层统一Web管理界面服务")
        logger.info("请先启动服务:")
        logger.info("  python scripts/engine/web/start_dashboard.py")
        return False
    ...
    logger.info("============================================================")
    logger.info("RQA2025 引擎层统一Web管理界面")
    logger.info("============================================================")
    ...
    try:
        ...
        logger.info("已自动打开浏览器访问引擎层统一Web管理界面")
        return True
...
def show_service_info():
    ...
    logger.info("- 前端界面: 现代化响应式设计，实时数据可视化")


def show_architecture_info():
    """显示架构信息"""
    logger.info("引擎层架构信息:")
    logger.info("- 位置: src/engine/web/")
    logger.info("- 主程序: unified_dashboard.py")
    logger.info("- 模块化组件: modules/")
    logger.info("- 前端模板: templates/dashboard.html")
    logger.info("- 静态资源: static/css/dashboard.css, static/js/dashboard.js")
