#!/usr/bin/env python3
"""
优化Core Event Bus相关模板文件

处理分布在src/core/event_bus目录中的多种模板文件：
- bus_*.py (总线组件)
- dispatcher_*.py (分发器组件)
- event_*.py (事件组件)
- publisher_*.py (发布者组件)
- subscriber_*.py (订阅者组件)
"""
...
class CoreEventBusTemplatesOptimizer:
    """Core Event Bus模板文件优化器"""

    def __init__(self):
        self.core_files = []
        self.backup_dirs = set()

    def find_core_event_bus_files(self):
        """查找Core Event Bus相关模板文件"""
        print("🔍 查找Core Event Bus相关模板文件...")
        print("="*60)
        ...
        template_patterns = {
            'bus': r'bus_(\d+)\.py$',
            'dispatcher': r'dispatcher_(\d+)\.py$',
            'event': r'event_(\d+)\.py$',
            'publisher': r'publisher_(\d+)\.py$',
            'subscriber': r'subscriber_(\d+)\.py$'
        }

        # 查找src/core/event_bus目录
        event_bus_dir = Path('src/core/event_bus')
        if not event_bus_dir.exists():
            print("   ⚠️  src/core/event_bus目录不存在")
            return []

        for file_path in event_bus_dir.iterdir():
        ...
        print(f"   📁 发现 {len(self.core_files)} 个Core Event Bus模板文件")
        print("   📂 按类型分布:")
...
'''
    ...
    def backup_and_remove_core_files(self, factories):
        ...
        for template_type, factory_info in factories.items():
            ...
            # 创建备份目录
            backup_dir = Path('src/core/event_bus').parent / \
                f"event_bus_{template_type}_backup_optimization"
    ...
    def save_factories(self, factories):
        ...
        for template_type, factory_info in factories.items():
            factory_file = Path('src/core/event_bus') / f"{template_type}_components.py"
    ...
    def run_optimization(self):
        """运行优化"""
        print("🚀 开始Core Event Bus模板文件优化...")
        print("="*60)

        try:
            # 1. 查找Core Event Bus文件
            self.find_core_event_bus_files()

            if not self.core_files:
                print("   ⚠️  未发现需要优化的Core Event Bus文件")
                return None
            ...
            # 5. 备份并删除Core Event Bus文件
            removed_count = self.backup_and_remove_core_files(factories)

            print("\n" + "="*60)
            print("✅ Core Event Bus模板文件优化完成！")
            print("="*60)

            print("\n📊 优化结果:")
            print(f"   删除Core Event Bus文件: {removed_count}个")
...
def main():
    """主函数"""
    optimizer = CoreEventBusTemplatesOptimizer()
    result = optimizer.run_optimization()

    if result:
        print("\n🎉 Core Event Bus模板文件优化成功完成！")
