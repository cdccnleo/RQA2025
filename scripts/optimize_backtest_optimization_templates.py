#!/usr/bin/env python3
"""
优化Backtest Optimization相关模板文件

处理分布在src/backtest/optimization目录中的多种模板文件：
...
class BacktestOptimizationTemplatesOptimizer:
    """Backtest Optimization模板文件优化器"""

    def __init__(self):
        self.backtest_files = []
        self.backup_dirs = set()

    def find_backtest_optimization_files(self):
        """查找Backtest Optimization相关模板文件"""
        print("🔍 查找Backtest Optimization相关模板文件...")
        print("="*60)
        ...
        template_patterns = {
            ...
            'parameter': r'parameter_(\d+)\.py$',
            'tuning': r'tuning_(\d+)\.py$'
        }

        # 查找src/backtest/optimization目录
        optimization_dir = Path('src/backtest/optimization')
        if not optimization_dir.exists():
            print("   ⚠️  src/backtest/optimization目录不存在")
            return []
        ...
        for file_info in self.backtest_files:
            type_groups[file_info['template_type']].append(file_info)

        print(f"   📁 发现 {len(self.backtest_files)} 个Backtest Optimization模板文件")
        print("   📂 按类型分布:")
    ...
    def analyze_backtest_structure(self):
        """分析backtest文件结构"""
        print("🔍 分析backtest文件结构...")

        if not self.backtest_files:
            return None

        # 读取第一个文件来了解结构
        first_file = self.backtest_files[0]['path']
        try:
    ...
    def create_backtest_component_factories(self):
        """为每个模板类型创建组件工厂"""
        print("🏭 创建Backtest Optimization组件工厂...")
...
'''
    ...
    def backup_and_remove_backtest_files(self, factories):
        """备份并删除backtest文件"""
        print("📦 备份和删除Backtest Optimization文件...")
        ...
        for template_type, factory_info in factories.items():
            ...
            # 创建备份目录
            backup_dir = Path('src/backtest/optimization').parent / \
                f"optimization_{template_type}_backup_optimization"
    ...
    def save_factories(self, factories):
        ...
        for template_type, factory_info in factories.items():
            factory_file = Path('src/backtest/optimization') / f"{template_type}_components.py"
    ...
    def run_optimization(self):
        """运行优化"""
        print("🚀 开始Backtest Optimization模板文件优化...")
        print("="*60)

        try:
            # 1. 查找Backtest Optimization文件
            self.find_backtest_optimization_files()

            if not self.backtest_files:
                print("   ⚠️  未发现需要优化的Backtest Optimization文件")
                return None

            # 2. 分析文件结构
            structure_info = self.analyze_backtest_structure()

            # 3. 创建组件工厂
            factories = self.create_backtest_component_factories()

            # 4. 保存组件工厂文件
            self.save_factories(factories)

            # 5. 备份并删除Backtest Optimization文件
            removed_count = self.backup_and_remove_backtest_files(factories)

            print("\n" + "="*60)
            print("✅ Backtest Optimization模板文件优化完成！")
            print("="*60)

            print("\n📊 优化结果:")
            print(f"   删除Backtest Optimization文件: {removed_count}个")
            ...
            return {
                ...
                'backtest_files': self.backtest_files,
...
def main():
    """主函数"""
    optimizer = BacktestOptimizationTemplatesOptimizer()
    result = optimizer.run_optimization()

    if result:
        print("\n🎉 Backtest Optimization模板文件优化成功完成！")
