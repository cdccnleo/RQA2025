#!/usr/bin/env python3
"""
优化Data Monitoring相关模板文件

处理分布在src/data/monitoring目录中的多种模板文件：
- metrics_*.py (指标组件)
- monitor_*.py (监控组件)
- observer_*.py (观察者组件)
- tracker_*.py (跟踪器组件)
...
class DataMonitoringTemplatesOptimizer:
    """Data Monitoring模板文件优化器"""

    def __init__(self):
        self.monitoring_files = []
        self.backup_dirs = set()

    def find_data_monitoring_files(self):
        """查找Data Monitoring相关模板文件"""
        print("🔍 查找Data Monitoring相关模板文件...")
        print("="*60)
        ...
        template_patterns = {
            ...
            'observer': r'observer_(\d+)\.py$',
            'tracker': r'tracker_(\d+)\.py$',
            'watcher': r'watcher_(\d+)\.py$'
        }

        # 查找src/data/monitoring目录
        monitoring_dir = Path('src/data/monitoring')
        if not monitoring_dir.exists():
            print("   ⚠️  src/data/monitoring目录不存在")
            return []
        ...
        print(f"   📁 发现 {len(self.monitoring_files)} 个Data Monitoring模板文件")
        print("   📂 按类型分布:")
...
'''
    ...
    def backup_and_remove_monitoring_files(self, factories):
        ...
        for template_type, factory_info in factories.items():
            ...
            # 创建备份目录
            backup_dir = Path('src/data/monitoring').parent / \
    ...
    def save_factories(self, factories):
        ...
        for template_type, factory_info in factories.items():
            factory_file = Path('src/data/monitoring') / f"{template_type}_components.py"
    ...
    def run_optimization(self):
        """运行优化"""
        print("🚀 开始Data Monitoring模板文件优化...")
        print("="*60)

        try:
            # 1. 查找Data Monitoring文件
            self.find_data_monitoring_files()

            if not self.monitoring_files:
                print("   ⚠️  未发现需要优化的Data Monitoring文件")
                return None
            ...
            # 5. 备份并删除Data Monitoring文件
            removed_count = self.backup_and_remove_monitoring_files(factories)

            print("\n" + "="*60)
            print("✅ Data Monitoring模板文件优化完成！")
            print("="*60)

            print("\n📊 优化结果:")
            print(f"   删除Data Monitoring文件: {removed_count}个")
...
def main():
    """主函数"""
    optimizer = DataMonitoringTemplatesOptimizer()
    result = optimizer.run_optimization()

    if result:
        print("\n🎉 Data Monitoring模板文件优化成功完成！")
