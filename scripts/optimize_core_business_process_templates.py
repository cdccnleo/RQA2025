#!/usr/bin/env python3
"""
优化Core Business Process相关模板文件

处理分布在src/core/business_process目录中的多种模板文件：
- coordinator_*.py (协调器组件)
- orchestrator_*.py (编排器组件)
- process_*.py (流程组件)
- workflow_*.py (工作流组件)
"""
...
class CoreBusinessProcessTemplatesOptimizer:
    """Core Business Process模板文件优化器"""

    def __init__(self):
        self.core_files = []
        self.backup_dirs = set()

    def find_core_business_process_files(self):
        """查找Core Business Process相关模板文件"""
        print("🔍 查找Core Business Process相关模板文件...")
        print("="*60)
        ...
        # 模板文件模式
        template_patterns = {
            'coordinator': r'coordinator_(\d+)\.py$',
            'orchestrator': r'orchestrator_(\d+)\.py$',
            'process': r'process_(\d+)\.py$',
            'workflow': r'workflow_(\d+)\.py$'
        }

        # 查找src/core/business_process目录
        business_process_dir = Path('src/core/business_process')
        if not business_process_dir.exists():
            print("   ⚠️  src/core/business_process目录不存在")
            return []

        for file_path in business_process_dir.iterdir():
        ...
        print(f"   📁 发现 {len(self.core_files)} 个Core Business Process模板文件")
        print("   📂 按类型分布:")
    ...
    def create_core_component_factories(self):
        """为每个模板类型创建组件工厂"""
        print("🏭 创建Core Business Process组件工厂...")
...
'''
    ...
    def backup_and_remove_core_files(self, factories):
        """备份并删除core文件"""
        print("📦 备份和删除Core Business Process文件...")
        ...
        for template_type, factory_info in factories.items():
            ...
            # 创建备份目录
            backup_dir = Path('src/core/business_process').parent / \
                f"business_process_{template_type}_backup_optimization"
    ...
    def save_factories(self, factories):
        ...
        for template_type, factory_info in factories.items():
            factory_file = Path('src/core/business_process') / f"{template_type}_components.py"
    ...
    def run_optimization(self):
        """运行优化"""
        print("🚀 开始Core Business Process模板文件优化...")
        print("="*60)

        try:
            # 1. 查找Core Business Process文件
            self.find_core_business_process_files()

            if not self.core_files:
                print("   ⚠️  未发现需要优化的Core Business Process文件")
                return None
            ...
            # 5. 备份并删除Core Business Process文件
            removed_count = self.backup_and_remove_core_files(factories)

            print("\n" + "="*60)
            print("✅ Core Business Process模板文件优化完成！")
            print("="*60)

            print("\n📊 优化结果:")
            print(f"   删除Core Business Process文件: {removed_count}个")
...
def main():
    """主函数"""
    optimizer = CoreBusinessProcessTemplatesOptimizer()
    result = optimizer.run_optimization()

    if result:
        print("\n🎉 Core Business Process模板文件优化成功完成！")
