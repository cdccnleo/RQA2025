#!/usr/bin/env python3
"""
优化Infrastructure Utils相关模板文件

处理分布在src/infrastructure/utils目录中的多种模板文件：
- base_*.py (基础组件)
- common_*.py (通用组件)
- factory_*.py (工厂组件)
- helper_*.py (辅助组件)
...
class InfrastructureUtilsTemplatesOptimizer:
    """Infrastructure Utils模板文件优化器"""

    def __init__(self):
        self.utils_files = []
        self.backup_dirs = set()

    def find_infrastructure_utils_files(self):
        """查找Infrastructure Utils相关模板文件"""
        print("🔍 查找Infrastructure Utils相关模板文件...")
        print("="*60)
        ...
        template_patterns = {
            'base': r'base_(\d+)\.py$',
            'common': r'common_(\d+)\.py$',
            'factory': r'factory_(\d+)\.py$',
            'helper': r'helper_(\d+)\.py$',
            'tool': r'tool_(\d+)\.py$',
            'util': r'util_(\d+)\.py$'
        }

        # 查找src/infrastructure/utils目录
        utils_dir = Path('src/infrastructure/utils')
        if not utils_dir.exists():
            print("   ⚠️  src/infrastructure/utils目录不存在")
            return []

        for file_path in utils_dir.iterdir():
        ...
        for file_info in self.utils_files:
            type_groups[file_info['template_type']].append(file_info)

        print(f"   📁 发现 {len(self.utils_files)} 个Infrastructure Utils模板文件")
        print("   📂 按类型分布:")
        for template_type, files in type_groups.items():
            print(f"      - {template_type}_*.py: {len(files)}个文件")

        return self.utils_files

    def analyze_utils_structure(self):
        """分析utils文件结构"""
        print("🔍 分析utils文件结构...")

        if not self.utils_files:
            return None

        # 读取第一个文件来了解结构
        first_file = self.utils_files[0]['path']
        try:
    ...
    def create_utils_component_factories(self):
        """为每个模板类型创建组件工厂"""
        print("🏭 创建Infrastructure Utils组件工厂...")
...
'''
    ...
    def backup_and_remove_utils_files(self, factories):
        """备份并删除utils文件"""
        print("📦 备份和删除Infrastructure Utils文件...")
        ...
        for template_type, factory_info in factories.items():
            ...
            # 创建备份目录
            backup_dir = Path('src/infrastructure/utils').parent / \
                f"utils_{template_type}_backup_optimization"
    ...
    def save_factories(self, factories):
        ...
        for template_type, factory_info in factories.items():
            factory_file = Path('src/infrastructure/utils') / f"{template_type}_components.py"
    ...
    def run_optimization(self):
        """运行优化"""
        print("🚀 开始Infrastructure Utils模板文件优化...")
        print("="*60)

        try:
            # 1. 查找Infrastructure Utils文件
            self.find_infrastructure_utils_files()

            if not self.utils_files:
                print("   ⚠️  未发现需要优化的Infrastructure Utils文件")
                return None

            # 2. 分析文件结构
            structure_info = self.analyze_utils_structure()

            # 3. 创建组件工厂
            factories = self.create_utils_component_factories()

            # 4. 保存组件工厂文件
            self.save_factories(factories)

            # 5. 备份并删除Infrastructure Utils文件
            removed_count = self.backup_and_remove_utils_files(factories)

            print("\n" + "="*60)
            print("✅ Infrastructure Utils模板文件优化完成！")
            print("="*60)

            print("\n📊 优化结果:")
            print(f"   删除Infrastructure Utils文件: {removed_count}个")
...
def main():
    """主函数"""
    optimizer = InfrastructureUtilsTemplatesOptimizer()
    result = optimizer.run_optimization()

    if result:
        print("\n🎉 Infrastructure Utils模板文件优化成功完成！")
