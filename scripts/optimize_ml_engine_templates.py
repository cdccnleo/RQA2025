...
处理分布在src/ml/engine目录中的多种模板文件：
- classifier_*.py (分类器组件)
- engine_*.py (引擎组件)
- inference_*.py (推理组件)
- predictor_*.py (预测器组件)
- regressor_*.py (回归器组件)
...
class MLEngineTemplatesOptimizer:
    """ML Engine模板文件优化器"""
    ...
    def find_ml_engine_files(self):
        """查找ML Engine相关模板文件"""
        print("🔍 查找ML Engine相关模板文件...")
        print("="*60)
        ...
        # 模板文件模式
        template_patterns = {
            'classifier': r'classifier_(\d+)\.py$',
            'engine': r'engine_(\d+)\.py$',
            'inference': r'inference_(\d+)\.py$',
            'predictor': r'predictor_(\d+)\.py$',
            'regressor': r'regressor_(\d+)\.py$'
        }

        # 查找src/ml/engine目录
        engine_dir = Path('src/ml/engine')
        if not engine_dir.exists():
            print("   ⚠️  src/ml/engine目录不存在")
            return []
        ...
        for file_info in self.ml_files:
            type_groups[file_info['template_type']].append(file_info)

        print(f"   📁 发现 {len(self.ml_files)} 个ML Engine模板文件")
        print("   📂 按类型分布:")
        for template_type, files in type_groups.items():
            print(f"      - {template_type}_*.py: {len(files)}个文件")

        return self.ml_files

    def analyze_ml_structure(self):
        """分析ml文件结构"""
        print("🔍 分析ml文件结构...")

        if not self.ml_files:
            return None

        # 读取第一个文件来了解结构
        first_file = self.ml_files[0]['path']
        try:
    ...
    def create_ml_component_factories(self):
        """为每个模板类型创建组件工厂"""
        print("🏭 创建ML Engine组件工厂...")
...
class {component_type}Component(I{component_type}Component):
    ...
    def get_info(self) -> Dict[str, Any]:
        ...
        return {{
            ...
            "version": "2.0.0",
            "type": "unified_ml_engine_component"
        }}
...
'''
    ...
    def backup_and_remove_ml_files(self, factories):
        """备份并删除ml文件"""
        print("📦 备份和删除ML Engine文件...")
        ...
        for template_type, factory_info in factories.items():
            ...
            # 创建备份目录
            backup_dir = Path('src/ml/engine').parent / \
    ...
    def save_factories(self, factories):
        ...
        for template_type, factory_info in factories.items():
            factory_file = Path('src/ml/engine') / f"{template_type}_components.py"
    ...
    def run_optimization(self):
        """运行优化"""
        print("🚀 开始ML Engine模板文件优化...")
        print("="*60)

        try:
            # 1. 查找ML Engine文件
            self.find_ml_engine_files()

            if not self.ml_files:
                print("   ⚠️  未发现需要优化的ML Engine文件")
                return None

            # 2. 分析文件结构
            structure_info = self.analyze_ml_structure()

            # 3. 创建组件工厂
            factories = self.create_ml_component_factories()

            # 4. 保存组件工厂文件
            self.save_factories(factories)

            # 5. 备份并删除ML Engine文件
            removed_count = self.backup_and_remove_ml_files(factories)

            print("\n" + "="*60)
            print("✅ ML Engine模板文件优化完成！")
            print("="*60)

            print("\n📊 优化结果:")
            print(f"   删除ML Engine文件: {removed_count}个")
...
def main():
    """主函数"""
    optimizer = MLEngineTemplatesOptimizer()
