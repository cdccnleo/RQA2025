...
from src.adapters.market_adapters import (
    MarketAdapterManager, MarketType, AStockAdapter,
    HStockAdapter, USStockAdapter, FuturesAdapter, CryptoAdapter
)
import sys
import os
import pandas as pd
...
def demo_qmt_integration():
    ...
    try:
        ...
        qmt_adapter = QMTAdapter(config=qmt_config)

        print("   🔌 QMT适配器配置:")
        print(f"      主机: {qmt_adapter.host}:{qmt_adapter.port}")
        print(f"      用户名: {qmt_adapter.username}")
        print(f"      账户ID: {qmt_adapter.account_id}")

        print("\n   📊 QMT连接状态:")
        print(f"      当前状态: {qmt_adapter.connection_status.value}")
        print(f"      是否连接: {qmt_adapter.is_connected()}")
...
def demo_real_time_system():
    ...
    try:
        ...
        print("\n   📋 统计信息:" print(f"      处理数据点: {processor.stats['data_processed']}")
...
def demo_monitoring_system():
    ...
    try:
        ...
        print("\n   🎯 监控系统特点:")
        print("      • 实时性能监控" print("      • 策略表现跟踪" print("      • 风险指标监控" print("      • 智能告警系统" print("      • 历史数据存储"
...
def demo_strategy_optimization():
    ...
    try:
        ...
        # 定义参数范围
        parameter_ranges=[
            ParameterRange("short_period", 3, 15, step=1, value_type="int"),
            ParameterRange("long_period", 15, 40, step=2, value_type="int"),
        ]
        ...
        print("   ✅ 优化完成" print(f"      执行时间: {result.execution_time.total_seconds():.2f}秒")
        ...
        if len(result.optimization_history) > 0:
            scores=[h['score'] for h in result.optimization_history[-10:]]  # 最后10个
            avg_score=np.mean(scores)
            best_in_recent=max(scores)
            print(f"      最近10次平均分数: {avg_score:.4f}")
            print(f"      最近10次最佳分数: {best_in_recent:.4f}")

        print("\n   🎯 优化方法:")
        print("      • 网格搜索" print("      • 随机搜索" print("      • 贝叶斯优化" print("      • 遗传算法" print("      • 粒子群优化"
