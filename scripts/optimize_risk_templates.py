...
class RiskTemplatesOptimizer:
    ...
    def find_risk_files(self):
        ...
        template_patterns = {
            ...
            'compliance': {
                'compliance': r'compliance_(\d+)\.py$',
                'policy': r'policy_(\d+)\.py$',
                'regulator': r'regulator_(\d+)\.py$',
                'rule': r'rule_(\d+)\.py$',
                'standard': r'standard_(\d+)\.py$'
            },
            'monitor': {
                'alert': r'alert_(\d+)\.py$',
        ...
        if not risk_dir.exists():
            print("   ⚠️  src/risk目录不存在")
            return []

        for subdir_name, patterns in template_patterns.items():
            subdir = risk_dir / subdir_name
            ...
            for file_path in subdir.iterdir():
                if file_path.is_file() and file_path.suffix == '.py':
                    ...
                    for template_type, pattern in patterns.items():
                        match = re.match(pattern, filename)
                        if match:
                            template_id = int(match.group(1))
                            size_kb = file_path.stat().st_size / 1024
                            self.risk_files.append({
                                'path': file_path,
    ...
    def analyze_risk_structure(self):
        ...
        # 读取第一个文件来了解结构
        first_file = self.risk_files[0]['path']
        try:
    ...
    def create_risk_component_factories(self):
        """为每个模板类型创建组件工厂"""
        print("🏭 创建Risk组件工厂...")
        ...
        for template_type, files in type_groups.items():
            ...
            # 获取目录信息
            category = files[0]['category'] if files else 'risk'
...
'''
    ...
    def backup_and_remove_risk_files(self, factories):
        """备份并删除risk文件"""
        print("📦 备份和删除Risk文件...")
        ...
        for template_type, factory_info in factories.items():
            ...
            # 创建备份目录
            backup_dir = Path('src/risk').parent / f"risk_{template_type}_backup_optimization"
    ...
    def save_factories(self, factories):
        ...
        for template_type, factory_info in factories.items():
            ...
            factory_file = Path('src/risk') / category / f"{template_type}_components.py"
    ...
    def run_optimization(self):
        ...
        try:
            # 1. 查找Risk文件
            self.find_risk_files()

            if not self.risk_files:
                print("   ⚠️  未发现需要优化的Risk文件")
                return None

            # 2. 分析文件结构
            structure_info = self.analyze_risk_structure()

            # 3. 创建组件工厂
            factories = self.create_risk_component_factories()

            # 4. 保存组件工厂文件
            self.save_factories(factories)

            # 5. 备份并删除Risk文件
            removed_count = self.backup_and_remove_risk_files(factories)
            ...
            print("\n📊 优化结果:")
            print(f"   删除Risk文件: {removed_count}个")
...
def main():
    """主函数"""
    optimizer = RiskTemplatesOptimizer()
