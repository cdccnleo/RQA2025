#!/usr/bin/env python3
"""
优化Feature Engineering相关模板文件

处理分布在src/features/engineering目录中的多种模板文件：
- builder_*.py (构建器组件)
- creator_*.py (创建器组件)
- engineer_*.py (工程师组件)
- extractor_*.py (提取器组件)
...
class FeatureEngineeringTemplatesOptimizer:
    """Feature Engineering模板文件优化器"""

    def __init__(self):
        self.feature_files = []
        self.backup_dirs = set()

    def find_feature_engineering_files(self):
        """查找Feature Engineering相关模板文件"""
        print("🔍 查找Feature Engineering相关模板文件...")
        print("="*60)
        ...
        # 模板文件模式
        template_patterns = {
            'builder': r'builder_(\d+)\.py$',
            'creator': r'creator_(\d+)\.py$',
            'engineer': r'engineer_(\d+)\.py$',
            'extractor': r'extractor_(\d+)\.py$',
            'generator': r'generator_(\d+)\.py$'
        }

        # 查找src/features/engineering目录
        engineering_dir = Path('src/features/engineering')
        if not engineering_dir.exists():
            print("   ⚠️  src/features/engineering目录不存在")
            return []

        for file_path in engineering_dir.iterdir():
        ...
        for file_info in self.feature_files:
            type_groups[file_info['template_type']].append(file_info)

        print(f"   📁 发现 {len(self.feature_files)} 个Feature Engineering模板文件")
        print("   📂 按类型分布:")
    ...
    def analyze_feature_structure(self):
        ...
        # 读取第一个文件来了解结构
        first_file = self.feature_files[0]['path']
        try:
    ...
    def create_feature_component_factories(self):
        """为每个模板类型创建组件工厂"""
        print("🏭 创建Feature Engineering组件工厂...")
...
'''
    ...
    def backup_and_remove_feature_files(self, factories):
        """备份并删除feature文件"""
        print("📦 备份和删除Feature Engineering文件...")
        ...
        for template_type, factory_info in factories.items():
            ...
            # 创建备份目录
            backup_dir = Path('src/features/engineering').parent / \
                f"engineering_{template_type}_backup_optimization"
    ...
    def save_factories(self, factories):
        ...
        for template_type, factory_info in factories.items():
            factory_file = Path('src/features/engineering') / f"{template_type}_components.py"
    ...
    def run_optimization(self):
        """运行优化"""
        print("🚀 开始Feature Engineering模板文件优化...")
        print("="*60)

        try:
            # 1. 查找Feature Engineering文件
            self.find_feature_engineering_files()

            if not self.feature_files:
                print("   ⚠️  未发现需要优化的Feature Engineering文件")
                return None

            # 2. 分析文件结构
            structure_info = self.analyze_feature_structure()

            # 3. 创建组件工厂
            factories = self.create_feature_component_factories()

            # 4. 保存组件工厂文件
            self.save_factories(factories)

            # 5. 备份并删除Feature Engineering文件
            removed_count = self.backup_and_remove_feature_files(factories)

            print("\n" + "="*60)
            print("✅ Feature Engineering模板文件优化完成！")
            print("="*60)

            print("\n📊 优化结果:")
            print(f"   删除Feature Engineering文件: {removed_count}个")
...
def main():
    """主函数"""
    optimizer = FeatureEngineeringTemplatesOptimizer()
    result = optimizer.run_optimization()

    if result:
        print("\n🎉 Feature Engineering模板文件优化成功完成！")
