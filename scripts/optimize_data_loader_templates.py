...
处理分布在src/data/loader目录中的多种模板文件：
- collector_*.py (收集器组件)
- fetcher_*.py (获取器组件)
- importer_*.py (导入器组件)
- loader_*.py (加载器组件)
...
class DataLoaderTemplatesOptimizer:
    """Data Loader模板文件优化器"""
    ...
    def find_data_loader_files(self):
        """查找Data Loader相关模板文件"""
        print("🔍 查找Data Loader相关模板文件...")
        print("="*60)
        ...
        # 模板文件模式
        template_patterns = {
            'collector': r'collector_(\d+)\.py$',
            'fetcher': r'fetcher_(\d+)\.py$',
            'importer': r'importer_(\d+)\.py$',
            'loader': r'loader_(\d+)\.py$',
            'reader': r'reader_(\d+)\.py$'
        }

        # 查找src/data/loader目录
        loader_dir = Path('src/data/loader')
        if not loader_dir.exists():
            print("   ⚠️  src/data/loader目录不存在")
            return []

        for file_path in loader_dir.iterdir():
        ...
        print(f"   📁 发现 {len(self.data_files)} 个Data Loader模板文件")
        print("   📂 按类型分布:")
...
'''
    ...
    def backup_and_remove_data_files(self, factories):
        ...
        for template_type, factory_info in factories.items():
            ...
            # 创建备份目录
            backup_dir = Path('src/data/loader').parent / \
                f"loader_{template_type}_backup_optimization"
    ...
    def save_factories(self, factories):
        ...
        for template_type, factory_info in factories.items():
            factory_file = Path('src/data/loader') / f"{template_type}_components.py"
    ...
    def run_optimization(self):
        """运行优化"""
        print("🚀 开始Data Loader模板文件优化...")
        print("="*60)

        try:
            # 1. 查找Data Loader文件
            self.find_data_loader_files()

            if not self.data_files:
                print("   ⚠️  未发现需要优化的Data Loader文件")
                return None
            ...
            # 5. 备份并删除Data Loader文件
            removed_count = self.backup_and_remove_data_files(factories)

            print("\n" + "="*60)
            print("✅ Data Loader模板文件优化完成！")
            print("="*60)

            print("\n📊 优化结果:")
            print(f"   删除Data Loader文件: {removed_count}个")
...
def main():
    """主函数"""
    optimizer = DataLoaderTemplatesOptimizer()
    result = optimizer.run_optimization()

    if result:
        print("\n🎉 Data Loader模板文件优化成功完成！")
