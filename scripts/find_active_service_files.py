...
def find_active_service_files():
    """查找实际存在的service_*.py文件（排除备份目录）"""
    print("🔍 查找实际存在的service_*.py文件...")
    print("="*60)
    ...
    for root, dirs, files in os.walk('.'):
        ...
        for file in files:
            if file.endswith('.py') and re.match(r'service_\d+\.py$', file):
                file_path = Path(root) / file
                match = re.search(r'service_(\d+)\.py$', file)
                if match:
    ...
    print(f"   📁 发现 {len(service_files)} 个实际存在的service_*.py文件")
    ...
    if service_files:
        ...
        print("   ✅ 没有发现需要处理的service_*.py文件")

    return service_files


if __name__ == "__main__":
    find_active_service_files()
