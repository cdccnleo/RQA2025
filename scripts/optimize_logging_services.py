#!/usr/bin/env python3
"""
优化infrastructure/logging目录下的service文件

专门处理src/infrastructure/logging/目录中的service_*.py文件
"""
...
class LoggingServiceOptimizer:
    """Logging Service优化器"""

    def __init__(self):
        self.logging_dir = Path("src/infrastructure/logging")
        self.backup_dir = self.logging_dir.parent.parent / \
            "infrastructure_logging_backup_service_optimization"
        self.service_files = []

    def find_service_files(self):
        """查找service文件"""
        print("🔍 查找infrastructure/logging目录中的service文件...")

        for file_path in self.logging_dir.glob("service_*.py"):
...
"""
统一Logging Service组件工厂

合并所有logging service_*.py模板文件为统一的管理架构
...
class BaseService:
    ...
    def __init__(self, service_name: str = "Logging_BaseService"):
        """初始化服务"""
    ...
    async def process_request(self, request_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            result = {{
                ...
                "result": f"Processed Logging Service {{self.service_id}} request",
...
class LoggingServiceComponentFactory:
    """Logging Service组件工厂"""
    ...
    def create_component(service_id: int) -> LoggingServiceComponent:
        ...
        if service_id not in LoggingServiceComponentFactory.SUPPORTED_SERVICE_IDS:
            raise ValueError(f"不支持的服务ID: {{service_id}}。支持的ID: {{LoggingServiceComponentFactory.SUPPORTED_SERVICE_IDS}}")
    ...
    def get_factory_info() -> Dict[str, Any]:
        ...
        return {{
            ...
            "description": "统一Logging Service组件工厂，替代原有的{{len(self.service_files)}}个模板化文件"
        }}
...
'''
        ...
        for service_id in service_ids:
            factory_content += f"def create_logging_service_component_{service_id}(): return LoggingServiceComponentFactory.create_component({service_id})\n"
...
'''
        ...
        # 添加所有兼容性函数到__all__
        for service_id in service_ids:
            factory_content += f'    "create_logging_service_component_{service_id}",\n'
    ...
    def run_optimization(self):
        """运行优化"""
        print("🚀 开始Infrastructure Logging Service文件优化...")
        print("="*60)
        ...
        try:
            ...
            # 3. 写入新的组件文件
            factory_file = self.logging_dir / "logging_service_components.py"
            with open(factory_file, 'w', encoding='utf-8') as f:
                f.write(factory_content)
            print(f"✅ 创建组件文件: logging_service_components.py")

            # 4. 备份并删除service文件
            removed_count = self.backup_and_remove_service_files()

            print("\n" + "="*60)
            print("✅ Infrastructure Logging Service文件优化完成！")
            print("="*60)

            print("\n📊 优化结果:")
            print(f"   删除service文件: {removed_count}个")
            print(f"   新增统一组件: 1个 (logging_service_components.py)")
...
def main():
    """主函数"""
    optimizer = LoggingServiceOptimizer()
    result = optimizer.run_optimization()

    if result:
        print("\n🎉 Infrastructure Logging Service优化成功完成！")
        print(f"共清理了 {result['removed_count']} 个重复模板文件")
        print("创建了 1 个统一组件工厂")
    else:
        print("\n❌ Infrastructure Logging Service优化失败！")
