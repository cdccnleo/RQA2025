#!/usr/bin/env python3
"""
优化Engine Monitoring相关模板文件

处理分布在src/engine/monitoring目录中的多种模板文件：
- health_*.py (健康检查组件)
- metrics_*.py (指标组件)
- monitoring_*.py (监控组件)
- monitor_*.py (监控器组件)
...
class EngineMonitoringTemplatesOptimizer:
    """Engine Monitoring模板文件优化器"""

    def __init__(self):
        self.engine_files = []
        self.backup_dirs = set()

    def find_engine_monitoring_files(self):
        """查找Engine Monitoring相关模板文件"""
        print("🔍 查找Engine Monitoring相关模板文件...")
        print("="*60)
        ...
        template_patterns = {
            ...
            'monitoring': r'monitoring_(\d+)\.py$',
            'monitor': r'monitor_(\d+)\.py$',
            'status': r'status_(\d+)\.py$'
        }

        # 查找src/engine/monitoring目录
        monitoring_dir = Path('src/engine/monitoring')
        if not monitoring_dir.exists():
            print("   ⚠️  src/engine/monitoring目录不存在")
            return []
        ...
        print(f"   📁 发现 {len(self.engine_files)} 个Engine Monitoring模板文件")
        print("   📂 按类型分布:")
...
'''
    ...
    def backup_and_remove_engine_files(self, factories):
        """备份并删除engine文件"""
        print("📦 备份和删除Engine Monitoring文件...")
        ...
        for template_type, factory_info in factories.items():
            ...
            # 创建备份目录
            backup_dir = Path('src/engine/monitoring').parent / \
    ...
    def save_factories(self, factories):
        ...
        for template_type, factory_info in factories.items():
            factory_file = Path('src/engine/monitoring') / f"{template_type}_components.py"
    ...
    def run_optimization(self):
        """运行优化"""
        print("🚀 开始Engine Monitoring模板文件优化...")
        print("="*60)

        try:
            # 1. 查找Engine Monitoring文件
            self.find_engine_monitoring_files()

            if not self.engine_files:
                print("   ⚠️  未发现需要优化的Engine Monitoring文件")
                return None
            ...
            # 5. 备份并删除Engine Monitoring文件
            removed_count = self.backup_and_remove_engine_files(factories)

            print("\n" + "="*60)
            print("✅ Engine Monitoring模板文件优化完成！")
            print("="*60)

            print("\n📊 优化结果:")
            print(f"   删除Engine Monitoring文件: {removed_count}个")
...
def main():
    """主函数"""
    optimizer = EngineMonitoringTemplatesOptimizer()
    result = optimizer.run_optimization()

    if result:
        print("\n🎉 Engine Monitoring模板文件优化成功完成！")
