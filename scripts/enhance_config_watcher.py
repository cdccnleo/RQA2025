...
def main():
    ...
    try:
        ...
        create_observer_pattern = re.compile(
            r'def _create_observer\(self\).*?return observer', re.DOTALL)
        ...
        debounce_pattern = re.compile(
            r'def _debounce\(self, env: str, callback: Callable\[\[\], None\]\) -> None:.*?self._debounce_timers\[env\] = timer', re.DOTALL)
        ...
        init_pattern = re.compile(
            r'def __init__\(self, config_dir: str, debounce_interval: float = 1.0\):.*?self._manager = None', re.DOTALL)
