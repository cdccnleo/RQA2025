#!/usr/bin/env python3
"""
优化Error Handling相关模板文件

处理分布在src/infrastructure/error目录中的多种模板文件：
- error_*.py (错误处理组件)
- exception_*.py (异常处理组件)
- fallback_*.py (降级处理组件)
- recovery_*.py (恢复处理组件)
"""
...
class ErrorHandlingTemplatesOptimizer:
    """Error Handling模板文件优化器"""

    def __init__(self):
        self.error_files = []
        self.backup_dirs = set()

    def find_error_handling_files(self):
        """查找Error Handling相关模板文件"""
        print("🔍 查找Error Handling相关模板文件...")
        print("="*60)
        ...
        template_patterns = {
            'error': r'error_(\d+)\.py$',
            'exception': r'exception_(\d+)\.py$',
            'fallback': r'fallback_(\d+)\.py$',
            'recovery': r'recovery_(\d+)\.py$'
        }

        # 查找src/infrastructure/error目录
        error_dir = Path('src/infrastructure/error')
        if not error_dir.exists():
            print("   ⚠️  src/infrastructure/error目录不存在")
            return []
        ...
        print(f"   📁 发现 {len(self.error_files)} 个Error Handling模板文件")
        print("   📂 按类型分布:")
    ...
    def analyze_error_structure(self):
        """分析error文件结构"""
        print("🔍 分析error文件结构...")
        ...
        # 读取第一个文件来了解结构
        first_file = self.error_files[0]['path']
        try:
    ...
    def create_error_component_factories(self):
        """为每个模板类型创建组件工厂"""
        print("🏭 创建Error Handling组件工厂...")
...
class {component_type}Component(I{component_type}Component):
    ...
    def get_info(self) -> Dict[str, Any]:
        ...
        return {{
            ...
            "version": "2.0.0",
            "type": "unified_error_handling_component"
        }}
...
'''
    ...
    def backup_and_remove_error_files(self, factories):
        """备份并删除error文件"""
        print("📦 备份和删除Error Handling文件...")
        ...
        for template_type, factory_info in factories.items():
            ...
            # 创建备份目录
            backup_dir = Path('src/infrastructure/error').parent / \
                f"error_{template_type}_backup_optimization"
    ...
    def save_factories(self, factories):
        ...
        for template_type, factory_info in factories.items():
            factory_file = Path('src/infrastructure/error') / f"{template_type}_components.py"
    ...
    def run_optimization(self):
        """运行优化"""
        print("🚀 开始Error Handling模板文件优化...")
        print("="*60)

        try:
            # 1. 查找Error Handling文件
            self.find_error_handling_files()

            if not self.error_files:
                print("   ⚠️  未发现需要优化的Error Handling文件")
                return None

            # 2. 分析文件结构
            structure_info = self.analyze_error_structure()

            # 3. 创建组件工厂
            factories = self.create_error_component_factories()

            # 4. 保存组件工厂文件
            self.save_factories(factories)

            # 5. 备份并删除Error Handling文件
            removed_count = self.backup_and_remove_error_files(factories)

            print("\n" + "="*60)
            print("✅ Error Handling模板文件优化完成！")
            print("="*60)

            print("\n📊 优化结果:")
            print(f"   删除Error Handling文件: {removed_count}个")
...
def main():
    """主函数"""
    optimizer = ErrorHandlingTemplatesOptimizer()
    result = optimizer.run_optimization()

    if result:
        print("\n🎉 Error Handling模板文件优化成功完成！")
