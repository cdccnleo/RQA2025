...
处理分布在src/trading/execution目录中的多种模板文件：
- execution_*.py (交易执行组件)
- executor_*.py (执行器组件)
- trader_*.py (交易者组件)
"""
...
class ExecutionEngineTemplatesOptimizer:
    """Execution Engine模板文件优化器"""

    def __init__(self):
        self.execution_files = []
        self.backup_dirs = set()

    def find_execution_engine_files(self):
        """查找execution引擎相关模板文件"""
        print("🔍 查找execution引擎相关模板文件...")
        print("="*60)
        ...
        # 模板文件模式
        template_patterns = {
            'execution': r'execution_(\d+)\.py$',
            'executor': r'executor_(\d+)\.py$',
            'trader': r'trader_(\d+)\.py$'
        }

        # 查找src/trading/execution目录
        execution_dir = Path('src/trading/execution')
        if not execution_dir.exists():
            print("   ⚠️  src/trading/execution目录不存在")
            return []
        ...
        for file_info in self.execution_files:
            type_groups[file_info['template_type']].append(file_info)

        print(f"   📁 发现 {len(self.execution_files)} 个execution引擎模板文件")
        print("   📂 按类型分布:")
        for template_type, files in type_groups.items():
            print(f"      - {template_type}_*.py: {len(files)}个文件")

        return self.execution_files

    def analyze_execution_structure(self):
        """分析execution文件结构"""
        print("🔍 分析execution文件结构...")

        if not self.execution_files:
            return None

        # 读取第一个文件来了解结构
        first_file = self.execution_files[0]['path']
        try:
    ...
    def create_execution_component_factories(self):
        """为每个模板类型创建组件工厂"""
        print("🏭 创建execution组件工厂...")
...
class {component_type}Component(I{component_type}Component):
    ...
    def get_info(self) -> Dict[str, Any]:
        ...
        return {{
            ...
            "version": "2.0.0",
            "type": "unified_execution_engine_component"
        }}
...
'''
    ...
    def backup_and_remove_execution_files(self, factories):
        """备份并删除execution文件"""
        print("📦 备份和删除execution文件...")
        ...
        for template_type, factory_info in factories.items():
            ...
            # 创建备份目录
            backup_dir = Path('src/trading/execution').parent / \
                f"execution_{template_type}_backup_optimization"
    ...
    def save_factories(self, factories):
        ...
        for template_type, factory_info in factories.items():
            factory_file = Path('src/trading/execution') / f"{template_type}_components.py"
    ...
    def run_optimization(self):
        """运行优化"""
        print("🚀 开始Execution Engine模板文件优化...")
        print("="*60)

        try:
            # 1. 查找execution engine文件
            self.find_execution_engine_files()

            if not self.execution_files:
                print("   ⚠️  未发现需要优化的execution engine文件")
                return None

            # 2. 分析文件结构
            structure_info = self.analyze_execution_structure()

            # 3. 创建组件工厂
            factories = self.create_execution_component_factories()

            # 4. 保存组件工厂文件
            self.save_factories(factories)

            # 5. 备份并删除execution engine文件
            removed_count = self.backup_and_remove_execution_files(factories)

            print("\n" + "="*60)
            print("✅ Execution Engine模板文件优化完成！")
            print("="*60)

            print("\n📊 优化结果:")
            print(f"   删除execution文件: {removed_count}个")
...
def main():
    """主函数"""
    optimizer = ExecutionEngineTemplatesOptimizer()
    result = optimizer.run_optimization()

    if result:
        print("\n🎉 Execution Engine模板文件优化成功完成！")
