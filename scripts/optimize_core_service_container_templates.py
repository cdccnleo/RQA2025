#!/usr/bin/env python3
"""
优化Core Service Container相关模板文件

处理分布在src/core/service_container目录中的多种模板文件：
- container_*.py (容器组件)
- factory_*.py (工厂组件)
- locator_*.py (定位器组件)
- registry_*.py (注册器组件)
...
class CoreServiceContainerTemplatesOptimizer:
    """Core Service Container模板文件优化器"""

    def __init__(self):
        self.core_files = []
        self.backup_dirs = set()

    def find_core_service_container_files(self):
        """查找Core Service Container相关模板文件"""
        print("🔍 查找Core Service Container相关模板文件...")
        print("="*60)
        ...
        # 模板文件模式
        template_patterns = {
            'container': r'container_(\d+)\.py$',
            'factory': r'factory_(\d+)\.py$',
            'locator': r'locator_(\d+)\.py$',
            'registry': r'registry_(\d+)\.py$',
            'resolver': r'resolver_(\d+)\.py$'
        }

        # 查找src/core/service_container目录
        service_container_dir = Path('src/core/service_container')
        if not service_container_dir.exists():
            print("   ⚠️  src/core/service_container目录不存在")
            return []

        for file_path in service_container_dir.iterdir():
        ...
        for file_info in self.core_files:
            type_groups[file_info['template_type']].append(file_info)

        print(f"   📁 发现 {len(self.core_files)} 个Core Service Container模板文件")
        print("   📂 按类型分布:")
    ...
    def analyze_core_structure(self):
        ...
        # 读取第一个文件来了解结构
        first_file = self.core_files[0]['path']
        try:
    ...
    def create_core_component_factories(self):
        """为每个模板类型创建组件工厂"""
        print("🏭 创建Core Service Container组件工厂...")
...
'''
    ...
    def backup_and_remove_core_files(self, factories):
        """备份并删除core文件"""
        print("📦 备份和删除Core Service Container文件...")
        ...
        for template_type, factory_info in factories.items():
            ...
            # 创建备份目录
            backup_dir = Path('src/core/service_container').parent / \
                f"service_container_{template_type}_backup_optimization"
    ...
    def save_factories(self, factories):
        ...
        for template_type, factory_info in factories.items():
            factory_file = Path('src/core/service_container') / f"{template_type}_components.py"
    ...
    def run_optimization(self):
        """运行优化"""
        print("🚀 开始Core Service Container模板文件优化...")
        print("="*60)

        try:
            # 1. 查找Core Service Container文件
            self.find_core_service_container_files()

            if not self.core_files:
                print("   ⚠️  未发现需要优化的Core Service Container文件")
                return None

            # 2. 分析文件结构
            structure_info = self.analyze_core_structure()

            # 3. 创建组件工厂
            factories = self.create_core_component_factories()

            # 4. 保存组件工厂文件
            self.save_factories(factories)

            # 5. 备份并删除Core Service Container文件
            removed_count = self.backup_and_remove_core_files(factories)

            print("\n" + "="*60)
            print("✅ Core Service Container模板文件优化完成！")
            print("="*60)

            print("\n📊 优化结果:")
            print(f"   删除Core Service Container文件: {removed_count}个")
...
def main():
    """主函数"""
    optimizer = CoreServiceContainerTemplatesOptimizer()
    result = optimizer.run_optimization()

    if result:
        print("\n🎉 Core Service Container模板文件优化成功完成！")
        ...
        print("\n❌ Core Service Container模板文件优化失败！")
