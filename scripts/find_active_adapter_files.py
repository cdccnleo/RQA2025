...
def find_active_adapter_files():
    """查找实际存在的adapter_*.py文件（排除备份目录）"""
    print("🔍 查找实际存在的adapter_*.py文件...")
    print("="*60)
    ...
    print(f"   📁 发现 {len(adapter_files)} 个实际存在的adapter_*.py文件")
    ...
    if adapter_files:
        ...
        print("   ✅ 没有发现需要处理的adapter_*.py文件")

    return adapter_files


if __name__ == "__main__":
    find_active_adapter_files()
