"""
验证新特征提取任务 - 最终验证
任务ID: feature_task_single_002837_1771759162
"""
...
def verify_task(task_id: str):
    ...
    try:
        ...
        # 2. 查询特征存储
        cursor.execute("""
            SELECT feature_name, feature_type, parameters, symbol
            FROM feature_store
            WHERE task_id = %s
        ...
        for i, row in enumerate(rows, 1):
            ...
            symbol = row[3] or 'N/A'
            print(f"   {i}. {feat_name} (类型: {feat_type}, 股票: {symbol})")
...
def main():
    task_id = "feature_task_single_002837_1771759162"
    
    print("🚀 开始验证新特征提取任务 - 最终验证")
