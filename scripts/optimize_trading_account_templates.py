#!/usr/bin/env python3
"""
优化trading/account目录中的模板文件

处理分布在src/trading/account目录中的多种模板文件：
- account_*.py (账户管理)
- balance_*.py (余额管理)
- capital_*.py (资本管理)
- fund_*.py (资金管理)
...
class TradingAccountTemplatesOptimizer:
    """Trading Account模板文件优化器"""

    def __init__(self):
        self.account_files = []
        self.backup_dirs = set()

    def find_trading_account_files(self):
        """查找trading/account目录中的模板文件"""
        print("🔍 查找trading/account目录中的模板文件...")
        print("="*60)
        ...
        # 模板文件模式
        template_patterns = {
            'account': r'account_(\d+)\.py$',
            'balance': r'balance_(\d+)\.py$',
            'capital': r'capital_(\d+)\.py$',
            'fund': r'fund_(\d+)\.py$',
            'margin': r'margin_(\d+)\.py$'
        }

        # 查找src/trading/account目录
        account_dir = Path('src/trading/account')
        if not account_dir.exists():
            print("   ⚠️  src/trading/account目录不存在")
            return []
        ...
        for file_info in self.account_files:
            type_groups[file_info['template_type']].append(file_info)

        print(f"   📁 发现 {len(self.account_files)} 个trading account模板文件")
        print("   📂 按类型分布:")
        for template_type, files in type_groups.items():
            print(f"      - {template_type}_*.py: {len(files)}个文件")

        return self.account_files

    def analyze_trading_account_structure(self):
        """分析trading account文件结构"""
        print("🔍 分析trading account文件结构...")

        if not self.account_files:
            return None

        # 读取第一个文件来了解结构
        first_file = self.account_files[0]['path']
        try:
    ...
    def create_trading_account_component_factories(self):
        """为每个模板类型创建组件工厂"""
        print("🏭 创建trading account组件工厂...")
...
class {component_type}Component(I{component_type}Component):
    ...
    def get_info(self) -> Dict[str, Any]:
        ...
        return {{
            ...
            "version": "2.0.0",
            "type": "unified_trading_account_component"
        }}
...
'''
    ...
    def backup_and_remove_trading_account_files(self, factories):
        """备份并删除trading account文件"""
        print("📦 备份和删除trading account文件...")
        ...
        for template_type, factory_info in factories.items():
            ...
            # 创建备份目录
            backup_dir = Path('src/trading/account').parent / \
                f"account_{template_type}_backup_optimization"
    ...
    def save_factories(self, factories):
        ...
        for template_type, factory_info in factories.items():
            factory_file = Path('src/trading/account') / f"{template_type}_components.py"
    ...
    def run_optimization(self):
        """运行优化"""
        print("🚀 开始Trading Account模板文件优化...")
        print("="*60)

        try:
            # 1. 查找trading account文件
            self.find_trading_account_files()

            if not self.account_files:
                print("   ⚠️  未发现需要优化的trading account文件")
                return None

            # 2. 分析文件结构
            structure_info = self.analyze_trading_account_structure()

            # 3. 创建组件工厂
            factories = self.create_trading_account_component_factories()

            # 4. 保存组件工厂文件
            self.save_factories(factories)

            # 5. 备份并删除trading account文件
            removed_count = self.backup_and_remove_trading_account_files(factories)

            print("\n" + "="*60)
            print("✅ Trading Account模板文件优化完成！")
            print("="*60)

            print("\n📊 优化结果:")
            print(f"   删除trading account文件: {removed_count}个")
...
def main():
    """主函数"""
    optimizer = TradingAccountTemplatesOptimizer()
    result = optimizer.run_optimization()

    if result:
        print("\n🎉 Trading Account模板文件优化成功完成！")
