#!/usr/bin/env python3
"""
优化Backtest Evaluation相关模板文件

处理分布在src/backtest/evaluation目录中的多种模板文件：
- assessor_*.py (评估器组件)
- evaluation_*.py (评估组件)
- evaluator_*.py (评估器组件)
- judge_*.py (判断器组件)
...
class BacktestEvaluationTemplatesOptimizer:
    """Backtest Evaluation模板文件优化器"""

    def __init__(self):
        self.backtest_files = []
        self.backup_dirs = set()

    def find_backtest_evaluation_files(self):
        """查找Backtest Evaluation相关模板文件"""
        print("🔍 查找Backtest Evaluation相关模板文件...")
        print("="*60)
        ...
        # 模板文件模式
        template_patterns = {
            'assessor': r'assessor_(\d+)\.py$',
            'evaluation': r'evaluation_(\d+)\.py$',
            'evaluator': r'evaluator_(\d+)\.py$',
            'judge': r'judge_(\d+)\.py$',
            'scorer': r'scorer_(\d+)\.py$'
        }

        # 查找src/backtest/evaluation目录
        evaluation_dir = Path('src/backtest/evaluation')
        if not evaluation_dir.exists():
            print("   ⚠️  src/backtest/evaluation目录不存在")
            return []

        for file_path in evaluation_dir.iterdir():
        ...
        print(f"   📁 发现 {len(self.backtest_files)} 个Backtest Evaluation模板文件")
        print("   📂 按类型分布:")
    ...
    def create_backtest_component_factories(self):
        """为每个模板类型创建组件工厂"""
        print("🏭 创建Backtest Evaluation组件工厂...")
...
'''
    ...
    def backup_and_remove_backtest_files(self, factories):
        """备份并删除backtest文件"""
        print("📦 备份和删除Backtest Evaluation文件...")
        ...
        for template_type, factory_info in factories.items():
            ...
            # 创建备份目录
            backup_dir = Path('src/backtest/evaluation').parent / \
                f"evaluation_{template_type}_backup_optimization"
    ...
    def save_factories(self, factories):
        ...
        for template_type, factory_info in factories.items():
            factory_file = Path('src/backtest/evaluation') / f"{template_type}_components.py"
    ...
    def run_optimization(self):
        """运行优化"""
        print("🚀 开始Backtest Evaluation模板文件优化...")
        print("="*60)

        try:
            # 1. 查找Backtest Evaluation文件
            self.find_backtest_evaluation_files()

            if not self.backtest_files:
                print("   ⚠️  未发现需要优化的Backtest Evaluation文件")
                return None
            ...
            # 5. 备份并删除Backtest Evaluation文件
            removed_count = self.backup_and_remove_backtest_files(factories)

            print("\n" + "="*60)
            print("✅ Backtest Evaluation模板文件优化完成！")
            print("="*60)

            print("\n📊 优化结果:")
            print(f"   删除Backtest Evaluation文件: {removed_count}个")
...
def main():
    """主函数"""
    optimizer = BacktestEvaluationTemplatesOptimizer()
    result = optimizer.run_optimization()

    if result:
        print("\n🎉 Backtest Evaluation模板文件优化成功完成！")
