#!/usr/bin/env python3
"""
基础设施resource模块测试修复脚本

专门修复resource子模块的测试文件
"""
...
class ResourceTestFixer:
    """Resource模块测试修复器"""

    def __init__(self, resource_test_dir: str):
        self.resource_test_dir = Path(resource_test_dir)
        self.fixed_files = []
        self.errors = []

    def find_resource_test_files(self) -> list:
        """查找所有resource测试文件"""
        test_files = []
        if self.resource_test_dir.exists():
            for file_path in self.resource_test_dir.rglob("test_*.py"):
    ...
    def fix_all_resource_tests(self) -> tuple:
        """修复所有resource测试文件"""
        test_files = self.find_resource_test_files()
        logger.info(f"找到 {len(test_files)} 个resource测试文件")
...
def main():
    """主函数"""
    resource_test_dir = "tests/unit/infrastructure/resource"

    if not os.path.exists(resource_test_dir):
        logger.error(f"resource测试目录不存在: {resource_test_dir}")
        return

    fixer = ResourceTestFixer(resource_test_dir)
    fixed_count, error_count = fixer.fix_all_resource_tests()
