...
专门修复tests/unit/infrastructure/根目录下的测试文件
"""
...
class RootInfrastructureTestFixer:
    """根目录基础设施测试修复器"""

    def __init__(self, infrastructure_test_dir: str):
        self.infrastructure_test_dir = Path(infrastructure_test_dir)
        self.fixed_files = []
        self.errors = []

    def find_root_test_files(self) -> list:
        """查找根目录的测试文件（排除子目录）"""
        test_files = []
        if self.infrastructure_test_dir.exists():
            for item in self.infrastructure_test_dir.iterdir():
                if item.is_file() and item.name.endswith('.py') and item.name.startswith('test_'):
    ...
    def fix_all_root_tests(self) -> tuple:
        """修复所有根目录测试文件"""
        test_files = self.find_root_test_files()
        logger.info(f"找到 {len(test_files)} 个根目录测试文件")
...
def main():
    """主函数"""
    infrastructure_test_dir = "tests/unit/infrastructure"

    if not os.path.exists(infrastructure_test_dir):
        logger.error(f"基础设施测试目录不存在: {infrastructure_test_dir}")
        return

    fixer = RootInfrastructureTestFixer(infrastructure_test_dir)
    fixed_count, error_count = fixer.fix_all_root_tests()
