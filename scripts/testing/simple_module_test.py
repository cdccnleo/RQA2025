...
class SimpleModuleTester:
    ...
    def __init__(self, project_root: str):
        ...
        self.results: List[SimpleModuleTestResult] = []
        ...
        self.core_modules = [
            ...
            "src.infrastructure.monitoring.visual_monitor",
            "src.infrastructure.error_handler",
            "src.infrastructure.service_launcher",
            "src.infrastructure.deployment_validator",
            "src.infrastructure.data_sync",
            "src.infrastructure.event",
            "src.infrastructure.lock",
            "src.infrastructure.version",
            "src.infrastructure.disaster_recovery",
            "src.infrastructure.degradation_manager",
            "src.infrastructure.circuit_breaker",
            "src.infrastructure.auto_recovery",
            "src.infrastructure.final_deployment_check",
            "src.infrastructure.database_adapter",
            "src.infrastructure.inference_engine",
            "src.infrastructure.module_optimizer",
        ]
    ...
    def test_single_module(self, module_name: str) -> SimpleModuleTestResult:
        """测试单个模块"""
        print(f"\n🔍 测试模块: {module_name}")
    ...
    def run_all_tests(self) -> List[SimpleModuleTestResult]:
        """运行所有模块测试"""
        print("🚀 开始基础设施层模块简化测试")
        print("=" * 60)
...
def main():
    ...
    print("🔧 初始化简化模块测试器...")
    tester = SimpleModuleTester(project_root)
    ...
    # 保存报告
    report_file = Path(project_root) / "reports" / "simple_module_test_report.md"
