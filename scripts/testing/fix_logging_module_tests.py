...
class LoggingTestFixer:
    """Logging模块测试修复器"""

    def __init__(self, logging_test_dir: str):
        self.logging_test_dir = Path(logging_test_dir)
        self.fixed_files = []
        self.errors = []

    def find_logging_test_files(self) -> list:
        """查找所有logging测试文件"""
        test_files = []
        if self.logging_test_dir.exists():
            for file_path in self.logging_test_dir.rglob("test_*.py"):
    ...
    def fix_all_logging_tests(self) -> tuple:
        """修复所有logging测试文件"""
        test_files = self.find_logging_test_files()
        logger.info(f"找到 {len(test_files)} 个logging测试文件")
...
def main():
    """主函数"""
    logging_test_dir = "tests/unit/infrastructure/logging"

    if not os.path.exists(logging_test_dir):
        logger.error(f"logging测试目录不存在: {logging_test_dir}")
        return

    fixer = LoggingTestFixer(logging_test_dir)
    fixed_count, error_count = fixer.fix_all_logging_tests()
