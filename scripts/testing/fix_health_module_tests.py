...
class HealthTestFixer:
    ...
    def __init__(self, health_test_dir: str):
        self.health_test_dir = Path(health_test_dir)
        self.fixed_files = []
        self.errors = []

    def find_health_test_files(self) -> list:
        """查找所有health测试文件"""
        test_files = []
        if self.health_test_dir.exists():
            for file_path in self.health_test_dir.rglob("test_*.py"):
    ...
    def fix_all_health_tests(self) -> tuple:
        """修复所有health测试文件"""
        test_files = self.find_health_test_files()
        logger.info(f"找到 {len(test_files)} 个health测试文件")
...
def main():
    """主函数"""
    health_test_dir = "tests/unit/infrastructure/health"

    if not os.path.exists(health_test_dir):
        logger.error(f"health测试目录不存在: {health_test_dir}")
        return

    fixer = HealthTestFixer(health_test_dir)
    fixed_count, error_count = fixer.fix_all_health_tests()
