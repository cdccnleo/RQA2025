...
class ErrorTestFixer:
    ...
    def __init__(self, error_test_dir: str):
        self.error_test_dir = Path(error_test_dir)
        self.fixed_files = []
        self.errors = []

    def find_error_test_files(self) -> list:
        """查找所有error测试文件"""
        test_files = []
        if self.error_test_dir.exists():
            for file_path in self.error_test_dir.rglob("test_*.py"):
    ...
    def fix_all_error_tests(self) -> tuple:
        """修复所有error测试文件"""
        test_files = self.find_error_test_files()
        logger.info(f"找到 {len(test_files)} 个error测试文件")
...
def main():
    """主函数"""
    error_test_dir = "tests/unit/infrastructure/error"

    if not os.path.exists(error_test_dir):
        logger.error(f"error测试目录不存在: {error_test_dir}")
        return

    fixer = ErrorTestFixer(error_test_dir)
    fixed_count, error_count = fixer.fix_all_error_tests()
