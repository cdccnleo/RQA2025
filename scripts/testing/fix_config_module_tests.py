...
class ConfigTestFixer:
    """Config模块测试修复器"""

    def __init__(self, config_test_dir: str):
        self.config_test_dir = Path(config_test_dir)
        self.fixed_files = []
        self.errors = []

    def find_config_test_files(self) -> list:
        """查找所有config测试文件"""
        test_files = []
        if self.config_test_dir.exists():
            for file_path in self.config_test_dir.rglob("test_*.py"):
    ...
    def fix_all_config_tests(self) -> tuple:
        """修复所有config测试文件"""
        test_files = self.find_config_test_files()
        logger.info(f"找到 {len(test_files)} 个config测试文件")
...
def main():
    """主函数"""
    config_test_dir = "tests/unit/infrastructure/config"

    if not os.path.exists(config_test_dir):
        logger.error(f"config测试目录不存在: {config_test_dir}")
        return

    fixer = ConfigTestFixer(config_test_dir)
    fixed_count, error_count = fixer.fix_all_config_tests()
