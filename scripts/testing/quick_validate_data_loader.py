...
def quick_validate():
    """快速验证BacktestDataLoader基本功能"""
    print("🚀 开始快速验证 BacktestDataLoader")
    print("=" * 50)
...
def test_with_mock():
    ...
    try:
        ...
        # 模拟StockDataLoader
        with patch('src.data.loader.stock_loader.StockDataLoader.create_from_config') as mock_create:
            mock_loader = Mock()
            ...
            # 测试OHLCV数据加载
            result = loader.load_ohlcv("000001", "2023-01-01", "2023-01-05")
        ...
        print(f"❌ 模拟数据加载测试失败: {str(e)}")
        return False


def main():
    """主函数"""
    print("🚀 BacktestDataLoader 快速验证")
    print("=" * 60)

    # 基本功能验证
    basic_success = quick_validate()
    ...
    if basic_success and mock_success:
        print("🎉 所有验证通过！BacktestDataLoader 可以正常使用。")
        return True
    else:
