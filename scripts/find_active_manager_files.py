...
def find_active_manager_files():
    """查找实际存在的manager_*.py文件（排除备份目录）"""
    print("🔍 查找实际存在的manager_*.py文件...")
    print("="*60)
    ...
    print(f"   📁 发现 {len(manager_files)} 个实际存在的manager_*.py文件")
    ...
    if manager_files:
        ...
        print("   ✅ 没有发现需要处理的manager_*.py文件")

    return manager_files


if __name__ == "__main__":
    find_active_manager_files()
