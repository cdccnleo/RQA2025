#!/usr/bin/env python3
"""
优化Security Management相关模板文件

处理分布在src/infrastructure/security目录中的多种模板文件：
- audit_*.py (审计组件)
- auth_*.py (认证组件)
- encrypt_*.py (加密组件)
- policy_*.py (策略组件)
...
class SecurityManagementTemplatesOptimizer:
    """Security Management模板文件优化器"""

    def __init__(self):
        self.security_files = []
        self.backup_dirs = set()

    def find_security_management_files(self):
        """查找Security Management相关模板文件"""
        print("🔍 查找Security Management相关模板文件...")
        print("="*60)
        ...
        template_patterns = {
            'audit': r'audit_(\d+)\.py$',
            'auth': r'auth_(\d+)\.py$',
            'encrypt': r'encrypt_(\d+)\.py$',
            'policy': r'policy_(\d+)\.py$',
            'security': r'security_(\d+)\.py$'
        }

        # 查找src/infrastructure/security目录
        security_dir = Path('src/infrastructure/security')
        if not security_dir.exists():
            print("   ⚠️  src/infrastructure/security目录不存在")
            return []
        ...
        print(f"   📁 发现 {len(self.security_files)} 个Security Management模板文件")
        print("   📂 按类型分布:")
        for template_type, files in type_groups.items():
            print(f"      - {template_type}_*.py: {len(files)}个文件")

        return self.security_files

    def analyze_security_structure(self):
        """分析security文件结构"""
        print("🔍 分析security文件结构...")

        if not self.security_files:
            return None

        # 读取第一个文件来了解结构
        first_file = self.security_files[0]['path']
        try:
    ...
    def create_security_component_factories(self):
        """为每个模板类型创建组件工厂"""
        print("🏭 创建Security Management组件工厂...")
...
'''
    ...
    def backup_and_remove_security_files(self, factories):
        """备份并删除security文件"""
        print("📦 备份和删除Security Management文件...")
        ...
        for template_type, factory_info in factories.items():
            ...
            # 创建备份目录
            backup_dir = Path('src/infrastructure/security').parent / \
                f"security_{template_type}_backup_optimization"
    ...
    def save_factories(self, factories):
        ...
        for template_type, factory_info in factories.items():
            factory_file = Path('src/infrastructure/security') / f"{template_type}_components.py"
    ...
    def run_optimization(self):
        """运行优化"""
        print("🚀 开始Security Management模板文件优化...")
        print("="*60)

        try:
            # 1. 查找Security Management文件
            self.find_security_management_files()

            if not self.security_files:
                print("   ⚠️  未发现需要优化的Security Management文件")
                return None

            # 2. 分析文件结构
            structure_info = self.analyze_security_structure()

            # 3. 创建组件工厂
            factories = self.create_security_component_factories()

            # 4. 保存组件工厂文件
            self.save_factories(factories)

            # 5. 备份并删除Security Management文件
            removed_count = self.backup_and_remove_security_files(factories)

            print("\n" + "="*60)
            print("✅ Security Management模板文件优化完成！")
            print("="*60)

            print("\n📊 优化结果:")
            print(f"   删除Security Management文件: {removed_count}个")
...
def main():
    """主函数"""
    optimizer = SecurityManagementTemplatesOptimizer()
    result = optimizer.run_optimization()

    if result:
        print("\n🎉 Security Management模板文件优化成功完成！")
