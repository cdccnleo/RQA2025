...
处理分布在src/ml/models目录中的多种模板文件：
- architecture_*.py (架构组件)
- definition_*.py (定义组件)
- model_*.py (模型组件)
- network_*.py (网络组件)
- structure_*.py (结构组件)
...
class MLModelsTemplatesOptimizer:
    """ML Models模板文件优化器"""
    ...
    def find_ml_models_files(self):
        """查找ML Models相关模板文件"""
        print("🔍 查找ML Models相关模板文件...")
        print("="*60)
        ...
        # 模板文件模式
        template_patterns = {
            'architecture': r'architecture_(\d+)\.py$',
            'definition': r'definition_(\d+)\.py$',
            'model': r'model_(\d+)\.py$',
            'network': r'network_(\d+)\.py$',
            'structure': r'structure_(\d+)\.py$'
        }

        # 查找src/ml/models目录
        models_dir = Path('src/ml/models')
        if not models_dir.exists():
            print("   ⚠️  src/ml/models目录不存在")
            return []
        ...
        for file_info in self.models_files:
            type_groups[file_info['template_type']].append(file_info)

        print(f"   📁 发现 {len(self.models_files)} 个ML Models模板文件")
        print("   📂 按类型分布:")
    ...
    def analyze_models_structure(self):
        ...
        if not self.models_files:
            return None

        # 读取第一个文件来了解结构
        first_file = self.models_files[0]['path']
        try:
    ...
    def create_models_component_factories(self):
        """为每个模板类型创建组件工厂"""
        print("🏭 创建ML Models组件工厂...")
...
class {component_type}Component(I{component_type}Component):
    ...
    def get_info(self) -> Dict[str, Any]:
        ...
        return {{
            ...
            "version": "2.0.0",
            "type": "unified_ml_models_component"
        }}
...
'''
    ...
    def backup_and_remove_models_files(self, factories):
        """备份并删除models文件"""
        print("📦 备份和删除ML Models文件...")
        ...
        for template_type, factory_info in factories.items():
            ...
            # 创建备份目录
            backup_dir = Path('src/ml/models').parent / \
                f"models_{template_type}_backup_optimization"
    ...
    def save_factories(self, factories):
        ...
        for template_type, factory_info in factories.items():
            factory_file = Path('src/ml/models') / f"{template_type}_components.py"
    ...
    def run_optimization(self):
        """运行优化"""
        print("🚀 开始ML Models模板文件优化...")
        print("="*60)

        try:
            # 1. 查找ML Models文件
            self.find_ml_models_files()

            if not self.models_files:
                print("   ⚠️  未发现需要优化的ML Models文件")
                return None

            # 2. 分析文件结构
            structure_info = self.analyze_models_structure()

            # 3. 创建组件工厂
            factories = self.create_models_component_factories()

            # 4. 保存组件工厂文件
            self.save_factories(factories)

            # 5. 备份并删除ML Models文件
            removed_count = self.backup_and_remove_models_files(factories)

            print("\n" + "="*60)
            print("✅ ML Models模板文件优化完成！")
            print("="*60)

            print("\n📊 优化结果:")
            print(f"   删除ML Models文件: {removed_count}个")
            ...
            return {
                ...
                'models_files': self.models_files,
...
def main():
    """主函数"""
    optimizer = MLModelsTemplatesOptimizer()
