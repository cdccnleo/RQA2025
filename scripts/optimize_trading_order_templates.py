#!/usr/bin/env python3
"""
优化Trading Order相关模板文件

处理分布在src/trading/order目录中的多种模板文件：
- order_*.py (订单组件)
- management_*.py (管理组件)
"""
...
class TradingOrderTemplatesOptimizer:
    """Trading Order模板文件优化器"""

    def __init__(self):
        self.order_files = []
        self.backup_dirs = set()

    def find_trading_order_files(self):
        """查找Trading Order相关模板文件"""
        print("🔍 查找Trading Order相关模板文件...")
        print("="*60)
        ...
        template_patterns = {
            'order': r'order_(\d+)\.py$',
            'management': r'management_(\d+)\.py$'
        }

        # 查找src/trading/order目录
        order_dir = Path('src/trading/order')
        if not order_dir.exists():
            print("   ⚠️  src/trading/order目录不存在")
            return []
        ...
        print(f"   📁 发现 {len(self.order_files)} 个Trading Order模板文件")
        print("   📂 按类型分布:")
...
class {component_type}Component(I{component_type}Component):
    ...
    def get_info(self) -> Dict[str, Any]:
        ...
        return {{
            ...
            "version": "2.0.0",
            "type": "unified_trading_order_component"
        }}
...
'''
    ...
    def backup_and_remove_order_files(self, factories):
        ...
        for template_type, factory_info in factories.items():
            ...
            # 创建备份目录
            backup_dir = Path('src/trading/order').parent / \
                f"order_{template_type}_backup_optimization"
    ...
    def save_factories(self, factories):
        ...
        for template_type, factory_info in factories.items():
            factory_file = Path('src/trading/order') / f"{template_type}_components.py"
    ...
    def run_optimization(self):
        """运行优化"""
        print("🚀 开始Trading Order模板文件优化...")
        print("="*60)

        try:
            # 1. 查找Trading Order文件
            self.find_trading_order_files()

            if not self.order_files:
                print("   ⚠️  未发现需要优化的Trading Order文件")
                return None
            ...
            # 5. 备份并删除Trading Order文件
            removed_count = self.backup_and_remove_order_files(factories)

            print("\n" + "="*60)
            print("✅ Trading Order模板文件优化完成！")
            print("="*60)

            print("\n📊 优化结果:")
            print(f"   删除Trading Order文件: {removed_count}个")
...
def main():
    """主函数"""
    optimizer = TradingOrderTemplatesOptimizer()
    result = optimizer.run_optimization()

    if result:
        print("\n🎉 Trading Order模板文件优化成功完成！")
