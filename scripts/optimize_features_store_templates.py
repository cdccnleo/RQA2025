#!/usr/bin/env python3
"""
优化Features Store相关模板文件

处理分布在src/features/store目录中的多种模板文件：
- database_*.py (数据库组件)
- persistence_*.py (持久化组件)
- repository_*.py (仓库组件)
- store_*.py (存储组件)
"""
...
class FeaturesStoreTemplatesOptimizer:
    """Features Store模板文件优化器"""

    def __init__(self):
        self.features_files = []
        self.backup_dirs = set()

    def find_features_store_files(self):
        """查找Features Store相关模板文件"""
        print("🔍 查找Features Store相关模板文件...")
        print("="*60)
        ...
        # 模板文件模式
        template_patterns = {
            'database': r'database_(\d+)\.py$',
            'persistence': r'persistence_(\d+)\.py$',
            'repository': r'repository_(\d+)\.py$',
            'store': r'store_(\d+)\.py$'
        }

        # 查找src/features/store目录
        store_dir = Path('src/features/store')
        if not store_dir.exists():
            print("   ⚠️  src/features/store目录不存在")
            return []
        ...
        print(f"   📁 发现 {len(self.features_files)} 个Features Store模板文件")
        print("   📂 按类型分布:")
...
'''
    ...
    def backup_and_remove_features_files(self, factories):
        ...
        for template_type, factory_info in factories.items():
            ...
            # 创建备份目录
            backup_dir = Path('src/features/store').parent / \
                f"store_{template_type}_backup_optimization"
    ...
    def save_factories(self, factories):
        ...
        for template_type, factory_info in factories.items():
            factory_file = Path('src/features/store') / f"{template_type}_components.py"
    ...
    def run_optimization(self):
        """运行优化"""
        print("🚀 开始Features Store模板文件优化...")
        print("="*60)

        try:
            # 1. 查找Features Store文件
            self.find_features_store_files()

            if not self.features_files:
                print("   ⚠️  未发现需要优化的Features Store文件")
                return None
            ...
            # 5. 备份并删除Features Store文件
            removed_count = self.backup_and_remove_features_files(factories)

            print("\n" + "="*60)
            print("✅ Features Store模板文件优化完成！")
            print("="*60)

            print("\n📊 优化结果:")
            print(f"   删除Features Store文件: {removed_count}个")
...
def main():
    """主函数"""
    optimizer = FeaturesStoreTemplatesOptimizer()
    result = optimizer.run_optimization()

    if result:
        print("\n🎉 Features Store模板文件优化成功完成！")
