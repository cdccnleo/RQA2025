#!/usr/bin/env python3
"""
优化Features Processors相关模板文件

处理分布在src/features/processors目录中的多种模板文件：
- encoder_*.py (编码器组件)
- normalizer_*.py (标准化组件)
- scaler_*.py (缩放器组件)
- transformer_*.py (变换器组件)
"""
...
class FeaturesProcessorsTemplatesOptimizer:
    """Features Processors模板文件优化器"""

    def __init__(self):
        self.processors_files = []
        self.backup_dirs = set()

    def find_features_processors_files(self):
        """查找Features Processors相关模板文件"""
        print("🔍 查找Features Processors相关模板文件...")
        print("="*60)
        ...
        # 模板文件模式
        template_patterns = {
            'encoder': r'encoder_(\d+)\.py$',
            'normalizer': r'normalizer_(\d+)\.py$',
            'scaler': r'scaler_(\d+)\.py$',
            'transformer': r'transformer_(\d+)\.py$'
        }

        # 查找src/features/processors目录
        processors_dir = Path('src/features/processors')
        if not processors_dir.exists():
            print("   ⚠️  src/features/processors目录不存在")
            return []

        for file_path in processors_dir.iterdir():
        ...
        for file_info in self.processors_files:
            type_groups[file_info['template_type']].append(file_info)

        print(f"   📁 发现 {len(self.processors_files)} 个Features Processors模板文件")
        print("   📂 按类型分布:")
        for template_type, files in type_groups.items():
            print(f"      - {template_type}_*.py: {len(files)}个文件")

        return self.processors_files

    def analyze_processors_structure(self):
        """分析processors文件结构"""
        print("🔍 分析processors文件结构...")

        if not self.processors_files:
            return None

        # 读取第一个文件来了解结构
        first_file = self.processors_files[0]['path']
        try:
    ...
    def create_processors_component_factories(self):
        """为每个模板类型创建组件工厂"""
        print("🏭 创建Features Processors组件工厂...")
...
'''
    ...
    def backup_and_remove_processors_files(self, factories):
        """备份并删除processors文件"""
        print("📦 备份和删除Features Processors文件...")
        ...
        for template_type, factory_info in factories.items():
            ...
            # 创建备份目录
            backup_dir = Path('src/features/processors').parent / \
                f"processors_{template_type}_backup_optimization"
    ...
    def save_factories(self, factories):
        ...
        for template_type, factory_info in factories.items():
            factory_file = Path('src/features/processors') / f"{template_type}_components.py"
    ...
    def run_optimization(self):
        """运行优化"""
        print("🚀 开始Features Processors模板文件优化...")
        print("="*60)

        try:
            # 1. 查找Features Processors文件
            self.find_features_processors_files()

            if not self.processors_files:
                print("   ⚠️  未发现需要优化的Features Processors文件")
                return None

            # 2. 分析文件结构
            structure_info = self.analyze_processors_structure()

            # 3. 创建组件工厂
            factories = self.create_processors_component_factories()

            # 4. 保存组件工厂文件
            self.save_factories(factories)

            # 5. 备份并删除Features Processors文件
            removed_count = self.backup_and_remove_processors_files(factories)

            print("\n" + "="*60)
            print("✅ Features Processors模板文件优化完成！")
            print("="*60)

            print("\n📊 优化结果:")
            print(f"   删除Features Processors文件: {removed_count}个")
            ...
            return {
                ...
                'processors_files': self.processors_files,
...
def main():
    """主函数"""
    optimizer = FeaturesProcessorsTemplatesOptimizer()
    result = optimizer.run_optimization()

    if result:
        print("\n🎉 Features Processors模板文件优化成功完成！")
