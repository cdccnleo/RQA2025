#!/usr/bin/env python3
"""
优化Data Processing相关模板文件

处理分布在src/data/processing目录中的多种模板文件：
- cleaner_*.py (数据清理组件)
- filter_*.py (数据过滤组件)
- transformer_*.py (数据转换组件)
- validator_*.py (数据验证组件)
"""
...
class DataProcessingTemplatesOptimizer:
    """Data Processing模板文件优化器"""

    def __init__(self):
        self.data_files = []
        self.backup_dirs = set()

    def find_data_processing_files(self):
        """查找Data Processing相关模板文件"""
        print("🔍 查找Data Processing相关模板文件...")
        print("="*60)
        ...
        # 模板文件模式
        template_patterns = {
            'cleaner': r'cleaner_(\d+)\.py$',
            'filter': r'filter_(\d+)\.py$',
            'transformer': r'transformer_(\d+)\.py$',
            'validator': r'validator_(\d+)\.py$'
        }

        # 查找src/data/processing目录
        processing_dir = Path('src/data/processing')
        if not processing_dir.exists():
            print("   ⚠️  src/data/processing目录不存在")
            return []
        ...
        print(f"   📁 发现 {len(self.data_files)} 个Data Processing模板文件")
        print("   📂 按类型分布:")
    ...
    def analyze_data_structure(self):
        ...
        # 读取第一个文件来了解结构
        first_file = self.data_files[0]['path']
        try:
    ...
    def create_data_component_factories(self):
        """为每个模板类型创建组件工厂"""
        print("🏭 创建Data Processing组件工厂...")
...
'''
    ...
    def backup_and_remove_data_files(self, factories):
        """备份并删除data文件"""
        print("📦 备份和删除Data Processing文件...")
        ...
        for template_type, factory_info in factories.items():
            ...
            # 创建备份目录
            backup_dir = Path('src/data/processing').parent / \
                f"processing_{template_type}_backup_optimization"
    ...
    def save_factories(self, factories):
        ...
        for template_type, factory_info in factories.items():
            factory_file = Path('src/data/processing') / f"{template_type}_components.py"
    ...
    def run_optimization(self):
        """运行优化"""
        print("🚀 开始Data Processing模板文件优化...")
        print("="*60)

        try:
            # 1. 查找Data Processing文件
            self.find_data_processing_files()

            if not self.data_files:
                print("   ⚠️  未发现需要优化的Data Processing文件")
                return None
            ...
            # 3. 创建组件工厂
            factories = self.create_data_component_factories()

            # 4. 保存组件工厂文件
            self.save_factories(factories)

            # 5. 备份并删除Data Processing文件
            removed_count = self.backup_and_remove_data_files(factories)

            print("\n" + "="*60)
            print("✅ Data Processing模板文件优化完成！")
            print("="*60)

            print("\n📊 优化结果:")
            print(f"   删除Data Processing文件: {removed_count}个")
...
def main():
    """主函数"""
    optimizer = DataProcessingTemplatesOptimizer()
    result = optimizer.run_optimization()

    if result:
        print("\n🎉 Data Processing模板文件优化成功完成！")
