#!/usr/bin/env python3
"""
优化order processing相关模板文件

处理分布在src/trading/execution目录中的order和trade模板文件：
- order_*.py (订单处理组件)
- trade_*.py (交易处理组件)
"""
...
class OrderProcessingTemplatesOptimizer:
    """Order Processing模板文件优化器"""

    def __init__(self):
        self.order_files = []
        self.backup_dirs = set()

    def find_order_processing_files(self):
        """查找order processing相关模板文件"""
        print("🔍 查找order processing相关模板文件...")
        print("="*60)
    ...
    def analyze_order_structure(self):
        ...
        # 读取第一个文件来了解结构
        first_file = self.order_files[0]['path']
        try:
    ...
    def create_order_component_factories(self):
        """为每个模板类型创建组件工厂"""
        print("🏭 创建order processing组件工厂...")
...
class {component_type}Component(I{component_type}Component):
    ...
    def get_info(self) -> Dict[str, Any]:
        ...
        return {{
            ...
            "version": "2.0.0",
            "type": "unified_order_processing_component"
        }}
...
'''
    ...
    def backup_and_remove_order_files(self, factories):
        """备份并删除order文件"""
        print("📦 备份和删除order processing文件...")
    ...
    def run_optimization(self):
        """运行优化"""
        print("🚀 开始Order Processing模板文件优化...")
        print("="*60)

        try:
            # 1. 查找order processing文件
            self.find_order_processing_files()

            if not self.order_files:
                print("   ⚠️  未发现需要优化的order processing文件")
                return None

            # 2. 分析文件结构
            structure_info = self.analyze_order_structure()

            # 3. 创建组件工厂
            factories = self.create_order_component_factories()

            # 4. 保存组件工厂文件
            self.save_factories(factories)

            # 5. 备份并删除order processing文件
            removed_count = self.backup_and_remove_order_files(factories)

            print("\n" + "="*60)
            print("✅ Order Processing模板文件优化完成！")
            print("="*60)

            print("\n📊 优化结果:")
            print(f"   删除order processing文件: {removed_count}个")
...
def main():
    """主函数"""
    optimizer = OrderProcessingTemplatesOptimizer()
    result = optimizer.run_optimization()

    if result:
        print("\n🎉 Order Processing模板文件优化成功完成！")
