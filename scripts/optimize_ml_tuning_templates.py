...
处理分布在src/ml/tuning目录中的多种模板文件：
- grid_*.py (网格搜索组件)
- hyperparameter_*.py (超参数组件)
- search_*.py (搜索组件)
- tuner_*.py (调优器组件)
"""
...
class MLTuningTemplatesOptimizer:
    """ML Tuning模板文件优化器"""
    ...
    def find_ml_tuning_files(self):
        """查找ML Tuning相关模板文件"""
        print("🔍 查找ML Tuning相关模板文件...")
        print("="*60)
        ...
        template_patterns = {
            'grid': r'grid_(\d+)\.py$',
            'hyperparameter': r'hyperparameter_(\d+)\.py$',
            'search': r'search_(\d+)\.py$',
            'tuner': r'tuner_(\d+)\.py$'
        }

        # 查找src/ml/tuning目录
        tuning_dir = Path('src/ml/tuning')
        if not tuning_dir.exists():
            print("   ⚠️  src/ml/tuning目录不存在")
            return []

        for file_path in tuning_dir.iterdir():
        ...
        for file_info in self.tuning_files:
            type_groups[file_info['template_type']].append(file_info)

        print(f"   📁 发现 {len(self.tuning_files)} 个ML Tuning模板文件")
        print("   📂 按类型分布:")
        for template_type, files in type_groups.items():
            print(f"      - {template_type}_*.py: {len(files)}个文件")

        return self.tuning_files

    def analyze_tuning_structure(self):
        """分析tuning文件结构"""
        print("🔍 分析tuning文件结构...")

        if not self.tuning_files:
            return None

        # 读取第一个文件来了解结构
        first_file = self.tuning_files[0]['path']
        try:
    ...
    def create_tuning_component_factories(self):
        """为每个模板类型创建组件工厂"""
        print("🏭 创建ML Tuning组件工厂...")
...
'''
    ...
    def backup_and_remove_tuning_files(self, factories):
        """备份并删除tuning文件"""
        print("📦 备份和删除ML Tuning文件...")
        ...
        for template_type, factory_info in factories.items():
            ...
            # 创建备份目录
            backup_dir = Path('src/ml/tuning').parent / \
                f"tuning_{template_type}_backup_optimization"
    ...
    def save_factories(self, factories):
        ...
        for template_type, factory_info in factories.items():
            factory_file = Path('src/ml/tuning') / f"{template_type}_components.py"
    ...
    def run_optimization(self):
        """运行优化"""
        print("🚀 开始ML Tuning模板文件优化...")
        print("="*60)

        try:
            # 1. 查找ML Tuning文件
            self.find_ml_tuning_files()

            if not self.tuning_files:
                print("   ⚠️  未发现需要优化的ML Tuning文件")
                return None

            # 2. 分析文件结构
            structure_info = self.analyze_tuning_structure()

            # 3. 创建组件工厂
            factories = self.create_tuning_component_factories()

            # 4. 保存组件工厂文件
            self.save_factories(factories)

            # 5. 备份并删除ML Tuning文件
            removed_count = self.backup_and_remove_tuning_files(factories)

            print("\n" + "="*60)
            print("✅ ML Tuning模板文件优化完成！")
            print("="*60)

            print("\n📊 优化结果:")
            print(f"   删除ML Tuning文件: {removed_count}个")
            ...
            return {
                ...
                'tuning_files': self.tuning_files,
...
def main():
    """主函数"""
    optimizer = MLTuningTemplatesOptimizer()
