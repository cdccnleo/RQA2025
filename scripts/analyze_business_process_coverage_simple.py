...
def analyze_single_process(process_name: str, process) -> Dict[str, Any]:
    ...
    # 输出分析结果
    print(
        f"步骤覆盖: {process_coverage['covered_steps']}/{process_coverage['total_steps']} ({process_coverage['step_coverage_rate']:.1f}%)")
    print(
        f"规则覆盖: {process_coverage['covered_rules']}/{process_coverage['total_rules']} ({process_coverage['rule_coverage_rate']:.1f}%)")
    print(
        f"场景覆盖: {process_coverage['covered_scenarios']}/{process_coverage['total_scenarios']} ({process_coverage['scenario_coverage_rate']:.1f}%)")
...
def calculate_overall_coverage(coverage_analysis: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if coverage_analysis["process_coverage"]:
        scenario_avg = sum(p.get("scenario_coverage_rate", 0)
                           for p in coverage_analysis["process_coverage"]) / len(coverage_analysis["process_coverage"])
    else:
        scenario_avg = 0
...
def generate_improvement_suggestions(coverage_analysis: Dict[str, Any]):
    ...
    if missing_tests:
        suggestions.append(f"高优先级: 创建 {len(missing_tests)} 个缺失的测试文件")

    # 检查低覆盖率的流程
    low_coverage = [p for p in coverage_analysis["process_coverage"]
                    if p["has_test_file"] and p.get("step_coverage_rate", 0) < 80]
    if low_coverage:
        suggestions.append(f"中优先级: 提升 {len(low_coverage)} 个流程的步骤覆盖率")
    ...
    if low_rule_coverage:
        suggestions.append(f"中优先级: 完善 {len(low_rule_coverage)} 个流程的业务规则测试")
    ...
    if low_scenario_coverage:
        suggestions.append(f"低优先级: 增加 {len(low_scenario_coverage)} 个流程的测试场景覆盖")
