#!/usr/bin/env python3
"""
优化Backtest Analysis相关模板文件

处理分布在src/backtest/analysis目录中的多种模板文件：
- analysis_*.py (分析组件)
- analyzer_*.py (分析器组件)
- metrics_*.py (指标组件)
- report_*.py (报告组件)
...
class BacktestAnalysisTemplatesOptimizer:
    """Backtest Analysis模板文件优化器"""

    def __init__(self):
        self.backtest_files = []
        self.backup_dirs = set()

    def find_backtest_analysis_files(self):
        """查找Backtest Analysis相关模板文件"""
        print("🔍 查找Backtest Analysis相关模板文件...")
        print("="*60)
        ...
        # 模板文件模式
        template_patterns = {
            'analysis': r'analysis_(\d+)\.py$',
            'analyzer': r'analyzer_(\d+)\.py$',
            'metrics': r'metrics_(\d+)\.py$',
            'report': r'report_(\d+)\.py$',
            'statistics': r'statistics_(\d+)\.py$'
        }

        # 查找src/backtest/analysis目录
        analysis_dir = Path('src/backtest/analysis')
        if not analysis_dir.exists():
            print("   ⚠️  src/backtest/analysis目录不存在")
            return []
        ...
        print(f"   📁 发现 {len(self.backtest_files)} 个Backtest Analysis模板文件")
        print("   📂 按类型分布:")
...
'''
    ...
    def backup_and_remove_backtest_files(self, factories):
        """备份并删除backtest文件"""
        print("📦 备份和删除Backtest Analysis文件...")
        ...
        for template_type, factory_info in factories.items():
            ...
            # 创建备份目录
            backup_dir = Path('src/backtest/analysis').parent / \
                f"analysis_{template_type}_backup_optimization"
    ...
    def save_factories(self, factories):
        ...
        for template_type, factory_info in factories.items():
            factory_file = Path('src/backtest/analysis') / f"{template_type}_components.py"
    ...
    def run_optimization(self):
        """运行优化"""
        print("🚀 开始Backtest Analysis模板文件优化...")
        print("="*60)

        try:
            # 1. 查找Backtest Analysis文件
            self.find_backtest_analysis_files()

            if not self.backtest_files:
                print("   ⚠️  未发现需要优化的Backtest Analysis文件")
                return None
            ...
            # 5. 备份并删除Backtest Analysis文件
            removed_count = self.backup_and_remove_backtest_files(factories)

            print("\n" + "="*60)
            print("✅ Backtest Analysis模板文件优化完成！")
            print("="*60)

            print("\n📊 优化结果:")
            print(f"   删除Backtest Analysis文件: {removed_count}个")
...
def main():
    """主函数"""
    optimizer = BacktestAnalysisTemplatesOptimizer()
    result = optimizer.run_optimization()

    if result:
        print("\n🎉 Backtest Analysis模板文件优化成功完成！")
