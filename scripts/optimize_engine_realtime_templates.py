#!/usr/bin/env python3
"""
优化Engine Realtime相关模板文件

处理分布在src/engine/realtime目录中的多种模板文件：
- engine_*.py (引擎组件)
- live_*.py (实时组件)
- realtime_*.py (实时组件)
- real_*.py (实时组件)
...
class EngineRealtimeTemplatesOptimizer:
    """Engine Realtime模板文件优化器"""

    def __init__(self):
        self.engine_files = []
        self.backup_dirs = set()

    def find_engine_realtime_files(self):
        """查找Engine Realtime相关模板文件"""
        print("🔍 查找Engine Realtime相关模板文件...")
        print("="*60)
        ...
        template_patterns = {
            ...
            'live': r'live_(\d+)\.py$',
            'realtime': r'realtime_(\d+)\.py$',
            'real': r'real_(\d+)\.py$',
            'stream': r'stream_(\d+)\.py$'
        }

        # 查找src/engine/realtime目录
        realtime_dir = Path('src/engine/realtime')
        if not realtime_dir.exists():
            print("   ⚠️  src/engine/realtime目录不存在")
            return []

        for file_path in realtime_dir.iterdir():
        ...
        print(f"   📁 发现 {len(self.engine_files)} 个Engine Realtime模板文件")
        print("   📂 按类型分布:")
...
'''
    ...
    def backup_and_remove_engine_files(self, factories):
        ...
        for template_type, factory_info in factories.items():
            ...
            # 创建备份目录
            backup_dir = Path('src/engine/realtime').parent / \
                f"realtime_{template_type}_backup_optimization"
    ...
    def save_factories(self, factories):
        ...
        for template_type, factory_info in factories.items():
            factory_file = Path('src/engine/realtime') / f"{template_type}_components.py"
    ...
    def run_optimization(self):
        """运行优化"""
        print("🚀 开始Engine Realtime模板文件优化...")
        print("="*60)

        try:
            # 1. 查找Engine Realtime文件
            self.find_engine_realtime_files()

            if not self.engine_files:
                print("   ⚠️  未发现需要优化的Engine Realtime文件")
                return None
            ...
            # 5. 备份并删除Engine Realtime文件
            removed_count = self.backup_and_remove_engine_files(factories)

            print("\n" + "="*60)
            print("✅ Engine Realtime模板文件优化完成！")
            print("="*60)

            print("\n📊 优化结果:")
            print(f"   删除Engine Realtime文件: {removed_count}个")
...
def main():
    """主函数"""
    optimizer = EngineRealtimeTemplatesOptimizer()
    result = optimizer.run_optimization()

    if result:
        print("\n🎉 Engine Realtime模板文件优化成功完成！")
