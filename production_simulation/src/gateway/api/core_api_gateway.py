...
重构说明：
- 类型定义已提取到 gateway_types.py
- 熔断器已提取到 resilience/circuit_breaker.py
- 负载均衡器已提取到 balancing/load_balancer.py
- 限流器已提取到 security/rate_limiter.py
- 认证管理器已提取到 security/auth_manager.py

注意：本文件保留原有类定义以确保向后兼容，
      新代码应使用上述模块化组件。
"""
...
try:
    from .gateway_types import (
        ...
        ServiceEndpoint, RateLimitRule, RouteRule,
        ApiRequest, ApiResponse
    )
    from .resilience import CircuitBreaker as CircuitBreakerV2
    from .balancing import LoadBalancer as LoadBalancerV2
    from .security import RateLimiter as RateLimiterV2, AuthenticationManager as AuthManagerV2
    
    MODULAR_COMPONENTS_AVAILABLE = True
    logger.info("模块化组件加载成功")
except ImportError as e:
    MODULAR_COMPONENTS_AVAILABLE = False
    logger.warning(f"模块化组件不可用，使用内置实现: {e}")
