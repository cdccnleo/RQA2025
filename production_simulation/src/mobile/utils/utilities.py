...
class MobileTradingService:
    ...
    def place_order(self, user_id: str, order_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if quantity < self.min_order_size or quantity > self.max_order_size:
                return {
                    "success": False,
                    "message": f"订单数量必须在 {self.min_order_size} 到 {self.max_order_size} 之间",
                }
            ...
            if (
                side == OrderSide.BUY
                and order_value + estimated_fees > user.buying_power
            ):
    ...
    def manage_watchlist(
        self, user_id: str, action: str, symbol: str
    ) -> Dict[str, Any]:
        """管理自选股"""
        if user_id not in self.users:
            return {"success": False, "message": "用户不存在"}
    ...
    def get_portfolio_summary(self, user_id: str) -> Dict[str, Any]:
        ...
        for position in positions:
            current_price = (
                self._get_market_price(position.symbol) or position.current_price
            )
            position.current_price = current_price
            position.market_value = position.quantity * current_price
            position.unrealized_pnl = (
        ...
        portfolio_return = (
            total_pnl / (user.balance + total_cost)
        ...
        for position in positions:
            allocation = (
                (position.market_value / total_value) * 100 if total_value > 0 else 0
            )
    ...
    def _update_market_data(self):
        ...
        for symbol in symbols:
            ...
            self.market_data[symbol] = {
                ...
                "market_cap": (
                    base_price * 1000000000 if symbol not in ["BTC", "ETH"] else None
                ),
                "sector": (
                    "Technology" if symbol not in ["BTC", "ETH"] else "Cryptocurrency"
                ),
                "updated_at": datetime.now(),
            }
