...
class ServiceDiscoveryServer:
    ...
    def _setup_routes(self, app):
        """设置所有路由"""
        self._setup_application_routes(app)
        self._setup_service_routes(app)
        self._setup_instance_routes(app)

    def _setup_application_routes(self, app):
        """设置应用级别路由"""
        @app.route('/eureka / apps', methods=['GET'])
        def get_applications():
            return self._handle_get_applications()

    def _setup_service_routes(self, app):
        """设置服务级别路由"""
        @app.route('/eureka / apps/<service_name>', methods=['GET', 'POST'])
        def handle_service(service_name):
            return self._handle_service_request(service_name)

    def _setup_instance_routes(self, app):
        """设置实例级别路由"""
        @app.route('/eureka / apps/<service_name>/<instance_id>', methods=['PUT', 'DELETE'])
        def handle_instance(service_name, instance_id):
            return self._handle_instance_request(service_name, instance_id)
    ...
    def _handle_get_applications(self):
        """处理获取所有应用的请求"""
    ...
    def _handle_service_request(self, service_name):
        """处理服务级别的请求"""
        from flask import request, jsonify
        if request.method == 'GET':
            return self._handle_get_service_instances(service_name)
        else:
            return self._handle_register_service_instance(service_name)

    def _handle_get_service_instances(self, service_name):
        """处理获取服务实例的请求"""
        from flask import jsonify
        with self.lock:
    ...
    def _handle_register_service_instance(self, service_name):
        """处理注册服务实例的请求"""
        from flask import request, jsonify
    ...
    def _handle_instance_request(self, service_name, instance_id):
        """处理实例级别的请求"""
        from flask import request
        if request.method == 'PUT':
            return self._handle_instance_heartbeat(service_name, instance_id)
        else:
            return self._handle_instance_deregistration(service_name, instance_id)

    def _handle_instance_heartbeat(self, service_name, instance_id):
        """处理实例心跳请求"""
        from flask import jsonify
        with self.lock:
    ...
    def _handle_instance_deregistration(self, service_name, instance_id):
        """处理实例注销请求"""
        from flask import jsonify
        with self.lock:
            if service_name in self.services and instance_id in self.services[service_name]:
