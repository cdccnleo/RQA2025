...
from src.core.constants import (
    DEFAULT_BATCH_SIZE, MAX_QUEUE_SIZE, SECONDS_PER_MINUTE
)
...
from .persistence.event_persistence import EventPersistence, PersistenceMode, EventStatus
...
class EventStatisticsManager:
    ...
    def get_recent_events(self, minutes: int = 5) -> Dict[str, int]:
        """获取最近事件"""
        cutoff_time = time.time() - (minutes * SECONDS_PER_MINUTE)
...
class EventBus(BaseComponent):
    ...
    def __init__(self, max_workers: int = DEFAULT_BATCH_SIZE, enable_async: bool = True,
                 enable_persistence: bool = True, enable_retry: bool = True,
                 enable_monitoring: bool = True, batch_size: int = DEFAULT_BATCH_SIZE,
                 max_queue_size: int = MAX_QUEUE_SIZE, config: Optional[EventBusConfig] = None):
        """初始化事件总线 - 重构版：参数封装兼容
        ...
        if hasattr(self, '_monitor'):
            ...
            try:
                ...
                stats.update({
                    ...
                    "active_handlers": sum(len(handlers) for handlers in self._handlers.values()) + sum(len(handlers) for handlers in self._async_handlers.values()),
    ...
    def publish_event(self, event: Event) -> str:
        ...
        if hasattr(self, '_publisher'):
            ...
            try:
                ...
                self.statistics_manager.update_statistics(str(transformed_event.event_type), handler_count=0, error_count=0)
    ...
    def _handle_event(self, event: Event) -> EventProcessingResult:
        ...
        if hasattr(self, '_processor'):
            ...
            # 向后兼容的原始实现
            from .components.event_processor import EventProcessingResult
