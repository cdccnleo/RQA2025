...
class StockDataLoader(BaseDataLoader):
    ...
    def _fetch_raw_data(self, symbol: str, start_date: str, end_date: str, adjust: str) -> pd.DataFrame:
        ...
        fetch_daily = getattr(ak, "stock_zh_a_daily", None)
        if callable(fetch_daily):
            try:
                ...
                message = str(exc)
                if any(keyword in message for keyword in ("Timeout", "timeout", "Network", "network")):
                    raise
                logger.debug("stock_zh_a_daily 返回数据不可用，尝试备用接口")
            except Exception as exc:
                logger.debug("调用 stock_zh_a_daily 失败: %s", exc)
    ...
    def _retry_api_call(self, func: callable, *args, **kwargs) -> Any:
        """带重试机制的API调用"""
        attempt_limit = self.max_retries if self.max_retries > 1 else 2
        for attempt in range(attempt_limit):
            try:
                result = func(*args, **kwargs)
                if isinstance(result, pd.DataFrame):
                    if result is None or result.empty:
                        logger.warning(f"API返回空数据，第{attempt + 1}次重试...")
