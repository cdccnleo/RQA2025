...
class TradingExecutor:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.supported_exchanges = self.config.get("supported_exchanges", ["SH", "SZ"])
    ...
    def cancel_order(self, order_id: str) -> Dict[str, Any]:
        ...
        Returns:
            取消结果字典，包含order_id和status
        """
        if order_id in self.active_orders:
            order = self.active_orders[order_id]
            order["status"] = OrderStatus.CANCELLED.value if hasattr(OrderStatus.CANCELLED, 'value') else "cancelled"
    ...
    def get_order_status(self, order_id: str) -> Dict[str, Any]:
        ...
        if order_id in self.active_orders:
            ...
            return {
                "order_id": order_id,
                "status": status if isinstance(status, str) else "filled"
            }
        else:
            # 即使订单不存在，也返回默认状态
            return {
    ...
    def get_execution_statistics(self) -> Dict[str, Any]:
        ...
        stats.update({
            ...
            "available_strategies": ["immediate_execution", "price_priority", "stop_loss_protection", "default_strategy"]
        })
        return stats
    ...
    def _validate_order(self, order: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 检查订单类型（可选，如果没有则使用默认值）
        if "type" not in order:
            order["type"] = "market"  # 默认市价单
        ...
        # 检查交易所支持（symbol可能包含交易所信息，这里简化处理）
    ...
    def _select_execution_strategy(self, order: Dict[str, Any]) -> str:
        ...
        Returns:
            执行策略名称
        """
        return self.select_execution_strategy(order)
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_orders": len(self.active_orders),
            "pending_orders": self.pending_orders.qsize(),
            "order_history_count": len(getattr(self, '_order_history', [])),
            "success_rate": self.execution_stats["successful_orders"] / max(1, self.execution_stats["total_orders"]),
            "avg_execution_time": self.execution_stats["avg_execution_time"],
            "available_strategies": list(getattr(self, '_executors', {}).keys())
        }
    ...
    def _calculate_orders_per_second(self) -> float:
        ...
        Returns:
            每秒订单数
        """
        return float(self.max_orders_per_second)
