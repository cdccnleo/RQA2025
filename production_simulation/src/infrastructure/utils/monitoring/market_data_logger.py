"""
market_data_logger 模块

提供 market_data_logger 相关功能和接口。
"""
...
class MarketDataDeduplicator:
    ...
    def _generate_hash(self, tick_data: Dict) -> str:
        ...
        try:
            ...
            # 如果缺少必要字段，返回默认哈希
            logging.warning(f"行情数据缺少必要字段: {e}，使用默认哈希")
            return hashlib.sha256(str(tick_data).encode()).hexdigest()
        except Exception as e:
            logging.error(f"生成行情数据哈希失败: {e}")
    ...
    def is_duplicate(self, tick_data: Dict) -> bool:
        ...
        try:
            ...
            logging.warning(f"行情数据缺少symbol字段: {e}")
            return False
        except Exception as e:
            logging.error(f"检查行情数据重复性失败: {e}")
            return False
