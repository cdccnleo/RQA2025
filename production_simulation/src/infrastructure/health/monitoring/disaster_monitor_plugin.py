"""
disaster_monitor_plugin 模块

提供 disaster_monitor_plugin 相关功能和接口。
"""
...
class DisasterMonitorPlugin:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.alert_history = []
        self.health_status = {}  # 初始化health_status属性
    ...
    def _is_node_healthy(self, node_status: NodeStatus) -> bool:
        ...
        # 检查资源使用率
        if (node_status.cpu_usage > self.alert_rules.get("cpu_threshold", 90)
            or node_status.memory_usage > self.alert_rules.get("memory_threshold", 90)
                or node_status.disk_usage > self.alert_rules.get("disk_threshold", 90)):
            return False
...
def check_health() -> Dict[str, Any]:
    ...
    try:
        ...
        result = {
            ...
            "service": "disaster_monitor_plugin",
            "checks": health_checks
        }

        if not overall_healthy:
            logger.warning("灾难监控插件模块健康检查发现问题")
            result["issues"] = [name for name, check in health_checks.items()
                                if not check.get("healthy", False)]

        logger.info(f"灾难监控插件模块健康检查完成，状态: {'健康' if overall_healthy else '异常'}")
        return result
    except Exception as e:
        logger = logging.getLogger(__name__)
        logger.error(f"灾难监控插件模块健康检查失败: {str(e)}", exc_info=True)
        return {"healthy": False, "timestamp": time.strftime("%Y-%m-%dT%H:%M:%S"), "service": "disaster_monitor_plugin", "error": str(e)}


def check_plugin_class() -> Dict[str, Any]:
    """检查插件类"""
    try:
        plugin_exists = 'DisasterMonitorPlugin' in globals()
        if not plugin_exists:
            return {"healthy": False, "error": "DisasterMonitorPlugin class not found"}

        required_methods = ['start_monitoring', 'stop_monitoring', 'get_node_status']
        methods_exist = all(hasattr(DisasterMonitorPlugin, method) for method in required_methods)
...
def check_node_status() -> Dict[str, Any]:
    """检查节点状态"""
    try:
        status_exists = 'NodeStatus' in globals()
        if not status_exists:
            return {"healthy": False, "error": "NodeStatus class not found"}

        return {"healthy": status_exists, "status_exists": status_exists}
    except Exception as e:
        return {"healthy": False, "error": str(e)}
...
def health_summary() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "disaster_monitor_plugin_module_info": {
                "service_name": "disaster_monitor_plugin",
                "purpose": "灾难监控插件",
...
def monitor_disaster_monitor_plugin() -> Dict[str, Any]:
    """监控灾难监控插件状态"""
    try:
        health_check = check_health()
...
def validate_disaster_monitor_plugin() -> Dict[str, Any]:
    """验证灾难监控插件"""
    try:
        validation_results = {
            "plugin_validation": check_plugin_class(),
            "status_validation": check_node_status()
        }
