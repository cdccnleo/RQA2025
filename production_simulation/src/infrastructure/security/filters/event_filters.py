...
class SensitiveDataFilter(IEventFilter):
    ...
    def should_process(self, event: Dict[str, Any]) -> bool:
        ...
        # 默认情况下（无指定模式）始终允许事件通过，由 sanitize_data 负责清理
        return True
    ...
    def get_filter_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "filter_type": self.filter_type.value if self.filter_type else "neutral"
        }
...
class EventFilterFactory:
    ...
    def create_sensitive_filter(sensitive_keys: List[str],
                                replacement: str = "***",
                                filter_type: Optional[FilterType] = None) -> SensitiveDataFilter:
        """创建敏感数据过滤器"""
