...
class GPUManager:
    ...
    def get_gpu_temperature(self) -> Dict[str, Any]:
        """获取GPU温度信息"""
        usage = self.get_gpu_usage()
    ...
    def get_gpu_health_status(self) -> Dict[str, Any]:
        ...
        # 检查温度
        for temp_info in temperature['temperature_info']:
            if temp_info['status'] == 'critical':
                issues.append(
                    f"GPU {temp_info['gpu_id']} temperature critical: {temp_info['temperature_celsius']}°C")
    ...
    def get_available_gpus(self, memory_required: int = 0, max_temperature: float = 80.0) -> List[int]:
        ...
        for gpu_id, gpu_info in self.gpus.items():
            if (gpu_id not in self.allocated_gpus and 
                gpu_info.get('memory_free', 0) >= memory_required and
                gpu_info.get('temperature', 0) <= max_temperature):
