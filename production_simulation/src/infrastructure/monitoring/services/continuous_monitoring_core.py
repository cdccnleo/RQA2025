...
class ContinuousMonitoringSystem(_HealthCheckInterfaceBase):
    ...
    def _init_components(self, project_root: Optional[str]) -> None:
        ...
        self._alert_manager = alert_cls(thresholds) if alert_cls else None
    ...
    def _collect_monitoring_data(self) -> Dict[str, Any]:
        if self._metrics_collector:
            ...
            performance_data = self._metrics_collector.collect_performance_metrics()
            resource_data = self._metrics_collector.collect_resource_usage()
            health_data = self._metrics_collector.collect_health_status()
        else:
