...
class APIDocumentationSearch:
    ...
    def _extract_endpoints(self, data: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
        ...
        if not data:
            return {}
        if isinstance(data, dict) and 'endpoints' in data and isinstance(data['endpoints'], dict):
    ...
    def _sync_navigation_builder_documents(self):
        """同步文档到导航构建器"""
        if hasattr(self._navigation_builder, '_documents'):
            self._navigation_builder._documents = self._endpoint_documents
