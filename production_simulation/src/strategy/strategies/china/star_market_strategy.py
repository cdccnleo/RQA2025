...
class StarMarketStrategy(ChinaMarketStrategy):
    ...
    def execute_strategy(self, signals: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for signal in signals['signals']:
            if signal.get('strength', 1.0) < 0.3:  # 兼容无strength字段的信号，默认强度为1.0
                continue
    ...
    def _create_order_from_signal(self, signal: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """根据信号创建订单"""
        order_type = signal.get('type', 'buy')

        symbol = signal.get('symbol', getattr(self, 'symbol', None))
        if 'buy' in order_type:
            return {
                'symbol': symbol,
                'side': 'buy',
                'quantity': self._calculate_order_quantity(signal.get('strength', 1.0)),
