...
from ..interfaces.strategy_interfaces import StrategyConfig, StrategySignal, StrategyType
...
class BasicStrategy(BaseStrategy):
    ...
    def __init__(self, config):
        # 支持StrategyConfig对象或dict
        if isinstance(config, dict):
            # 如果是dict，提取参数
            strategy_id = config.get("strategy_id", "basic_strategy_001")
            strategy_name = config.get("name", config.get("strategy_name", "Basic Strategy"))
            strategy_type = config.get("strategy_type", "basic")
            self.config = config
        else:
            # 如果是StrategyConfig对象
            strategy_id = config.strategy_id
            strategy_name = config.strategy_name
            strategy_type = config.strategy_type.value if hasattr(config.strategy_type, 'value') else str(config.strategy_type)
            self.config = config
        
        # BaseStrategy需要strategy_id, name, strategy_type三个参数
        super().__init__(strategy_id, strategy_name, strategy_type)
        # 设置策略类型，如果StrategyType没有BASIC，使用字符串
        try:
            self.strategy_type = StrategyType.MOMENTUM  # 使用MOMENTUM作为默认类型
        except (AttributeError, ValueError):
            self.strategy_type = "basic"
...
class ChinaMarketStrategy(ABC):
    ...
    def is_trading_time(self, current_time: pd.Timestamp) -> bool:
        ...
        return (morning_start <= time_str <= morning_end
...
class BasicChinaStrategy(ChinaMarketStrategy):
