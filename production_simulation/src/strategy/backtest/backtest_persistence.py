...
class BacktestPersistence(IBacktestPersistence):
    ...
    def __init__(self, storage_path: str = "./data / backtest"):
        ...
        # 初始化内存缓存
        self._cache = {
            'results': {},  # {backtest_id: (result, timestamp)}
            'configs': {},  # {backtest_id: (config, timestamp)}
            'reports': {}   # {backtest_id: (report, timestamp)}
        }
        
        # 缓存配置
        self._cache_max_size = 100  # 每个缓存类别最大条目数
        self._cache_ttl = 3600  # 缓存过期时间（秒）

        logger.info(f"回测持久化服务初始化完成，存储路径: {self.storage_path}")

    def _get_cache(self, category: str, key: str):
        """
        从缓存获取数据
        
        Args:
            category: 缓存类别 ('results', 'configs', 'reports')
            key: 缓存键
        ...
        data, timestamp = self._cache[category][key]
    ...
    def _set_cache(self, category: str, key: str, data):
        """
        设置缓存数据
        ...
        if len(self._cache[category]) >= self._cache_max_size:
            # 按时间戳排序，移除最旧的
            oldest_key = min(self._cache[category], 
                            key=lambda k: self._cache[category][k][1])
            del self._cache[category][oldest_key]
        
        # 添加到缓存
        self._cache[category][key] = (data, datetime.now().timestamp())
    
    def _clear_cache(self, category: str = None, key: str = None):
        """
        清除缓存
        
        Args:
            category: 缓存类别，如果为None则清除所有类别
            key: 缓存键，如果为None则清除整个类别
        """
        if category is None:
            # 清除所有缓存
            for cat in self._cache:
                self._cache[cat].clear()
        elif category in self._cache:
            if key is None:
                # 清除整个类别
                self._cache[category].clear()
            elif key in self._cache[category]:
                # 清除特定键
                del self._cache[category][key]
    ...
    def save_backtest_result(self, result: BacktestResult) -> bool:
        ...
        try:
            ...
            # 更新缓存
            self._set_cache('results', result.backtest_id, result)
    ...
    def load_backtest_result(self, backtest_id: str) -> Optional[BacktestResult]:
        ...
        try:
            # 先从缓存获取
            cached_result = self._get_cache('results', backtest_id)
            if cached_result:
                logger.info(f"从缓存加载回测结果: {backtest_id}")
            ...
            # 更新缓存
            self._set_cache('results', backtest_id, result)
    ...
    def save_backtest_config(self, config: BacktestConfig) -> bool:
        ...
        try:
            ...
            # 更新缓存
            self._set_cache('configs', config.backtest_id, config)
    ...
    def load_backtest_config(self, backtest_id: str) -> Optional[BacktestConfig]:
        ...
        try:
            # 先从缓存获取
            cached_config = self._get_cache('configs', backtest_id)
            if cached_config:
                logger.info(f"从缓存加载回测配置: {backtest_id}")
            ...
            # 更新缓存
            self._set_cache('configs', backtest_id, config)
    ...
    def delete_backtest_data(self, backtest_id: str) -> bool:
        ...
        try:
            ...
            # 清除缓存
            self._clear_cache('results', backtest_id)
            self._clear_cache('configs', backtest_id)
            self._clear_cache('reports', backtest_id)
