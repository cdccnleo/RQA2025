...
class VisualStrategyEditor:
    ...
    def _has_cycle(self) -> bool:
        ...
        def dfs(node_id):
            ...
            # Access variables from enclosing scope
            visited.add(node_id)  # type: ignore
            recursion_stack.add(node_id)  # type: ignore

            for neighbor in self.nodes[node_id].next_nodes:  # type: ignore
                if neighbor not in visited:  # type: ignore
                    if dfs(neighbor):  # type: ignore
                        return True
                elif neighbor in recursion_stack:  # type: ignore
                    return True

            recursion_stack.remove(node_id)  # type: ignore
            return False
