    ...
    try:
        ...
        def permutation_importance(estimator, X, y, **kwargs):
            ...
            return type('obj', (object,), {
                'importances_mean': [secrets.random() for _ in range(n_features)],
                'importances_std': [secrets.random() * 0.1 for _ in range(n_features)]
            })()
...
class FeatureImportanceAnalyzer:
    ...
    def calculate_permutation_importance(
        ...
        except ImportError as exc:  # pragma: no cover - fallback path tested separately
            logger.warning("permutation_importance 不可用，返回降级结果: %s", exc)
        ...
        self.importance_std = {
            name: std for name, std in zip(feature_names, stds)
        }
