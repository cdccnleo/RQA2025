...
if result:
    ...
    if trades:
        ...
        if trade_dates:
            ...
            print(f"end_date 值: '{end_date_str}'")
            print(f"最后交易日期值: '{last_trade_date}'")
            print(f"类型: end_date={type(end_date)}, last_trade_date={type(last_trade_date)}")
            print(f"是否相等: {end_date_str == last_trade_date}")
            print()
            
            if end_date_str == last_trade_date:
                print(f"✅ 修复成功！end_date ({end_date_str}) 与最后交易日期 ({last_trade_date}) 匹配")
            else:
                print(f"❌ 修复失败！end_date ({end_date_str}) 与最后交易日期 ({last_trade_date}) 不匹配")
