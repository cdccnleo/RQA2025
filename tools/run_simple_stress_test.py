...
class StressTestConfig:
    ...
    concurrent_users: int = 5  # 降低并发数
    requests_per_user: int = 10  # 降低请求数
...
class SimpleStressTestRunner:
    ...
    async def run_stress_test(self) -> Dict[str, Any]:
        """运行压力测试"""
        logger.info(f"开始简化压力测试: {self.config.concurrent_users} 并发用户, "
                   f"{self.config.requests_per_user} 请求/用户")
        
        # 检查API服务器是否运行
        if not self._check_api_server():
            logger.warning("API服务器未运行，尝试启动...")
            if not self._start_api_server():
                return {"error": "无法启动API服务器"}
    ...
    def _start_api_server(self) -> bool:
        ...
        try:
            ...
            # 使用简单的API服务器脚本
            api_script = project_root / "scripts" / "simple_api_server.py"
        ...
        async with aiohttp.ClientSession(
            ...
            for request_id in range(self.config.requests_per_user):
                ...
                try:
                    ...
                    # 添加随机延迟模拟真实用户行为
                    await asyncio.sleep(0.01 + (user_id % 5) * 0.001)
    ...
    def save_report(self, report: Dict[str, Any], filename: str = "simple_stress_test_report.json"):
        """保存测试报告"""
        # 确保日志目录存在
