...
RQA2025是一个基于业务流程驱动的量化交易系统，采用分层架构设计，具备高可用性、高扩展性和高性能的特点。系统基于职责定位划分为13个架构层级（8个核心子系统 + 5个辅助支撑层级），每层都有明确的职责和边界，通过接口驱动设计实现层间解耦。

> **架构优化说明**: 2026-03-08进行架构优化，将17层优化至13层，合并了弹性层、工具层、流处理层、自动化层到核心服务层和基础设施层，降低了架构复杂度，提高了可维护性。
...
graph TB
    ...
    subgraph "辅助支撑层 ⭐ 5个支撑层级"
        direction TB

        subgraph "架构支撑 (3个)"
            direction LR
            AS1[核心服务层<br/>214文件<br/>⭐ 架构编排<br/>⭐ 含弹性层/工具层/自动化层]
