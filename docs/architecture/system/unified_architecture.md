...
<!-- 
版本更新记录：
2025-07-19 v3.9.1 - 数据存储架构增强
    - 新增数据一致性保障机制 (DataConsistencyManager)
    - 新增统一查询接口 (UnifiedQueryInterface)
    - 新增归档失败处理策略 (ArchiveFailureHandler)
    - 完善存储模块架构和测试覆盖
    - 更新架构设计文档
...
**集成要点**：
- MiniQMTAdapter在所有关键操作前后调用SecurityService进行安全校验、访问控制和审计。
- 通过ConfigManager实现MiniQMT配置的动态加载和热更新。
- 监控指标采集包括连接状态、数据延迟、订单数量、缓存命中率、重连次数等。
- 错误处理流程自动区分连接错误、数据错误和其他异常，分别执行重连、数据修正或默认处理，并实时更新监控指标。
