...
MiniQMTAdapter在关键操作前后调用SecurityService进行安全校验、访问控制和审计。

## 典型调用流程
- 连接前：SecurityService.validate_config(config)
- 操作前：SecurityService.check_access(resource, user)
- 操作后：SecurityService.audit(action, details)

## 集成示例
```python
security = SecurityService()
if not security.validate_config(miniqmt_config):
    raise InvalidConfigError
if not security.check_access('miniqmt.data', current_user):
    raise PermissionDenied
# ...业务操作...
security.audit('miniqmt_data_access', {'user': current_user})
```
