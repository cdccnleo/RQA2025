...
**检查时间**: 2026-01-10T13:42:50.876187
...
- **文件**: N/A
- **状态**: warning
- **消息**: 业务流程编排器集成: 仅找到 0/1 个必需模式
- **匹配情况**: 0/1

### websocket_broadcast ⚠️

- **文件**: N/A
- **状态**: warning
- **消息**: WebSocket实时广播: 仅找到 0/1 个必需模式
- **匹配情况**: 0/1
...
- **文件**: N/A
- **状态**: warning
- **消息**: 统一日志系统使用（API路由）: 仅找到 0/1 个必需模式
- **匹配情况**: 0/1
...
- **文件**: N/A
- **状态**: passed
- **消息**: MLIntegrationAnalyzer类定义: 找到 3/1 个必需模式
- **匹配情况**: 3/1
...
- **文件**: N/A
- **状态**: warning
- **消息**: 通过统一适配器工厂访问ML层: 仅找到 0/1 个必需模式
- **匹配情况**: 0/1
...
### ml_adapter_in_service ⚠️

- **文件**: N/A
- **状态**: warning
- **消息**: 回测服务中的ML层适配器获取: 仅找到 0/1 个必需模式
- **匹配情况**: 0/1

### model_prediction_usage ⚠️

- **文件**: N/A
- **状态**: warning
- **消息**: 模型预测服务使用: 仅找到 0/1 个必需模式
- **匹配情况**: 0/1
...
- **文件**: N/A
- **状态**: warning
- **消息**: BusinessProcessOrchestrator使用: 仅找到 0/2 个必需模式
- **匹配情况**: 0/2
...
- **文件**: N/A
- **状态**: warning
- **消息**: 流程状态管理（业务流程编排器使用）: 仅找到 0/2 个必需模式
- **匹配情况**: 0/2
...
```json
{
  "timestamp": "2026-01-10T13:42:50.876187",
  "frontend_modules": {
    "dashboard_exists": {
  ...
  "backend_apis": {
    ...
    "business_orchestrator": {
      "status": "warning",
      "message": "业务流程编排器集成: 仅找到 0/1 个必需模式",
      "found_patterns": [],
      "missing_patterns": [
    ...
    "websocket_broadcast": {
      "status": "warning",
      "message": "WebSocket实时广播: 仅找到 0/1 个必需模式",
      "found_patterns": [],
      "missing_patterns": [
        "WebSocketManager|manager\\.broadcast|broadcast.*backtest"
      ],
      "found_count": 0,
      "required_count": 1
    }
  ...
  "architecture_compliance": {
    "unified_logger": {
      "status": "warning",
      "message": "统一日志系统使用（API路由）: 仅找到 0/1 个必需模式",
      "found_patterns": [],
      "missing_patterns": [
        "get_unified_logger|统一日志"
      ],
    ...
    "ml_layer_access": {
      "status": "warning",
      "message": "机器学习层组件访问: 仅找到 0/1 个必需模式",
      "found_patterns": [],
      "missing_patterns": [
  ...
  "ml_integration": {
    "ml_integration_analyzer": {
      "status": "passed",
      "message": "MLIntegrationAnalyzer类定义: 找到 3/1 个必需模式",
      "found_patterns": [
        [
    ...
    "adapter_factory_usage": {
      "status": "warning",
      "message": "通过统一适配器工厂访问ML层: 仅找到 0/1 个必需模式",
      "found_patterns": [],
      "missing_patterns": [
    ...
    "ml_layer_usage": {
      "status": "passed",
      "message": "ML层组件使用（模型预测）: 找到 2/1 个必需模式",
      "found_patterns": [
        [
    ...
    "ml_adapter_in_service": {
      "status": "warning",
      "message": "回测服务中的ML层适配器获取: 仅找到 0/1 个必需模式",
      "found_patterns": [],
      "missing_patterns": [
    ...
    "model_prediction_usage": {
      "status": "warning",
      "message": "模型预测服务使用: 仅找到 0/1 个必需模式",
      "found_patterns": [],
      "missing_patterns": [
  ...
  "business_orchestration": {
    "orchestrator_usage": {
      "status": "warning",
      "message": "BusinessProcessOrchestrator使用: 仅找到 0/2 个必需模式",
      "found_patterns": [],
      "missing_patterns": [
    ...
    "process_management": {
      "status": "warning",
      "message": "流程状态管理（业务流程编排器使用）: 仅找到 0/2 个必需模式",
      "found_patterns": [],
      "missing_patterns": [
    ...
    "event_publishing": {
      "status": "warning",
      "message": "回测流程事件发布: 仅找到 0/1 个必需模式",
      "found_patterns": [],
      "missing_patterns": [
