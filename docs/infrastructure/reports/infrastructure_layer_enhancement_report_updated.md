...
## Prometheus监控注册隔离与单元测试健壮性提升

### 1. 问题背景
- 多模块（如VisualMonitor、DeploymentValidator、ApplicationMonitor、PrometheusMonitor、MonitoringSystem等）在单元测试并发执行时，因Prometheus指标全局注册导致“Duplicated timeseries”冲突，影响CI稳定性。

### 2. 解决方案
- 采用最佳实践，所有涉及Prometheus指标注册的模块均支持通过构造参数传递registry（CollectorRegistry），生产环境默认全局REGISTRY，测试用例传入独立CollectorRegistry，实现注册表彻底隔离。
- 重构相关模块（如VisualMonitor、DeploymentValidator、ApplicationMonitor、PrometheusMonitor、MonitoringSystem、BacktestMonitor、SystemMonitor、decorators.py、performance_monitor.py等），所有Prometheus注册点均支持registry参数。
- 修正/补充所有相关测试用例，实例化时传入独立CollectorRegistry，消除注册冲突。

### 3. 推进成效
- 监控/性能/告警/回测/系统资源相关核心模块已全部完成Prometheus注册隔离重构。
- 所有相关单元测试用例已修正，pytest-xdist并发下无注册冲突，CI健壮性大幅提升。
- 形成统一开发规范：后续所有新模块/新用例均需支持registry参数隔离。

### 4. 后续建议
- 持续推进decorators、performance_monitor、MonitoringSystem等模块的registry隔离与测试补全。
- 持续提升异常分支、极端场景的测试覆盖率，保障可观测性与工程健壮性。
