...
**复核时间**: 2026-01-10T08:14:25.463852
...
- **文件**: N/A
- **状态**: passed
- **消息**: AdapterRegistry使用（通过统一适配器工厂）: 找到 24/1 个必需模式
- **匹配情况**: 24/1
...
- **文件**: N/A
- **状态**: passed
- **消息**: 适配器注册（通过统一适配器工厂自动管理）: 找到 24/1 个必需模式
- **匹配情况**: 24/1
...
- **文件**: N/A
- **状态**: passed
- **消息**: 适配器选择逻辑（通过get_adapter方法）: 找到 6/1 个必需模式
- **匹配情况**: 6/1
...
- **文件**: N/A
- **状态**: passed
- **消息**: 流程指标收集（通过update_process_state传递metrics）: 找到 3/1 个必需模式
- **匹配情况**: 3/1
...
- **文件**: N/A
- **状态**: passed
- **消息**: 配置变更通知（在datasource_routes.py中通过EventBus和WebSocket实现）: 找到 11/1 个必需模式
- **匹配情况**: 11/1
...
```json
{
  "timestamp": "2026-01-10T08:14:25.463852",
  "frontend_modules": {
    "data_sources_config_dashboard": {
  ...
  "architecture_compliance": {
    ...
    "adapter_registry": {
      "status": "passed",
      "message": "AdapterRegistry使用（通过统一适配器工厂）: 找到 24/1 个必需模式",
      "found_patterns": [
        [
          "get_unified_adapter_factory|adapter_factory|AdapterRegistry",
          24
        ]
      ],
      "found_count": 24,
  ...
  "adapter_pattern": {
    ...
    "adapter_registration": {
      "status": "passed",
      "message": "适配器注册（通过统一适配器工厂自动管理）: 找到 24/1 个必需模式",
      "found_patterns": [
        [
          "get_unified_adapter_factory|adapter_factory|register.*adapter|AdapterRegistry|register_source",
          24
        ]
      ],
      "found_count": 24,
    ...
    "adapter_selection": {
      "status": "passed",
      "message": "适配器选择逻辑（通过get_adapter方法）: 找到 6/1 个必需模式",
      "found_patterns": [
        [
          "get_adapter|adapter_factory\\.get|BusinessLayerType\\.DATA|select.*adapter|choose.*adapter",
          6
        ]
      ],
      "found_count": 6,
  ...
  "business_orchestration": {
    ...
    "process_metrics": {
      "status": "passed",
      "message": "流程指标收集（通过update_process_state传递metrics）: 找到 3/1 个必需模式",
      "found_patterns": [
        [
          "update_process_state.*metrics|metrics\\s*=\\s*\\{|process.*metrics|orchestrator.*metrics",
          3
        ]
      ],
      "found_count": 3,
    ...
    "config_notification": {
      "status": "passed",
      "message": "配置变更通知（在datasource_routes.py中通过EventBus和WebSocket实现）: 找到 11/1 个必需模式",
      "found_patterns": [
        [
          "broadcast_data_source_change|websocket_manager\\.broadcast|EventBus.*publish|CONFIG_UPDATED|\\.publish\\(.*CONFIG",
          11
        ]
      ],
      "found_count": 11,
