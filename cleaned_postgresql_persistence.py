...
def get_all_stock_basic_info(limit: int = 100, offset: int = 0) -> List[Dict[str, Any]]:
    """
    获取所有股票基本信息（分页）
...
def persist_akshare_fund_data_to_postgresql(
    source_id: str,
    data: List[Dict[str, Any]],
    metadata: Dict[str, Any],
    source_config: Dict[str, Any]
) -> Dict[str, Any]:
    """将AKShare基金数据持久化到PostgreSQL"""
    start_time = time.time()
    conn = None
    ...
    try:
        ...
        # 确保表存在
        possible_paths = [
            os.path.join(os.path.dirname(__file__), "..", "..", "..", "scripts", "sql", "akshare_fund_data_schema.sql"),
            os.path.join("scripts", "sql", "akshare_fund_data_schema.sql"),
            os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "scripts", "sql", "akshare_fund_data_schema.sql")
        ]
        script_path = None
        for path in possible_paths:
            abs_path = os.path.abspath(path)
        ...
        insert_query = """
            INSERT INTO akshare_fund_data (
                source_id, symbol, date, net_value, accumulated_value,
                fund_type, daily_return, data_source, collected_at, persistence_timestamp
            ) VALUES (
                %s, %s, %s, %s, %s, %s, %s, %s, %s, %s
            )
            ON CONFLICT (source_id, symbol, date) 
...
def persist_akshare_macro_data_to_postgresql(
    source_id: str,
    data: List[Dict[str, Any]],
    metadata: Dict[str, Any],
    source_config: Dict[str, Any]
) -> Dict[str, Any]:
    """将AKShare宏观经济数据持久化到PostgreSQL"""
    start_time = time.time()
    conn = None
    ...
    try:
        ...
        # 确保表存在
        possible_paths = [
            os.path.join(os.path.dirname(__file__), "..", "..", "..", "scripts", "sql", "akshare_macro_data_schema.sql"),
            os.path.join("scripts", "sql", "akshare_macro_data_schema.sql"),
            os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "scripts", "sql", "akshare_macro_data_schema.sql")
        ]
        script_path = None
        for path in possible_paths:
            abs_path = os.path.abspath(path)
        ...
        insert_query = """
            INSERT INTO akshare_macro_data (
                source_id, indicator, date, value, unit, macro_type, period,
                data_source, collected_at, persistence_timestamp
            ) VALUES (
                %s, %s, %s, %s, %s, %s, %s, %s, %s, %s
            )
            ON CONFLICT (source_id, indicator, date) 
            DO UPDATE SET
...
# News and Alternative Data Persistence Functions

def persist_akshare_news_data_to_postgresql(
    source_id: str,
    data: List[Dict[str, Any]],
    metadata: Dict[str, Any],
    source_config: Dict[str, Any]
) -> Dict[str, Any]:
    """将AKShare新闻数据持久化到PostgreSQL"""
    start_time = time.time()
    conn = None
    ...
    try:
        ...
        # 确保表存在
        possible_paths = [
            os.path.join(os.path.dirname(__file__), "..", "..", "..", "scripts", "sql", "akshare_news_data_schema.sql"),
            os.path.join("scripts", "sql", "akshare_news_data_schema.sql"),
            os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "scripts", "sql", "akshare_news_data_schema.sql")
        ]
        script_path = None
        for path in possible_paths:
            abs_path = os.path.abspath(path)
        ...
        insert_query = """
            INSERT INTO akshare_news_data (
                source_id, title, content, summary, publish_time, url,
                category, tags, news_source, news_source_code,
                update_frequency, delay, hot_index, data_source,
                collected_at, persistence_timestamp
            ) VALUES (
                %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s
            )
            ON CONFLICT (source_id, title, publish_time) 
            DO UPDATE SET
        ...
        for record in data:
            try:
                # 字段映射：支持中英文字段名
                # AKShare返回的字段可能是中文（"发布时间"、"内容"）或英文（"publish_time"、"content"）
                title = record.get("title") or record.get("标题") or record.get("title_cn", "")
                content = record.get("content") or record.get("内容") or record.get("content_cn")
                summary = record.get("summary") or record.get("摘要") or record.get("summary_cn")
                
                # 解析发布时间（支持多种字段名）
                publish_time_str = (
                    record.get("publish_time") or 
                    record.get("发布时间") or 
                    record.get("publish_time_cn") or 
                    ""
                )
                publish_time = None
                if publish_time_str:
                    try:
                        ...
                        # 如果是ISO格式字符串
                        elif "T" in str(publish_time_str):
                            publish_time = datetime.fromisoformat(str(publish_time_str).replace("Z", "+00:00"))
                        # 如果是标准日期时间格式
                        else:
                            publish_time_str_clean = str(publish_time_str)[:19]
                            publish_time = datetime.strptime(publish_time_str_clean, "%Y-%m-%d %H:%M:%S")
                    except:
                        try:
                            # 尝试只解析日期
                            publish_time_str_clean = str(publish_time_str)[:10]
                            publish_time = datetime.strptime(publish_time_str_clean, "%Y-%m-%d")
                        except:
                            publish_time = None
                
                # 如果没有标题，使用内容的前100个字符作为标题
                if not title and content:
                    title = str(content)[:100]
                
                # 如果没有标题也没有内容，跳过这条记录
                if not title:
                    logger.warning(f"跳过无标题的记录: {record}")
                    skipped_count += 1
                    continue
                ...
                cursor.execute(insert_query, (
                    ...
                    summary,
                    publish_time,
                    record.get("url") or record.get("链接"),
                    record.get("category") or record.get("分类"),
                    record.get("tags") or record.get("标签"),
                    record.get("news_source") or record.get("新闻来源"),
                    record.get("news_source_code") or record.get("新闻来源代码"),
                    record.get("update_frequency") or record.get("更新频率"),
                    record.get("delay") or record.get("延迟"),
                    record.get("hot_index") or record.get("热度指数"),
                    "akshare",
                    collected_at,
                    persistence_timestamp
                ))
                ...
                logger.warning(f"插入新闻数据失败: {e}", exc_info=True)
                skipped_count += 1
...
def query_latest_news_data_from_postgresql(
    source_id: str,
    limit: int = 10
) -> List[Dict[str, Any]]:
    """
    从PostgreSQL查询指定数据源的最新新闻数据
    
    Args:
        source_id: 数据源ID
        limit: 返回的记录数限制，默认10条
    ...
    try:
        conn = get_db_connection()
        if not conn:
            logger.warning(f"无法获取数据库连接，无法查询数据源 {source_id} 的最新新闻数据")
            return []
        ...
        cursor.execute("""
            SELECT 
                id, source_id, title, content, summary, publish_time, url,
                category, tags, news_source, news_source_code,
                update_frequency, delay, hot_index, data_source,
                collected_at, persistence_timestamp
            FROM akshare_news_data 
            WHERE source_id = %s
            ORDER BY persistence_timestamp DESC, publish_time DESC
            LIMIT %s
        """, (source_id, limit))
        ...
        if not rows:
            logger.debug(f"数据源 {source_id} 在PostgreSQL中无新闻数据")
            return []
        ...
        for row in rows:
            ...
            # 转换时间戳为字符串
            for time_field in ['publish_time', 'collected_at', 'persistence_timestamp']:
                if record.get(time_field) and hasattr(record[time_field], 'isoformat'):
                    record[time_field] = record[time_field].isoformat()
            result.append(record)
        
        logger.info(f"从PostgreSQL查询到数据源 {source_id} 的 {len(result)} 条最新新闻数据")
        return result
        
    except Exception as e:
        logger.error(f"查询数据源 {source_id} 最新新闻数据失败: {e}", exc_info=True)
        return []
    finally:
        if conn:
            return_db_connection(conn)
...
def query_latest_stock_data_from_postgresql(
    source_id: str,
    limit: int = 10,
    data_type: str = None
) -> List[Dict[str, Any]]:
    """
    从PostgreSQL查询指定数据源的最新股票数据

    Args:
        source_id: 数据源ID
        limit: 返回的记录数限制，默认10条
        data_type: 数据类型过滤，如'daily', '1min', '5min'等，为None则查询所有类型
    ...
    try:
        conn = get_db_connection()
        if not conn:
            logger.warning(f"无法获取数据库连接，无法查询数据源 {source_id} 的最新股票数据")
            return []
        
        cursor = conn.cursor()

        # 首先确保表结构是最新的（这会调用迁移逻辑）
        if not ensure_table_exists():
            logger.warning(f"无法确保表结构正确，无法查询数据源 {source_id} 的最新股票数据")
            return []
        ...
        # 构建查询SQL，根据字段是否存在动态调整
        if data_type_exists:
            query = """
                SELECT
                    id, source_id, symbol, date, data_type, open_price, high_price, low_price,
                    close_price, volume, amount, pct_change, change,
            ...
            # 兼容旧表结构（没有data_type字段）
            logger.warning(f"数据源 {source_id} 的表结构较旧，缺少data_type字段，使用兼容查询")
            query = """
                SELECT
                    id, source_id, symbol, date, 'daily' as data_type, open_price, high_price, low_price,
                    close_price, volume, amount, pct_change, change,
                    turnover_rate, amplitude, data_source,
                    collected_at, persistence_timestamp
                FROM akshare_stock_data
        ...
        query += " ORDER BY persistence_timestamp DESC, date DESC LIMIT %s"
        params.append(limit)
        ...
        if not rows:
            logger.debug(f"数据源 {source_id} 在PostgreSQL中无股票数据")
            return []
        ...
        for row in rows:
            record = dict(zip(columns, row))
            # 转换时间戳为字符串
            for time_field in ['date', 'collected_at', 'persistence_timestamp']:
                if record.get(time_field) and hasattr(record[time_field], 'isoformat'):
                    record[time_field] = record[time_field].isoformat()

            # 转换Decimal对象为float，确保JSON序列化兼容
        ...
        logger.info(f"从PostgreSQL查询到数据源 {source_id} 的 {len(result)} 条最新股票数据")
        return result
        
    except Exception as e:
        logger.error(f"查询数据源 {source_id} 最新股票数据失败: {e}", exc_info=True)
        return []
    finally:
        if conn:
            return_db_connection(conn)
...
def query_latest_macro_data_from_postgresql(
    source_id: str,
    limit: int = 10
) -> List[Dict[str, Any]]:
    """
    从PostgreSQL查询指定数据源的最新宏观经济数据
    ...
    try:
        conn = get_db_connection()
        if not conn:
            logger.warning(f"无法获取数据库连接，无法查询数据源 {source_id} 的最新宏观经济数据")
            return []
        
        cursor = conn.cursor()
        
        # 查询最新的宏观经济数据（假设表名为akshare_macro_data）
        # 注意：如果表不存在，返回空列表
        try:
            cursor.execute("""
                SELECT 
                    id, source_id, indicator, date, value, unit, macro_type, period,
                    data_source, collected_at, persistence_timestamp
                FROM akshare_macro_data 
                WHERE source_id = %s
                ORDER BY persistence_timestamp DESC, date DESC
                LIMIT %s
            """, (source_id, limit))
            ...
            if not rows:
                logger.debug(f"数据源 {source_id} 在PostgreSQL中无宏观经济数据")
                return []
            ...
            logger.info(f"从PostgreSQL查询到数据源 {source_id} 的 {len(result)} 条最新宏观经济数据")
            return result
        except Exception as table_error:
            logger.debug(f"表akshare_macro_data可能不存在: {table_error}")
            return []
        
    except Exception as e:
        logger.error(f"查询数据源 {source_id} 最新宏观经济数据失败: {e}", exc_info=True)
        return []
    finally:
        if conn:
            return_db_connection(conn)
...
def query_latest_index_data_from_postgresql(
    source_id: str,
    limit: int = 10
) -> List[Dict[str, Any]]:
    """
    从PostgreSQL查询指定数据源的最新指数数据
    ...
    try:
        conn = get_db_connection()
        if not conn:
            logger.warning(f"无法获取数据库连接，无法查询数据源 {source_id} 的最新指数数据")
            return []
        ...
        try:
            cursor.execute("""
                SELECT 
                    id, source_id, symbol, date, open_price, high_price, low_price,
                    close_price, volume, amount, pct_change, change,
                    data_source, collected_at, persistence_timestamp
            ...
            if not rows:
                logger.debug(f"数据源 {source_id} 在PostgreSQL中无指数数据")
                return []
            ...
            logger.info(f"从PostgreSQL查询到数据源 {source_id} 的 {len(result)} 条最新指数数据")
            return result
        except Exception as table_error:
            logger.debug(f"表akshare_index_data可能不存在: {table_error}")
            return []
        
    except Exception as e:
        logger.error(f"查询数据源 {source_id} 最新指数数据失败: {e}", exc_info=True)
        return []
    finally:
        if conn:
            return_db_connection(conn)
...
def query_latest_data_from_postgresql(
    source_id: str,
    source_type: str,
    limit: int = 10
) -> List[Dict[str, Any]]:
    """
    从PostgreSQL查询指定数据源的最新数据（通用函数）
    
    Args:
        source_id: 数据源ID
        source_type: 数据源类型（股票数据、财经新闻、宏观经济、指数数据等）
        limit: 返回的记录数限制，默认10条
    ...
    """
    if source_type == "财经新闻":
        return query_latest_news_data_from_postgresql(source_id, limit)
    elif source_type == "股票数据":
        return query_latest_stock_data_from_postgresql(source_id, limit)
    elif source_type == "宏观经济":
        return query_latest_macro_data_from_postgresql(source_id, limit)
    elif source_type == "指数数据":
        return query_latest_index_data_from_postgresql(source_id, limit)
    else:
        logger.warning(f"数据源类型 {source_type} 暂不支持从PostgreSQL查询最新数据")
        return []


def persist_akshare_alternative_data_to_postgresql(
    source_id: str,
    data: List[Dict[str, Any]],
    metadata: Dict[str, Any],
    source_config: Dict[str, Any]
) -> Dict[str, Any]:
    """将AKShare另类数据持久化到PostgreSQL"""
    start_time = time.time()
    conn = None
    ...
    try:
        ...
        # 确保表存在
        possible_paths = [
            os.path.join(os.path.dirname(__file__), "..", "..", "..", "scripts", "sql", "akshare_alternative_data_schema.sql"),
            os.path.join("scripts", "sql", "akshare_alternative_data_schema.sql"),
            os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "scripts", "sql", "akshare_alternative_data_schema.sql")
        ]
        script_path = None
        for path in possible_paths:
            abs_path = os.path.abspath(path)
        ...
        insert_query = """
            INSERT INTO akshare_alternative_data (
                source_id, data_category, data_subtype, keyword, city,
                index_value, index_name, rank, trend, date, value, unit,
                movie_name, boxoffice, boxoffice_unit, product_name,
                sales_volume, price, shop_name, change_value, change_pct,
                aqi, pm25, pm10, co, no2, o3, so2, quality_level,
                temperature_high, temperature_low, weather, wind_direction,
                wind_level, humidity, update_frequency, data_source,
                collected_at, persistence_timestamp
            ) VALUES (
            ...
            ON CONFLICT (source_id, data_category, data_subtype, keyword, city, date) 
            DO UPDATE SET
        ...
        for record in data:
            try:
                ...
                cursor.execute(insert_query, (
                    ...
                    record.get("boxoffice"),
                    record.get("boxoffice_unit"),
                    ...
                    record.get("temperature_high"),
                    record.get("temperature_low"),
                    record.get("weather"),
                    record.get("wind_direction"),
                    record.get("wind_level"),
                    record.get("humidity"),
