...
# -*- coding: utf-8 -*-
"""
Phase 4 交易策略执行引擎重构脚本
修复策略执行引擎，实现完整的IStrategy接口
"""
...
class BaseStrategy(IStrategy):
    ...
    def set_parameters(self, parameters: Dict[str, Any]) -> bool:
        try:
            ...
            self.logger.info(f"Parameters updated for strategy {self.strategy_id}")
            return True
    ...
    def initialize(self, config: Dict[str, Any]) -> bool:
        try:
            ...
            self.logger.error(f"Failed to initialize strategy {self.strategy_id}: {e}")
    ...
    def should_enter_position(self, symbol: str, data: pd.DataFrame) -> Optional[StrategySignal]:
        ...
        if current_price > prev_price * 1.02:  # 上涨2%以上
            return StrategySignal(
                ...
                symbol=symbol,
                price=current_price,
                quantity=self.calculate_position_size(100000, 0.02, symbol),  # 假设资本和风险
                confidence=0.7,
                timestamp=datetime.now(),
    ...
    def load_state(self, path: str) -> bool:
        try:
            ...
            self.logger.info(f"Strategy state loaded from {path}")
            return True
...
class StrategyEngine:
    ...
    def register_strategy(self, strategy: IStrategy) -> bool:
        ...
        if not strategy_id:
            self.logger.error("Strategy must have a strategy_id")
            return False

        self.strategies[strategy_id] = strategy
        self.logger.info(f"Strategy {strategy_id} registered")
        return True
    ...
    def execute_strategy(self, strategy_id: str, market_data: MarketData) -> List[StrategySignal]:
        """执行策略"""
...
def test_strategy_engine():
    ...
    # 创建并注册策略
    trend_strategy = TrendFollowingStrategy("trend_001")
    ...
    success = engine.register_strategy(trend_strategy)
    ...
    signals = engine.execute_strategy("trend_001", market_data)
    print(f"生成信号数量: {len(signals)}")

    for signal in signals:
        print(f"信号: {signal.signal_type} {signal.symbol} {signal.quantity}股 @ {signal.price}")

    # 测试策略状态查询
    strategy = engine.get_strategy("trend_001")
    if strategy:
        print(f"策略名称: {strategy.get_strategy_name()}")
        print(f"策略状态: {strategy.get_strategy_status()}")
        print(f"支持的交易品种: {strategy.get_supported_symbols()}")
