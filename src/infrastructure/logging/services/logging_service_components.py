...
class BaseService:
    ...
    def __init__(self, service_name_or_config="BaseService", service_id: int = None):
        """初始化服务"""
        # 支持配置字典或服务名
        if isinstance(service_name_or_config, dict):
            config = service_name_or_config
            self.service_name = config.get('name', 'BaseService')
            self._name = self.service_name
            self._version = config.get('version', '1.0.0')
            self._description = config.get('description', '')
            self._enabled = config.get('enabled', True)
            self.service_id = service_id
        else:
            self.service_name = service_name_or_config
            self.service_id = service_id
            self._name = service_name_or_config
            self._version = "1.0.0"
            self._description = ""
            self._enabled = True
    ...
    def record_request(self, success: bool, duration: float, method: str, endpoint: str):
        """记录请求"""
        if 'total_requests' not in self._stats:
            self._stats['total_requests'] = 0
        if 'total_response_time' not in self._stats:
            self._stats['total_response_time'] = 0.0
        if 'successful_requests' not in self._stats:
            self._stats['successful_requests'] = 0
        if 'failed_requests' not in self._stats:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        if 'avg_response_time' in stats:
            stats['average_response_time'] = stats['avg_response_time']
        return stats
