...
class HotReloadService:
    ...
    def reload_configuration(self, config: Dict[str, Any]):
        """重载配置
        ...
        with self._lock:
            ...
            if 'reload_interval' in config:
                self.reload_interval = config['reload_interval']
            elif 'check_interval' in config:
                self.reload_interval = config['check_interval']
            if 'enabled' in config:
                # 如果配置中有enabled字段，可以在这里处理
                pass
            logger.info("配置已重载")
