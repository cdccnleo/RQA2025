...
try:
    ...
    if SYNCHRONOUS is None:  # 如果InfluxDB的SYNCHRONOUS也未定义
        SYNCHRONOUS = None
...
class ApplicationMonitor:
    ...
    def _init_prometheus_metrics(self):
        ...
        if not PROMETHEUS_AVAILABLE:
            ...
            self.registry = None
            self.prom_function_calls = None
            self.prom_function_errors = None
            self.prom_function_duration = None
            return
    ...
    def _init_influxdb_client(self):
        ...
        if not INFLUXDB_AVAILABLE:
            logger.debug("influxdb_client not available, InfluxDB metrics persistence disabled")
            return
