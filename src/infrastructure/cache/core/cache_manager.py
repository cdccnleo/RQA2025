...
class UnifiedCacheManager:
    ...
    def _init_multi_level_cache(self, multi_level_config: Optional[MultiLevelCacheConfig] = None) -> None:
        # 尝试从不同位置导入 MultiLevelCache
        multi_level_class = None
        try:
            from ..advanced_cache import MultiLevelCache as AdvancedMultiLevelCache
            multi_level_class = AdvancedMultiLevelCache
        except ImportError:
            try:
        ...
        # 如果没有配置或类不存在，跳过初始化
        if multi_level_class is None:
            return

        # 尝试使用提供的配置创建实例
        try:
            # 检查 MultiLevelCache 的 __init__ 签名
            import inspect
            sig = inspect.signature(multi_level_class.__init__)
            params = list(sig.parameters.keys())
            
            # 如果 __init__ 不接受参数（除了 self），则无参调用
            if len(params) <= 1:  # 只有 self 参数
                candidate = multi_level_class()
            elif config_obj:
                candidate = multi_level_class(config_obj)
            else:
                # 使用默认配置（None），让MultiLevelCache内部处理
                candidate = multi_level_class(None)
