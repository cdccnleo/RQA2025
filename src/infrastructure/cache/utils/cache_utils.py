...
def generate_cache_key(base_key: str, **kwargs) -> str:
    ...
    for k, v in sorted(kwargs.items()):
        if isinstance(v, (list, tuple)):
            # 对于列表/元组，包含长度信息
            key_parts.append(f"{k}:{len(v)}:{hash(str(v))}")
        else:
            key_parts.append(f"{k}:{v}")
    return serialize_cache_key(tuple(key_parts))


def is_cache_expired(timestamp: Optional[float], ttl_seconds: int = 3600) -> bool:
    """检查缓存是否过期"""
...
def safe_cache_operation(operation: Callable, default_return: Any = None, log_level: str = "error"):
    """安全的缓存操作执行"""
    try:
        return operation()
    except Exception as e:
        log_func = getattr(logger, log_level.lower(), logger.error)
        log_func(f"Safe cache operation failed: {e}")
        return default_return


def compress_cache_data(data: Any, compression_threshold: int = 1024) -> Any:
    """压缩缓存数据"""
    if data is None:
        return None

    # 对于小数据不压缩
    if isinstance(data, (str, bytes)) and len(data) < compression_threshold:
        return data

    # 简单的压缩实现（实际项目中应该使用更高效的算法）
    if isinstance(data, str):
        # 对于字符串，进行简单的压缩
        return f"compressed:{hash(data)}"  # 简化实现
...
def decompress_cache_data(data: Any) -> Any:
    """解压缓存数据"""
    if data is None:
        return None
...
def validate_cache_config(config: Dict[str, Any]) -> bool:
    ...
    required_keys = ['max_size', 'ttl']
    for key in required_keys:
        if key not in config:
            return False

    # 检查数值类型
    if not isinstance(config.get('max_size'), (int, float)):
        return False
    if not isinstance(config.get('ttl'), (int, float)):
        return False
