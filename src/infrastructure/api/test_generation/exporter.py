...
class TestSuiteExporter:
    ...
    def export_to_json(self, test_suites=None, output_path="docs/api/tests"):
        """导出为JSON格式（兼容性方法）"""
        # 如果传递的是单个TestSuite对象，转换为字典格式
        if isinstance(test_suites, TestSuite):
            test_suites = {"default": test_suites}
        return self.export(test_suites=test_suites, format_type="json", output_dir=output_path)

    def export_to_yaml(self, test_suites=None, output_path="docs/api/tests"):
        """导出为YAML格式（兼容性方法）"""
        return self.export(test_suites=test_suites, format_type="yaml", output_dir=output_path)

    def export_to_html(self, test_suites=None, output_path="docs/api/tests"):
        """导出为HTML格式（兼容性方法）"""
        return self.export(test_suites=test_suites, format_type="html", output_dir=output_path)

    def get_supported_formats(self):
        """获取支持的导出格式"""
        return ["json", "yaml", "html", "markdown"]
