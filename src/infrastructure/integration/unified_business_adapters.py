...
class UnifiedBusinessAdapter(BaseAdapter[Any, Any], IBusinessAdapter):
    ...
    # 注意：单例服务管理已迁移到InfrastructureServiceRegistry
    ...
    def _get_service_from_registry(self, service_name: str) -> Optional[Any]:
        """
        从基础设施服务注册表获取服务
        ...
        try:
            ...
            if service is None:
                self._logger.debug(f"服务 {service_name} 未在注册表中找到")
            
            return service
        except Exception as e:
            self._logger.warning(f"从服务注册表获取服务失败 {service_name}: {e}")
            return None
    ...
    def _init_infrastructure_services(self):
        ...
        try:
            ...
            self._logger.warning(f"{self._layer_type.value}层基础设施服务初始化失败: {e}，使用降级服务")
    ...
    def _ensure_services_registered(self, registry):
        ...
        try:
            ...
            self._logger.debug(f"部分基础设施服务导入失败（非关键）: {e}")
...
def get_unified_adapter_factory() -> BusinessAdapterFactory:
    """
    获取统一的适配器工厂实例（便捷函数）
    
    Returns:
        BusinessAdapterFactory: 适配器工厂实例（单例）
    """
    return BusinessAdapterFactory()
