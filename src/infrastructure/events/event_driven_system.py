...
class Event:
    ...
    def __post_init__(self):
        """初始化后处理"""
        # 如果data为空且payload是字典，则将payload赋值给data
        if not self.data and isinstance(self.payload, dict):
    ...
    def is_expired(self) -> bool:
        ...
        if self.ttl is not None:
            if (datetime.now() - self.timestamp).total_seconds() > self.ttl:
                return True
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'data': self.data or self.payload or {},
            'source': self.source,
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'Event':
        ...
        return cls(
            ...
            expiry_time=datetime.fromisoformat(data.get('expiry_time')) if data.get('expiry_time') else None,
...
class EventDrivenSystem:
    ...
    def __init__(self,
        ...
        self.worker_count = 4  # 默认worker数量
    ...
    def processors(self):
        """获取所有命名处理器（用于测试兼容性）"""
        return list(self._event_processors.keys())
    ...
    async def unsubscribe(self, subscription_id: str) -> bool:
        ...
        Returns:
            是否成功取消订阅
        """
        # 解析订阅ID - 从右边开始分割，因为handler_id是数字
    ...
    def get_event_types(self) -> Set[EventType]:
        ...
        for handlers in self._event_handlers.values():
            for handler in handlers:
                if hasattr(handler, 'event_types') and handler.event_types:
                    event_types.update(handler.event_types)
        for processor in self._event_processors.values():
            if hasattr(processor.handler, 'event_types') and processor.handler.event_types:
                event_types.update(processor.handler.event_types)
        return event_types

    def get_system_info(self) -> Dict[str, Any]:
        """获取系统信息"""
        return {
            'name': getattr(self, 'name', 'EventDrivenSystem'),
            'is_running': getattr(self, 'is_running', False),
            'handler_count': sum(len(handlers) for handlers in self._event_handlers.values()),
            'processor_count': len(self._event_processors),
            'queue_size': self._event_queue.qsize(),
            'total_subscriptions': self._stats.get('active_subscriptions', 0),
            'events_published': self._stats.get('events_published', 0),
