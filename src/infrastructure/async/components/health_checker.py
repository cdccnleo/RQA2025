...
class HealthChecker:
    ...
    def add_health_check(self,
        ...
        Returns:
            bool: True if added successfully, False if component already exists
                 添加成功返回True，组件已存在返回False
        """
        if component_name in self.health_checks:
            logger.warning(f"Health check for {component_name} already exists")
            return False
        ...
        try:
            ...
            logger.error(f"Failed to add health check for {component_name}: {e}")
            return False
