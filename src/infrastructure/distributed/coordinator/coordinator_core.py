...
from .priority_engine import PriorityEngine, TaskPriority as PETaskPriority
from .queue_engine import QueueEngine, QueuePriority

# 获取统一基础设施 - 使用EventBusProxy避免循环依赖和提供降级机制
try:
    from ...core.integration.event_bus_proxy import EventBusProxy, get_event_bus_proxy
    # 延迟初始化，在需要时获取
    ...
    async def _get_event_bus():
        ...
        if _event_bus_proxy is None:
            _event_bus_proxy = await get_event_bus_proxy()
        return _event_bus_proxy
    
    event_bus_available = True
except ImportError as e:
    logger.warning(f"EventBusProxy import failed: {e}")
...
try:
    from ...core.integration import get_service_container
...
class DistributedCoordinator:
    ...
    def register_node(self, node_info: NodeInfo) -> bool:
        ...
        if result:
            # 发布节点注册事件（使用EventBusProxy）
            if event_bus_available:
                try:
                    import asyncio
                    event_bus_proxy = asyncio.run(_get_event_bus())
                    if event_bus_proxy:
                        event_bus_proxy.publish_sync({
                    ...
                    logger.warning(f"Failed to publish node registration event: {e}")
    ...
    def unregister_node(self, node_id: str) -> bool:
        ...
        if result and event_bus_available:
            try:
                import asyncio
                event_bus_proxy = asyncio.run(_get_event_bus())
                if event_bus_proxy:
                ...
                logger.warning(f"Failed to publish node unregistration event: {e}")
        ...
        try:
            ...
            if success:
                ...
                if task_to_preempt in self.tasks:
                    ...
                    # 重新加入队列等待调度
                    queue_priority = self._convert_task_priority_to_queue_priority(preempted_task.priority)
                    self.queue_engine.enqueue_task(
                        task_id=preempted_task.task_id,
                        task_data={'original_task': preempted_task},
            ...
            for task_id in tasks_to_reassign:
                if task_id in self.tasks:
                    ...
                    if task_id not in self.task_queue:
                        ...
                        # 同时加入队列引擎
                        queue_priority = self._convert_task_priority_to_queue_priority(task.priority)
                        self.queue_engine.enqueue_task(
                            task_id=task.task_id,
                            task_data={'original_task': task},
                            priority=queue_priority
                        )
    ...
    def _convert_task_priority_to_queue_priority(self, task_priority) -> QueuePriority:
        """将任务优先级转换为队列优先级"""
        priority_map = {
            ...
            TaskPriority.BACKGROUND: QueuePriority.BACKGROUND
        }
        return priority_map.get(task_priority, QueuePriority.NORMAL)
