...
class CommonComponents(ComponentFactory):
    """通用组件工厂类
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self._component_type = "Common"
        self._components = {}

    def create_component(self, component_id: int, **kwargs) -> Any:
        """创建通用组件"""
        if component_id not in CommonComponentConstants.SUPPORTED_COMMON_IDS:
            raise ValueError(f"Unsupported common component ID: {component_id}")

        # 使用工厂方法创建组件
        factory_method_name = f"create_common_common_component_{component_id}"
    ...
    def get_supported_components(self) -> List[int]:
        """获取支持的组件ID列表"""
        return CommonComponentConstants.SUPPORTED_COMMON_IDS.copy()
