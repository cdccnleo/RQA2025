...
class HelperComponents(ComponentFactory):
    """辅助组件工厂类
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self._component_type = "Helper"
        self._components = {}

    def create_component(self, component_id: int, **kwargs) -> Any:
        """创建辅助组件"""
        if component_id not in HelperComponentConstants.SUPPORTED_HELPER_IDS:
            raise ValueError(f"Unsupported helper component ID: {component_id}")

        # 使用工厂方法创建组件
        factory_method_name = f"create_helper_helper_component_{component_id}"
        if hasattr(self, factory_method_name):
            factory_method = getattr(self, factory_method_name)
            return factory_method(**kwargs)
        else:
            raise NotImplementedError(f"Factory method {factory_method_name} not implemented")

    def get_supported_components(self) -> List[int]:
        """获取支持的组件ID列表"""
        return HelperComponentConstants.SUPPORTED_HELPER_IDS.copy()
