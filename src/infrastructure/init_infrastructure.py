...
class Infrastructure:
    ...
    def __init__(self):
        ...
        with self.__class__._lock:
            ...
            # 标记为正在初始化
            Infrastructure._initialized = True
            ...
            try:
                ...
                self._logger.info("Infrastructure layer initialized successfully")
            except Exception as e:
                # 如果初始化失败，重置标志以允许重试
                Infrastructure._initialized = False
                self._logger.error(f"Infrastructure layer initialization failed: {e}")
                raise
...
# 全局基础设施访问点 - 延迟初始化，避免导入时就创建实例
infra = None

def _get_global_infra() -> Infrastructure:
    """获取全局基础设施实例（延迟初始化）"""
    global infra
    if infra is None:
