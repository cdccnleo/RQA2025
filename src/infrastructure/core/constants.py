...
class ConfigConstants:
    ...
    MAX_RETRIES = CONFIG_LOAD_MAX_RETRIES  # 兼容性别名
...
class LoggingConstants:
    ...
    MAX_LOG_FILE_SIZE = 10485760  # 10MB (覆盖测试中的期望值)
    MAX_LOG_BACKUP_FILES = 10

    # 日志文件大小
    MAX_LOG_FILE_SIZE_ORIGINAL = 104857600  # 100MB
...
# 扩展配置常量（保留原有定义）
setattr(ConfigConstants, 'MAX_CONFIG_SIZE', 10485760)  # 10MB
setattr(ConfigConstants, 'MAX_KEY_LENGTH', 1000)
setattr(ConfigConstants, 'MAX_VALUE_LENGTH', 1048576)  # 1MB
setattr(ConfigConstants, 'CONFIG_CACHE_TTL', 300)  # 5分钟
setattr(ConfigConstants, 'CONFIG_REFRESH_INTERVAL', 60)  # 1分钟
...
class ErrorConstants:
    ...
    ERR_CONFIG_ERROR = 2000
    ERR_NETWORK_ERROR = 3000
...
class NotificationConstants:
    ...
    # 通知频率控制 (语义化)
    MAX_NOTIFICATIONS_PER_HOUR = 1000  # 每小时最大通知数
    NOTIFICATION_COOLDOWN_SECONDS = 60  # 通知冷却时间
