...
class BaseComponent(ABC):
    ...
    def initialize(self) -> bool:
        """初始化组件"""
        print(f"DEBUG: BaseComponent.initialize called for {self.name}")
        try:
        print(f"DEBUG: BaseComponent.initialize START - {self.name}", flush=True)
        sys.stdout.flush()
        try:
        try:
            self.set_status(ComponentStatus.INITIALIZING)
            print(f"DEBUG: About to call _initialize_impl for {self.name}")
            result = self._initialize_impl()
            print(f"DEBUG: _initialize_impl returned {result} for {self.name}")
            if result:
            ...
            print(f"DEBUG: Exception in initialize for {self.name}: {e}")
            import traceback
            traceback.print_exc()
            self.set_status(ComponentStatus.ERROR)
            self.set_health(ComponentHealth.UNHEALTHY)
