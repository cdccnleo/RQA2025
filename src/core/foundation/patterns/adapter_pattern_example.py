...
class AdapterRegistry:
    ...
    def register(self, adapter_id: str, adapter: BaseAdapter) -> bool:
        """注册适配器"""
...
# 异步适配器别名
AsyncDataAdapter = DataAdapter
AsyncServiceAdapter = ServiceAdapter
AsyncProtocolAdapter = ProtocolAdapter
...
class AdapterManager:
    ...
    def add_adapter(self, adapter: BaseAdapter) -> bool:
        return self.registry.register(adapter.adapter_id, adapter)
    
    def get_adapter(self, adapter_id: str) -> BaseAdapter:
...
class AdapterHealthChecker:
    ...
    async def check(self, adapter: BaseAdapter) -> Dict[str, Any]:
...
class AdapterLoadBalancer:
    ...
    def get_next_adapter(self) -> BaseAdapter:
        if not self._adapters:
            return None
        adapter = self._adapters[self._current_index]
...
class AdapterRateLimiter:
    ...
    def is_allowed(self, adapter_id: str) -> bool:
        ...
        if adapter_id not in self.requests:
            self.requests[adapter_id] = []
        
        # 清理过期记录
        self.requests[adapter_id] = [t for t in self.requests[adapter_id] 
                              if current_time - t < self.time_window]
        
        # 检查是否超限
        if len(self.requests[adapter_id]) >= self.max_requests:
            return False
        
        self.requests[adapter_id].append(current_time)
        return True
    
    def reset(self, adapter_id: str):
        """重置限流计数"""
        if adapter_id in self.requests:
            self.requests[adapter_id] = []
