...
class ServiceFramework:
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            logger.info("服务框架已初始化")
            return True
        except Exception as e:
            logger.error(f"服务框架初始化失败: {e}")
            return False

    def register_service(self, name: str, service: IService) -> bool:
        """注册服务"""
        try:
        ...
        try:
            for name, service in self.services.items():
                try:
                    ...
                    logger.warning(f"停止服务失败 {name}: {e}")
...
# 创建默认实例
default_service_framework = ServiceFramework()

__all__.extend(['ServiceFramework'])
