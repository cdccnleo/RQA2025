...
from src.database_service import get_database_service, DatabaseService
...
class TradingAPIService:
    ...
    async def get_market_data(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            # 模拟市场数据（实际应该从数据源获取）
            base_price = 50.0 + (hash(symbol) % 100)  # 基于股票代码生成基准价格
            current_price = base_price * (1 + (time.time() % 10 - 5) / 100)
...
def _configure_other_routes(app: FastAPI, api_service):
    ...
    async def get_orders(
        ...
        orders = await api_service.get_orders(current_user["user_id"], status=status, limit=limit)
        return APIResponse(
            success=True,
            message="获取订单成功",
            data=orders
    ...
    # 其他路由配置可以在这里继续添加
    # 由于_configure_routes函数已被重构，所有路由现在分布在各个专用配置函数中
