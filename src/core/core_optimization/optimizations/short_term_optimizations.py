...
from src.core.constants import (
    ...
    DEFAULT_BATCH_SIZE, MAX_DATA_SIZE_BYTES, MAX_QUEUE_SIZE
)
...
class PerformanceMonitor(BaseComponent):
    ...
    def __init__(self, monitoring_interval: int = SECONDS_PER_MINUTE):
...
class BestPracticeGenerator(Protocol):
    """最佳实践生成器协议"""
    def generate_best_practices(self) -> List[Dict[str, Any]]: ...


class DocumentationUpdater(Protocol):
    """文档更新器协议"""
...
class ContentGenerator(Protocol):
    """内容生成器协议"""
    def generate_event_bus_example(self) -> str: ...
    def generate_container_example(self) -> str: ...
    def generate_orchestrator_example(self) -> str: ...
    def generate_event_bus_best_practices(self) -> str: ...
    def generate_container_best_practices(self) -> str: ...
...
class ExampleGeneratorImpl:
    """示例生成器实现 - 职责：生成各种使用示例"""

    def __init__(self, config: DocumentationConfig, content_generator: ContentGenerator, examples_dir: Path):
        self.config = config
        ...
        examples = [
            {
                ...
                "content": self.content_generator.generate_event_bus_example(),
                "category": "core",
            },
            {
                "name": "依赖注入容器使用示例",
                "file": "container_example.md",
                "content": self.content_generator.generate_container_example(),
                "category": "core",
            },
            {
                "name": "业务流程编排器使用示例",
                "file": "orchestrator_example.md",
                "content": self.content_generator.generate_orchestrator_example(),
                "category": "core",
            },
        ]
...
class BestPracticeGeneratorImpl:
    """最佳实践生成器实现 - 职责：生成最佳实践文档"""

    def __init__(self, config: DocumentationConfig, content_generator: ContentGenerator, best_practices_dir: Path):
        self.config = config
        self.content_generator = content_generator
        self.best_practices_dir = best_practices_dir
    ...
    def generate_best_practices(self) -> List[Dict[str, Any]]:
        ...
        best_practices = [
            {
                ...
                "content": self.content_generator.generate_event_bus_best_practices(),
                "category": "core",
            },
            {
                "name": "依赖注入最佳实践",
                "file": "container_best_practices.md",
                "content": self.content_generator.generate_container_best_practices(),
                "category": "core",
            },
        ]
        ...
        logger.info(f"生成了 {len(best_practices)} 个最佳实践文档")
        return best_practices


class ContentGeneratorImpl:
    """内容生成器实现 - 职责：生成具体的内容"""
...
# 发布事件
event_bus.emit('user_registered', {'username': 'john_doe'})
```
...
event_bus.on('async_event', async_handler)
await event_bus.emit_async('async_event', {'message': 'test'})
```
"""
...
# 创建容器
container = ServiceContainer('my_app')
...
# 定义业务流程
@orchestrator.process('user_registration')
...
# 执行流程
result = orchestrator.execute('user_registration', user_data)
```
...
```python
# 监控流程状态
status = orchestrator.get_process_status('user_registration')
...
1. **单一职责**: 每个事件处理器只负责一个具体的业务逻辑
2. **异步优先**: 优先使用异步事件处理，避免阻塞
3. **错误处理**: 在事件处理器中妥善处理异常，避免影响其他处理器
...
class UserService:
    def __init__(self, event_bus):
        ...
        self.event_bus.on('user_created', self._on_user_created)

    async def _on_user_created(self, data):
        try:
            # 异步处理用户创建后的逻辑
            await self.send_welcome_email(data['user_id'])
            await self.update_user_stats()
        except Exception as e:
            logger.error(f"处理用户创建事件失败: {e}")
```
"""
...
class OrderService:
    def __init__(self):
        self.repository = ServiceLocator.get('repository')
```
...
class DocumentationUpdaterImpl:
    """文档更新器实现 - 职责：更新和维护文档"""

    def __init__(self, config: DocumentationConfig, docs_dir: Path):
        self.config = config
...
class DocumentationEnhancer(BaseComponent):
    ...
    def __init__(self, docs_dir: str = "docs"):
        ...
        # 初始化配置
        self.config = DocumentationConfig(docs_dir=docs_dir)
        ...
        # 初始化专门的组件
        self.content_generator = ContentGeneratorImpl()
        self.example_generator = ExampleGeneratorImpl(self.config, self.content_generator, self.examples_dir)
        self.best_practice_generator = BestPracticeGeneratorImpl(self.config, self.content_generator, self.best_practices_dir)
        self.doc_updater = DocumentationUpdaterImpl(self.config, self.docs_dir)

        logger.info("重构后的文档增强器初始化完成")
    ...
    def generate_best_practices(self) -> List[Dict[str, Any]]:
        """生成最佳实践 - 代理到最佳实践生成器"""
        return self.best_practice_generator.generate_best_practices()

    def update_documentation(self):
        """更新文档 - 代理到文档更新器"""
        return self.doc_updater.update_documentation()

    # 保持向后兼容性
    def _generate_event_bus_example(self) -> str:
        """生成事件总线示例（向后兼容）"""
        return self.content_generator.generate_event_bus_example()

    def _generate_container_example(self) -> str:
        """生成容器示例（向后兼容）"""
        return self.content_generator.generate_container_example()

    def _generate_orchestrator_example(self) -> str:
        """生成编排器示例（向后兼容）"""
        return self.content_generator.generate_orchestrator_example()

    def _generate_event_bus_best_practices(self) -> str:
        """生成事件总线最佳实践（向后兼容）"""
        return self.content_generator.generate_event_bus_best_practices()

    def _generate_container_best_practices(self) -> str:
        """生成容器最佳实践（向后兼容）"""
        return self.content_generator.generate_container_best_practices()
...
class TestingEnhancer(BaseComponent):
    ...
    def _generate_event_bus_boundary_tests(self) -> str:
        """生成事件总线边界测试 - 重构版：拆分职责"""
        header = self._template_generator.generate_file_header("事件总线边界测试")
        imports = "from src.core import EventBus, EventType, EventPriority\n"
        class_start = self._template_generator.generate_test_class_start("TestEventBusBoundary", "事件总线边界测试")
...
'''
    ...
    def _generate_test_large_event_data(self) -> str:
        """生成大事件数据测试 - 职责：生成单个测试方法"""
        return '''    def test_large_event_data(self):
        """测试大事件数据"""
        event_bus = EventBus()
        large_data = {"data": "x" * MAX_DATA_SIZE_BYTES}  # 1MB数据
...
'''
    ...
    def _generate_test_high_frequency_events(self) -> str:
        ...
        # 确保1000个事件能在合理时间内处理
        assert duration < DEFAULT_BATCH_SIZE

'''

    def _generate_test_concurrent_events(self) -> str:
        """生成并发事件测试 - 职责：生成单个测试方法"""
...
'''
    ...
    def _generate_container_boundary_tests(self) -> str:
        """生成容器边界测试 - 重构版：拆分职责"""
        header = self._template_generator.generate_file_header("容器边界测试")
        imports = "from unittest.mock import Mock\nfrom src.core import DependencyContainer, Lifecycle\n"
        class_start = self._template_generator.generate_test_class_start("TestContainerBoundary", "容器边界测试")
...
'''
    ...
    def _generate_test_large_number_of_services(self) -> str:
        ...
        # 注册1000个服务
        for i in range(MAX_RECORDS):
            container.register(f"service_{i}", lambda: f"service_{i}")
...
'''
    ...
    def _generate_orchestrator_boundary_tests(self) -> str:
        """生成编排器边界测试 - 重构版：拆分职责"""
        header = self._template_generator.generate_file_header("编排器边界测试")
        imports = "import time\nfrom src.core import BusinessProcessOrchestrator, BusinessProcessState\n"
        class_start = self._template_generator.generate_test_class_start("TestOrchestratorBoundary", "编排器边界测试")
...
'''
    ...
    def _generate_test_memory_limits(self) -> str:
        ...
        assert memory_usage < MAX_RECORDS  # 内存使用应该小于1GB
'''

    def _generate_event_bus_performance_tests(self) -> str:
        """生成事件总线性能测试 - 重构版：拆分职责"""
        header = self._template_generator.generate_file_header("事件总线性能测试")
        imports = "import time\nfrom src.core import EventBus, EventType, EventPriority\n"
        class_start = self._template_generator.generate_test_class_start("TestEventBusPerformance", "事件总线性能测试")
    ...
    def _generate_test_event_publishing_performance(self) -> str:
        ...
        duration = end_time - start_time
        events_per_second = MAX_QUEUE_SIZE / duration

        # 确保每秒能处理至少MAX_RECORDS个事件
        assert events_per_second > MAX_RECORDS

'''

    def _generate_test_event_subscription_performance(self) -> str:
        """生成事件订阅性能测试 - 职责：生成单个测试方法"""
        ...
        duration = end_time - start_time
        subscriptions_per_second = MAX_RECORDS / duration

        # 确保每秒能处理至少MAX_RETRIES个订阅
        assert subscriptions_per_second > MAX_RETRIES
'''
...
class TestContainerPerformance:
    ...
    def test_service_resolution_performance(self):
        ...
        duration = end_time - start_time
        resolutions_per_second = MAX_RECORDS / duration

        # 确保每秒能解析至少MAX_RECORDS个服务
        assert resolutions_per_second > MAX_RECORDS
'''

    def _generate_memory_usage_tests(self) -> str:
        """生成内存使用测试 - 重构版：拆分职责"""
        header = self._template_generator.generate_file_header("内存使用测试")
        ...
        class_start = self._template_generator.generate_test_class_start("TestMemoryUsage", "内存使用测试")
        
        # 生成各个测试方法
        test_methods = [
            self._generate_test_event_bus_memory_usage(),
            self._generate_test_container_memory_usage(),
        ]
        
        # 组合所有部分
        return header + imports + class_start + "".join(test_methods)

    def _generate_test_event_bus_memory_usage(self) -> str:
        """生成事件总线内存使用测试 - 职责：生成单个测试方法"""
        return '''    def test_event_bus_memory_usage(self):
        """测试事件总线内存使用"""
...
'''
    ...
    def _generate_test_container_memory_usage(self) -> str:
        """生成容器内存使用测试 - 职责：生成单个测试方法"""
        return '''    def test_container_memory_usage(self):
        """测试容器内存使用"""
        process = psutil.Process()
        initial_memory = process.memory_info().rss
...
'''
    ...
    def _generate_business_process_integration_tests(self) -> str:
        """生成业务流程集成测试 - 重构版：拆分职责"""
        header = self._template_generator.generate_file_header("业务流程集成测试")
        imports = "import time\nfrom src.core import BusinessProcessOrchestrator, BusinessProcessState\n"
        class_start = self._template_generator.generate_test_class_start("TestBusinessProcessIntegration", "业务流程集成测试")
        
        # 生成各个测试方法
        test_methods = [
            self._generate_test_complete_trading_cycle(),
            self._generate_test_process_pause_resume(),
        ]
        
        # 组合所有部分
        return header + imports + class_start + "".join(test_methods)

    def _generate_test_complete_trading_cycle(self) -> str:
        """生成完整交易周期测试 - 职责：生成单个测试方法"""
        return '''    def test_complete_trading_cycle(self):
        """测试完整交易周期"""
        orchestrator = BusinessProcessOrchestrator()
...
'''
    ...
    def _generate_test_process_pause_resume(self) -> str:
        """生成流程暂停恢复测试 - 职责：生成单个测试方法"""
        return '''    def test_process_pause_resume(self):
        """测试流程暂停和恢复"""
        orchestrator = BusinessProcessOrchestrator()
