...
def get_ml_process_builder() -> MLProcessBuilder:
    """
    获取ML流程构建器实例

    Returns:
        MLProcessBuilder: ML流程构建器实例
    """
    return MLProcessBuilder()


def create_process_from_template(template_name: str, config: Optional[Dict[str, Any]] = None) -> MLProcess:
    """
    从模板创建ML流程
    ...
    builder.from_template(template_name, config)
    return builder.build()


def quick_train_model(model_type: str = "random_forest", data_config: Optional[Dict[str, Any]] = None) -> str:
    """
    快速训练模型
    ...
    builder = MLProcessBuilder()
    builder.from_template("basic_training", config)
    return builder.build_and_submit()


def quick_predict(data_config: Optional[Dict[str, Any]] = None) -> str:
    """
    快速预测
    ...
    builder = MLProcessBuilder()
    builder.from_template("batch_prediction", config)
    return builder.build_and_submit()
...
__all__ = [
    "MLProcessBuilder",
    "ProcessPriority",
    "get_ml_process_builder",
    "create_process_from_template",
    "quick_train_model",
    "quick_predict"
]
