...
from infrastructure.logging.unified_logger import get_unified_logger  # 当前层级内部导入：统一日志器
from typing import Dict, List, Any, Optional
from infrastructure.monitoring import MetricsCollector  # 合理跨层级导入：基础设施层监控组件
from data.china.market_data import MarketData  # 合理跨层级导入：监控层需要市场数据进行调度优化
import numpy as np
import time
        ...
        while True:
            try:
                ...
                with self._lock:
                    ...
                    self.performance_metrics['avg_processing_time_ms'] = np.mean(
                        self.processing_times)
