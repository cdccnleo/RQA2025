...
class StrategyEvaluator:
    ...
    def calculate_performance_metrics(self, returns) -> Dict[str, float]:
        """计算绩效指标"""
        ...
        try:
            ...
            if isinstance(returns, list):
                if len(returns) > 0 and isinstance(returns[0], dict):
                    # 如果是交易列表，计算PNL序列
                    pnl_values = []
            ...
            if len(returns_series) > 1:
                # 对于PNL序列，直接求和计算总收益率
                total_pnl = returns_series.sum()
                total_return = total_pnl / \
                    abs(returns_series.iloc[0]) if returns_series.iloc[0] != 0 else 0
            else:
                total_return = 0

            annual_return = total_return * (252 / len(returns_series)
                                            ) if len(returns_series) > 0 else 0
    ...
    def calculate_risk_metrics(self, returns) -> Dict[str, float]:
        """计算风险指标"""
        if returns is None or len(returns) == 0:
            return {}
        ...
        try:
            ...
            sortino_ratio = excess_returns.mean() / downside_excess.std() * \
                np.sqrt(252) if downside_excess.std() > 0 else 0
            ...
            # 计算Beta（简化为波动率的相对值）
            beta = returns_array.std() / (returns_array.std() + 0.1) if returns_array.std() > 0 else 1.0
