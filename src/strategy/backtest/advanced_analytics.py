...
class MLIntegrationAnalyzer:
    ...
    """
    机器学习集成分析器
    符合架构设计：通过统一适配器工厂访问ML层服务，支持模型预测和分析
    """

    def __init__(self):
        """
        初始化ML集成分析器
        符合架构设计：优先使用统一适配器工厂访问ML层服务，降级到直接实例化模型
        """
        self.models = {}
        ...
        try:
            ...
            if adapter_factory:
                ...
                try:
                    ...
                    if self.ml_adapter:
                        # 通过ML层适配器获取MLCore和模型管理器（符合架构设计：模型分析层集成）
                        if hasattr(self.ml_adapter, 'get_ml_core'):
                            self.ml_core = self.ml_adapter.get_ml_core()
                        if hasattr(self.ml_adapter, 'get_model_manager'):
                            self.model_manager = self.ml_adapter.get_model_manager()
                        logger.info("MLIntegrationAnalyzer: ML层适配器已初始化（通过统一适配器工厂，符合架构设计：模型分析层集成）")
                except Exception as e:
                    logger.debug(f"MLIntegrationAnalyzer: ML层适配器初始化失败（降级到直接实例化模型）: {e}")
        except Exception as e:
            logger.debug(f"MLIntegrationAnalyzer: 统一适配器工厂初始化失败（降级到直接实例化模型）: {e}")
    ...
    def predict_price(self, symbol: str, features: pd.DataFrame) -> float:
        """
        预测价格
        符合架构设计：优先使用统一适配器工厂获取的ML层服务进行预测，降级到本地模型
        ...
        Returns:
            预测价格
        """
        # 优先使用统一适配器工厂获取的ML层服务进行预测（符合架构设计：模型分析层集成）
        if self.ml_core:
            try:
                # 通过MLCore进行预测（符合架构设计：统一适配器访问）
                if hasattr(self.ml_core, 'predict'):
                    prediction = self.ml_core.predict(features)
                    if isinstance(prediction, (list, np.ndarray)) and len(prediction) > 0:
                        prediction_value = float(prediction[0])
                        self.predictions[symbol] = prediction_value
                        logger.debug(f"MLIntegrationAnalyzer: 通过MLCore预测价格成功 {symbol}（符合架构设计：模型分析层集成）")
                        return prediction_value
            except Exception as e:
                logger.debug(f"MLIntegrationAnalyzer: 通过MLCore预测价格失败（降级到本地模型） {symbol}: {e}")
        
        # 降级方案：使用本地模型进行预测（如果统一适配器工厂不可用）
