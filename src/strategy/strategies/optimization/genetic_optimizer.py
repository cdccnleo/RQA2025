...
class GeneticOptimizer:
        ...
        for _ in range(self.population_size):
            ...
            for spec in self.gene_specs:
                if spec.gene_type == GeneType.FLOAT:
                    value = round(secrets.uniform(*spec.bounds), spec.precision)
                elif spec.gene_type == GeneType.INT:
                    value = secrets.randint(*spec.bounds)
                else:  # CATEGORICAL
                    value = secrets.choice(spec.categories)
    ...
    def crossover(self, parent1: Dict, parent2: Dict) -> Tuple[Dict, Dict]:
        ...
        # 随机选择交叉点
        crossover_point = secrets.randint(1, len(self.gene_specs) - 1)
    ...
    def mutate(self, chromosome: Dict) -> Dict:
        ...
        for spec in self.gene_specs:
            if secrets.random() < self.mutation_rate:
                if spec.gene_type == GeneType.FLOAT:
                    mutated[spec.name] = round(secrets.uniform(*spec.bounds), spec.precision)
                elif spec.gene_type == GeneType.INT:
                    mutated[spec.name] = secrets.randint(*spec.bounds)
                else:  # CATEGORICAL
                    mutated[spec.name] = secrets.choice(spec.categories)
        return mutated
    ...
    def evolve(self):
        ...
        while len(next_population) < self.population_size:
            # 选择两个父代
            parent1, parent2 = secrets.sample(parents, 2)

            # 交叉
            if secrets.random() < self.crossover_rate:
