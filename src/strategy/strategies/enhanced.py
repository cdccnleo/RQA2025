...
import pandas as pd
# from src.infrastructure.utils.tools import time_execution  # 注释掉不存在的导入
...
class EnhancedTradingStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        # 交易成本模型
        self.market_impact_model = getattr(self.p, 'market_impact_model', None)
        self.slippage_model = getattr(self.p, 'slippage_model', None)

        # 初始化指标为空的LineBuffer，需确保self.datas存在
        if hasattr(self, 'data') and hasattr(self.data, 'close') and self.data.close is not None:
            self.volatility = bt.indicators.StandardDeviation(
                self.data.close,
                period=getattr(self.p, 'volatility_window', 14)
            )
            self.atr = bt.indicators.ATR(self.data, period=getattr(self.p, 'volatility_window', 14))
            # 延迟计算，确保数据足够
    ...
    def _handle_buy_order(self, order):
        ...
        if self.atr is not None and len(self.atr) > 0:
            self.position_stop_loss = order.executed.price - \
                self.atr[0] * getattr(self.p, 'risk_multiplier', 2.0)
            self.position_take_profit = order.executed.price + \
                self.atr[0] * getattr(self.p, 'risk_multiplier', 2.0) * 2
        else:
    ...
    def next(self):
        """执行下一个时间步的逻辑"""
        # 确保数据线存在
        if not (hasattr(self, 'data') and hasattr(self.data, 'close') and hasattr(self.data, 'high') and hasattr(self.data, 'low')):
            return
        if self.data.close is None or not hasattr(self.data.close, '__getitem__'):
            return

        # 延迟初始化指标
        if self.volatility is None:
            if len(self.data) >= getattr(self.p, 'volatility_window', 14):
                self.atr = bt.indicators.ATR(
                    self.data, period=getattr(self.p, 'volatility_window', 14))
                self.volatility = bt.indicators.StandardDeviation(
                    self.data.close,
                    period=getattr(self.p, 'volatility_window', 14)
                )
        ...
        required_bars = getattr(self.p, 'volatility_window', 14) + 1
        ...
        if (
                ...
                or self.atr is None
                or not hasattr(self.volatility, '__len__')
                or not hasattr(self.atr, '__len__')
        ...
        try:
            if len(self.volatility) > 0 and len(self.atr) > 0:
                ...
                self.volatility[-1] if len(self.volatility) > 1 else current_vol
                current_atr = self.atr[0]
                self.atr[-1] if len(self.atr) > 1 else current_atr
            else:
                return
        except Exception:
            return
    ...
    def _update_trailing_stop(self):
        ...
        if self.position_trail_price:
            new_trail = self.data.close[0] - self.atr[0] * getattr(self.p, 'trail_offset', 0.05)
    ...
    def _check_max_drawdown(self):
        ...
        if current_value < self.position_entry_value * (1 - getattr(self.p, 'max_drawdown_pct', 0.2)):
            self.sell()

    def _perform_stress_test(self):
        """执行压力测试"""
        self.stress_test_counter += 1
        if self.stress_test_counter >= getattr(self.p, 'stress_test_period', 30):
    ...
    def _adjust_risk_parameters(self):
        ...
        if self.market_state_changes > 3:
            ...
            setattr(self.p, 'position_size_pct', 0.02)
            setattr(self.p, 'risk_multiplier', 1.0)
    ...
    def _pass_volatility_filter(self) -> bool:
        ...
        if self.volatility is None or not hasattr(self.volatility, '__len__') or len(self.volatility) < 2:
    ...
    def _determine_market_state(self):
        ...
        if (self.volatility is None or self.atr is None
                or not hasattr(self.volatility, '__len__') or not hasattr(self.atr, '__len__')
                or len(self.volatility) < 2 or len(self.atr) < 2):
            return
        try:
    ...
    def _execute_new_trade(self):
        """执行新交易"""
        if self.atr is None or not hasattr(self.atr, '__len__') or len(self.atr) == 0:
            return
        try:
            risk_unit = self.broker.getvalue() * getattr(self.p, 'position_size_pct', 0.1)
            position_size = risk_unit / (self.atr[0] * getattr(self.p, 'risk_multiplier', 2.0))
        except Exception:
            return
    ...
    def _get_trading_signal(self) -> bool:
        ...
        if not (hasattr(self, 'data') and hasattr(self.data, 'close')):
            return False
        if self.data.close is None or not hasattr(self.data.close, '__getitem__'):
            return False
...
class StrategyExecutor:
    ...
    def _setup_cerebro(self):
        ...
        self.cerebro.adddata(bt.feeds.PandasData(self.data))
        self.cerebro.broker.setcash(100000.0)
        self.cerebro.broker.setcommission(commission=0.001)
        self.cerebro.addanalyzer(Backtester, _name="backtest_analyzer")
