...
class OrderBookAnalyzer:
    ...
    def _register_order_book_features(self):
        ...
        self.engine.register_feature(FeatureRegistrationConfig(
            ...
            feature_type=FeatureType.ORDERBOOK,
            params={"window": 20},  # 默认窗口大小
            dependencies=["bid_volume", "ask_volume"],
            a_share_specific=False
        ))
        ...
        # A股特有特征
        if hasattr(self.config, 'a_share_specific') and self.config.a_share_specific:
    ...
    def calculate_impact_cost(self, bid_vol: np.ndarray, ask_vol: np.ndarray) -> Dict[str, float]:
        """计算大单冲击成本"""
        window_size = getattr(self.config, 'window_size', 20)  # 默认窗口大小
        bid_large, ask_large = self._calculate_impact_cost_numba(
            bid_vol, ask_vol, window_size
        )
        return {
    ...
    def calculate_a_share_features(self, order_book: Dict[str, np.ndarray],
        ...
        """计算A股特有订单簿特征"""
        if not getattr(self.config, 'a_share_specific', False):
            return {}
    ...
    def calculate_all_features(self, order_book: Dict[str, np.ndarray],
        ...
        # A股特有特征
        a_share_features = self.calculate_a_share_features(
            order_book, prev_close) if prev_close else {}
