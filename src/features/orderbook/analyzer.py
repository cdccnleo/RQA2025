...
class OrderbookAnalyzer:
    ...
    def calculate_metrics(self, symbol: str) -> Dict[str, float]:
        ...
        if orderbook['asks'] and orderbook['bids']:
            # 使用round避免浮点数精度问题
            metrics['spread'] = round(orderbook['asks'][0][0] - orderbook['bids'][0][0], 6)

        # 计算订单簿不平衡度
        metrics['imbalance'] = self._calculate_imbalance(orderbook)

        # 计算订单簿深度
        depth_result = self._calculate_depth(orderbook)
        if depth_result:  # 只有在有深度数据时才添加
            metrics['depth'] = depth_result
    ...
    def _calculate_depth(self, orderbook: Dict) -> Dict[str, float]:
        ...
        for level in depth_levels:
            ...
            bid_depth = sum(
                vol for price, vol in orderbook['bids'] if price >= mid_price - price_range)
            ask_depth = sum(
                vol for price, vol in orderbook['asks'] if price <= mid_price + price_range)
    ...
    def register_features(self):
        """向特征管理器注册订单簿特征"""
        self.feature_manager._register_feature_config(FeatureRegistrationConfig(
