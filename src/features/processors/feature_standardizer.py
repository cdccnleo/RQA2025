...
from ..utils.sklearn_imports import (
    StandardScaler, MinMaxScaler, RobustScaler, NotFittedError
)
...
class FeatureStandardizer:
    ...
    def __init__(self, model_path: Path, method: str = "standard"):
        """初始化标准化器"""
        ...
        if is_training:
            ...
            try:
                ...
                if not self.scaler_path.exists():
                    self.logger.warning(f"标准化器文件保存失败，路径: {self.scaler_path}")
                    # 不抛出异常，继续执行
                else:
                    self.logger.info(f"标准化器保存成功: {self.scaler_path}")

            except Exception as e:
                self.logger.warning(f"标准化器保存失败: {str(e)}")
                # 不抛出异常，继续执行
    ...
    def standardize_features(self, features: pd.DataFrame, config: Optional[Any] = None) -> pd.DataFrame:
        """标准化特征（别名方法）"""
        return self.fit_transform(features, is_training=True)
    ...
    def inverse_transform(self, features: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            inverse_data = self.scaler.inverse_transform(features)
            return pd.DataFrame(
                inverse_data,
            ...
            self.logger.error(f"逆变换失败: {str(e)}")
            raise
    ...
    def partial_fit(self, features: pd.DataFrame):
        ...
        if not numeric_features.empty:
            if hasattr(self.scaler, 'partial_fit'):
                self.scaler.partial_fit(numeric_features)
                self.is_fitted = True
            else:
                self.logger.warning(f"{self.method}标准化器不支持增量拟合")
