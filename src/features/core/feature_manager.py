...
class FeatureManager:
    ...
    def __init__(self, config: Optional[FeatureConfig] = None):
        ...
        # 为FeatureStandardizer提供model_path
        model_path = Path(self.config.cache_dir) / "models"
        self.feature_standardizer = FeatureStandardizer(model_path)
    ...
    def process_features(self, data: Union[pd.DataFrame, Dict[str, pd.DataFrame]],
        ...
        if self.config.max_features and not processed_data.empty:
            # 暂时跳过特征选择，因为FeatureSelector接口不匹配
            # TODO: 实现特征选择逻辑
            logger.info(f"特征选择暂未实现，保留所有{len(processed_data.columns)}个特征")
