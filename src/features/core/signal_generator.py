...
try:
    from ..acceleration.fpga import FpgaManager
except ImportError:
    # 如果FPGA模块不可用，使用模拟实现
    class FpgaManager:
        def __init__(self):
...
class SignalGenerator:
    ...
    def _init_a_share_rules(self):
        ...
        self.t1_settlement = True  # T + 结算制度
    ...
    def _check_a_share_restrictions(self, symbol: str, signal_type: str) -> bool:
        ...
        if signal_type == "BUY":
            if symbol.startswith("688"):
                self.price_limit_rules["688"]
            elif symbol.startswith(("ST", "*ST")):
                self.price_limit_rules["ST"]
            else:
                self.price_limit_rules["normal"]
