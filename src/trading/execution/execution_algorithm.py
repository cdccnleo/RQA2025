...
class TWAPAlgorithm(BaseExecutionAlgorithm):
    ...
    def _execute_single_order(self, order: Dict[str, Any], algo_params: Optional[Dict[str, Any]] = None) -> List[ExecutionSlice]:
        ...
        for i in range(slices_count):
            # 添加一些价格随机性
            import random
            price_variation = random.uniform(-0.01, 0.01) if self.config.randomize_timing else 0
    ...
    def calculate_next_slice(self, remaining_quantity: int, time_remaining: float) -> Optional[ExecutionSlice]:
        ...
        Args:
            remaining_quantity: 剩余执行数量
            time_remaining: 剩余时间（秒）

        Returns:
            下一次执行的切片，如果无法计算则返回None
        """
        if remaining_quantity <= 0 or time_remaining <= 0:
            return None

        # TWAP策略：根据剩余时间比例分配数量
        # 假设总时间是120秒，当前剩余时间是time_remaining
        # 那么这次应该执行的比例是 (120 - time_remaining) / 120
        total_duration = getattr(self.config, 'duration', 120)  # 默认2分钟
        elapsed_time = total_duration - time_remaining
        if elapsed_time <= 0:
            # 初始状态，执行一半
            slice_quantity = remaining_quantity // 2
        else:
            # 根据时间比例计算
            time_ratio = elapsed_time / total_duration
            slice_quantity = int(remaining_quantity * time_ratio)

        slice_quantity = max(1, min(slice_quantity, remaining_quantity))
        ...
        return self._record_slice(slice_quantity, price, "twap_exchange")
