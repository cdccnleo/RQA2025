...
class ExecutionEngine:
    ...
    def submit_order(self, symbol: str, side: str, quantity: float,
        ...
        """提交订单（create_execution的别名）"""
        return self.create_execution(symbol, side, quantity, price, **kwargs)
    ...
    def get_order_status(self, execution_id: str) -> Optional[str]:
        """获取订单状态"""
        if execution_id in self.executions:
            return self.executions[execution_id].get('status')
        return None
    ...
    def get_execution_metrics(self) -> Dict[str, Any]:
        ...
        try:
            total_orders = len(self.executions)
            completed_orders = sum(1 for exec_info in self.executions.values()
                                 if exec_info.get('status') == ExecutionStatus.COMPLETED)
            pending_orders = sum(1 for exec_info in self.executions.values()
                               if exec_info.get('status') == ExecutionStatus.PENDING)
            ...
            return {
                ...
                'avg_execution_time': 0.05  # 模拟平均执行时间
            }
        except Exception as e:
