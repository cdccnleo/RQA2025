...
class Trade:
    def __init__(self, symbol, price, quantity, side, prev_close=0, close_price=0):
...
class SettlementConfig:
    ...
    """结算配置"""
    t1_settlement: bool = True  # 是否启用T + 结算
    freeze_ratio: float = 1.0  # 资金冻结比例
    settlement_time: str = "16:00"  # 结算时间
    a_share_fees: dict = field(default_factory=lambda: {
        "commission": 0.0003,  # 佣金费率 0.03%
        "stamp_duty": 0.001,   # 印花税费率 0.1%
        "transfer_fee": 0.00002  # 过户费率 0.002%
    })  # A股费用标准
    margin_rules: dict = field(default_factory=lambda: {
...
class SettlementEngine:
    ...
    def __init__(self, feature_engine: Any, config: Optional[SettlementConfig] = None):
    ...
    def process_t1_settlement(self, trades: List[Trade]) -> float:
        ...
        logger.info(f"T + 结算完成，冻结资金: {self.frozen_cash:.2f}元")
        return total_amount
...
class ChinaSettlementEngine(SettlementEngine):
    ...
    def __init__(self, feature_engine: Any, config: Optional[SettlementConfig] = None):
        ...
        self.config.t1_settlement = True  # 强制启用T+
    ...
    def process_t1_settlement(self, trades: List[Trade]) -> float:
        ...
        logger.info(f"A股T + 结算完成，冻结资金: {self.frozen_cash:.2f}元")
        return total_amount
