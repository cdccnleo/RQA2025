...
class PerformanceAnalyzer:
    ...
    def plot_performance(self, figsize: tuple = (12, 8)) -> plt.Figure:
        ...
        try:
            ...
            # 绘制收益率曲线
            if 'returns' in self._results:
                axes[0].plot(self._results['returns'].cumsum())
                axes[0].set_title('累计收益率')
                axes[0].set_ylabel('收益率')
                axes[0].grid(True)

            # 绘制回撤曲线
            if 'drawdown' in self._results:
                axes[1].fill_between(range(len(self._results['drawdown'])),
                                     self._results['drawdown'], 0, alpha=0.3, color='red')
                axes[1].set_title('回撤')
                axes[1].set_ylabel('回撤')
                axes[1].grid(True)

            # 绘制夏普比率
            if 'sharpe_ratio' in self._results:
                axes[2].bar(['夏普比率'], [self._results['sharpe_ratio']])
                axes[2].set_title('风险调整收益')
