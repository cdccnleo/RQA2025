...
class EnhancedCacheManager:
    ...
    def __init__(self, cache_dir: str = "cache", max_memory_size: int = 100 * 1024 * 1024,  # 100MB
        ...
        self._stop_consistency = False
        self._start_consistency_thread()
    ...
    def _start_consistency_thread(self):
        """启动一致性检查线程"""

        def consistency_worker():

            while not self._stop_consistency:
                try:
                    # 使用可中断的睡眠机制
                    for _ in range(self.consistency_check_interval):
                        if self._stop_consistency:
                            break
                        time.sleep(1)

                    if not self._stop_consistency:
                        self._check_consistency()
                except Exception as e:
                    try:
                        logger.error(f"Error in consistency thread: {e}")
                    except BaseException:
                        pass  # 忽略日志记录失败
            ...
            with self._lock:
                ...
                if prefix:
                    if prefix in self.prefix_index:
                        self.prefix_index[prefix].discard(cache_key)
    ...
    def shutdown(self):
        ...
        # 等待一致性检查线程结束
        if self._consistency_thread and self._consistency_thread.is_alive():
            self._consistency_thread.join(timeout=5.0)
            if self._consistency_thread.is_alive():
                logger.warning("增强版缓存管理器一致性检查线程未能及时停止")
        ...
        try:
            with self._lock:
                ...
                if os.path.exists(self.disk_cache_dir):
                    for filename in os.listdir(self.disk_cache_dir):
                        if filename.endswith('.pkl'):
                            cache_key = filename[:-4]  # 移除.pkl后缀
                            disk_keys.add(cache_key)

                # 检查内存中存在但磁盘中不存在的项
                memory_only_keys = memory_keys - disk_keys
                if memory_only_keys:
                    logger.debug(f"Memory-only keys found: {len(memory_only_keys)}")

                # 检查磁盘中存在但内存中不存在的项
                disk_only_keys = disk_keys - memory_keys
                if disk_only_keys:
                    logger.debug(f"Disk-only keys found: {len(disk_only_keys)}")
    ...
    def add_warmup_data(self, key: str, value: Any, expire: int = 3600, prefix: str = ""):
        """
        添加缓存预热数据
    ...
    def execute_warmup(self):
        ...
        try:
            with self._lock:
                count = 0
                for cache_key, data in self.warmup_data.items():
                    try:
                        self.set(data['key'], data['value'], data['expire'], data['prefix'])
                        count += 1
                    except Exception as e:
                        logger.error(f"Error warming up cache for key {data['key']}: {e}")

                self.stats['warmup_count'] = count
                logger.info(f"Cache warmup completed: {count} items")
        except Exception as e:
            logger.error(f"Error executing cache warmup: {e}")

    def batch_set(self, items: list) -> Dict[str, bool]:
        """
        批量设置缓存

        Args:
            items: 缓存项列表，每个项为 (key, value, expire, prefix) 元组

        Returns:
            Dict[str, bool]: 每个键的设置结果
        """
        results = {}
        try:
            with self._lock:
                for item in items:
                    if len(item) >= 2:
                        key = item[0]
                        value = item[1]
                        expire = item[2] if len(item) > 2 else 3600
                        prefix = item[3] if len(item) > 3 else ""
                        results[key] = self.set(key, value, expire, prefix)

                self.stats['batch_operations'] += 1
                logger.info(f"Batch set completed: {len(results)} items")
        except Exception as e:
            logger.error(f"Error in batch set: {e}")
        return results

    def batch_get(self, keys: list, prefix: str = "") -> Dict[str, Any]:
        """
        批量获取缓存
        ...
        try:
            with self._lock:
                ...
                self.stats['batch_operations'] += 1
                logger.info(f"Batch get completed: {len(results)} items")
        except Exception as e:
            logger.error(f"Error in batch get: {e}")
        return results

    def batch_delete(self, keys: list, prefix: str = "") -> Dict[str, bool]:
        """
        批量删除缓存
        ...
        try:
            with self._lock:
                ...
                self.stats['batch_operations'] += 1
                logger.info(f"Batch delete completed: {len(results)} items")
        except Exception as e:
