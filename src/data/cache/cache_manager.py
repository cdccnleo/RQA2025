...
class CacheManager:
    ...
    def __init__(self, config: CacheConfig = None, strategy: Optional[ICacheStrategy] = None):
        ...
        # 智能策略扩展点
        try:
            from .enhanced_cache_strategy import get_cache_strategy
            if strategy is None:
                # 默认使用自适应策略
                self.strategy = get_cache_strategy('adaptive')
            else:
                self.strategy = strategy
        except Exception as e:
            self.logger.warning(f"Failed to initialize enhanced cache strategy: {e}")
