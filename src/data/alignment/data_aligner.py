...
except Exception:  # pragma: no cover - 兼容不同导入路径
    from src.data.processing.data_processor import DataProcessor  # type: ignore
...
class DataAligner:
    ...
    def _convert_enums_to_strings(self, freq, method):
        """
        转换枚举为字符串
    ...
    def _ensure_datetime_index(self, data_frames: Dict[str, pd.DataFrame]) -> Dict[str, pd.DataFrame]:
        """
        确保所有数据框都有DatetimeIndex
    ...
    def _determine_date_range(
        ...
        Args:
            data_frames: 数据框字典
            method: 对齐方法
            start_date: 用户指定的开始日期（可选）
            end_date: 用户指定的结束日期（可选）
            
        Returns:
            (start_date, end_date): 确定的日期范围
            
        Raises:
            DataProcessingError: 如果对齐方法不支持
        """
        # 确定开始日期
        if start_date is not None:
            ...
            start_date = self._get_start_date_by_method(data_frames, method)
        ...
        if end_date is not None:
            ...
            end_date = self._get_end_date_by_method(data_frames, method)
        
        return start_date, end_date
    
    def _get_start_date_by_method(self, data_frames: Dict[str, pd.DataFrame], method: str):
        """根据对齐方法确定开始日期"""
        if method == 'outer':
            return min(df.index.min() for df in data_frames.values())
        elif method == 'inner':
            return max(df.index.min() for df in data_frames.values())
        elif method == 'left':
            return list(data_frames.values())[0].index.min()
        elif method == 'right':
            return list(data_frames.values())[-1].index.min()
        else:
            raise DataProcessingError(f"不支持的对齐方法: {method}")
    
    def _get_end_date_by_method(self, data_frames: Dict[str, pd.DataFrame], method: str):
        """根据对齐方法确定结束日期"""
        if method == 'outer':
            return max(df.index.max() for df in data_frames.values())
        elif method == 'inner':
            return min(df.index.max() for df in data_frames.values())
        elif method == 'left':
            return list(data_frames.values())[0].index.max()
        elif method == 'right':
            return list(data_frames.values())[-1].index.max()
        else:
            raise DataProcessingError(f"不支持的对齐方法: {method}")
    ...
    def _apply_fill_method(
        ...
        if source_fill_method:
            return self.processor.fill_missing(aligned_df, method=source_fill_method)
    ...
    def align_time_series(
        ...
        此方法已重构以降低复杂度，原复杂度为25，现在使用辅助方法简化逻辑。
        ...
        try:
            ...
            self._record_alignment(
                data_frames, aligned_frames, freq, method, fill_method, start_date, end_date
            )
        ...
        try:
            ...
            for name, df in target_dfs.items():
                ...
                if fill_method:
                    ...
                    if source_fill_method:
                        aligned_df = self.processor.fill_missing(
                            aligned_df, method=source_fill_method)
        ...
        try:
            ...
            for name, df in aligned_frames.items():
                ...
                if fill_method:
                    ...
                    if source_fill_method:
                        aligned_frames[name] = self.processor.fill_missing(
                            aligned_frames[name], method=source_fill_method)
