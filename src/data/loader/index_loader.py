...
try:
    from src.infrastructure.error import DataLoaderError  # 兼容旧路径
except ImportError:
...
class IndexDataLoader(BaseDataLoader):
    ...
    def __init__(
        self,
        save_path: Union[str, Path] = "data/index",
        max_retries: int = 3,
        cache_days: int = 30,
        thread_pool=None,
    ):
    ...
    def create_from_config(cls, config: Union[configparser.ConfigParser, dict], thread_pool=None):
        ...
        if isinstance(config, configparser.ConfigParser):
            loader_config = config['Index'] if 'Index' in config else {}
        ...
        default_loader_config = default_config['Index'] if 'Index' in default_config else {}

        # 安全获取配置值
        save_path = loader_config.get('save_path', default_loader_config.get(
    ...
    def load_data(
        ...
        file_path = self._get_file_path(index_code, start_date.strftime(
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                # 合并缓存数据和新数据
                if file_path.exists():
                    cached_data = pd.read_csv(file_path, encoding='utf - 8',
    ...
    def load_single_index(
        ...
        mapped_code = self._get_index_code(index_name)
        end_date = end_date or datetime.now().strftime("%Y-%m-%d")
        start_date = start_date or (datetime.now() - timedelta(days=30)).strftime("%Y-%m-%d")

        cache_file = self._get_cache_file_path(index_name)
        if not force_refresh:
            cached_payload = self._load_cache_payload(cache_file)
            if cached_payload:
                cached_payload.setdefault("metadata", {})
                cached_payload["metadata"].setdefault("index_name", index_name)
                cached_payload["metadata"].setdefault("index_code", mapped_code)
        ...
        raw_df = self._fetch_raw_data(mapped_code, start_date, end_date)
        processed_df = self._process_raw_data(raw_df)
        is_valid, errors = self._validate_index_data(processed_df.reset_index())
        if not is_valid:
            raise DataLoaderError(f"数据验证失败: {errors}")

        metadata = {
            "index_name": index_name,
            "index_code": mapped_code,
    ...
    def _load_single_index_with_cache(
        self,
        index_name: str,
        start_date: Optional[str] = None,
        end_date: Optional[str] = None,
        ...
        return self.load_single_index(index_name, start_date, end_date, force_refresh)

    def load_multiple_indexes(self, index_list: List[str], max_workers: int = 4) -> Dict[str, Dict[str, Any]]:
        if not index_list:
            return {}

        def task(name: str):
            try:
                return name, self._load_single_index_with_cache(name)
            except Exception as exc:
                logger.error("加载指数 %s 失败: %s", name, exc)
        ...
        if use_thread_pool:
            futures = [self.thread_pool.submit(task, name) for name in index_list]
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
                for key, value in executor.map(task, index_list):
                    if "error" not in value:
                        results[key] = value
    ...
    def _fetch_raw_data(self, index_symbol: str, start_date: str, end_date: str) -> pd.DataFrame:
        ...
        if not AKSHARE_AVAILABLE:
            raise DataLoaderError("akshare不可用，无法获取指数数据")
    ...
    def _get_index_code(self, index_name: str) -> str:
        return self.INDEX_MAPPING.get(index_name, index_name)

    def _get_cache_key(self, index_name: str) -> str:
        return f"{index_name}_index"

    def _get_cache_file_path(self, index_name: str) -> Path:
        mapped_code = self._get_index_code(index_name)
        return self.cache_dir / f"{mapped_code}_index.pkl"
    ...
    def _validate_index_data(self, data: Any) -> Tuple[bool, List[str]]:
        errors: List[str] = []
        if data is None:
        ...
        normalized = data.rename(columns=column_map).copy()
        required_columns = ['date', 'open', 'high', 'low', 'close', 'volume']
        ...
        if 'volume' in normalized.columns:
            ...
            if numeric_volume.isna().all():
                errors.append("volume contains non-numeric values")
            elif (numeric_volume < 0).any():
                errors.append("volume contains negative values")
        return len(errors) == 0, errors

    def _validate_index_list(self, index_list: List[str]) -> bool:
        return all(index in self.INDEX_MAPPING for index in index_list)
